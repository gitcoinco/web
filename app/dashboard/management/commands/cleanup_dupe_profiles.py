...
def combine_profiles(p1, p2):
    ...
    p1.github_access_token = (
        p2.github_access_token if p2.github_access_token else p1.github_access_token
    )
    ...
    p1.preferred_payout_address = (
        p2.preferred_payout_address
        if p2.preferred_payout_address
        else p1.preferred_payout_address
    )
    p1.max_tip_amount_usdt_per_tx = max(
        p1.max_tip_amount_usdt_per_tx, p2.max_tip_amount_usdt_per_tx
    )
    p1.max_tip_amount_usdt_per_week = max(
        p1.max_tip_amount_usdt_per_week, p2.max_tip_amount_usdt_per_week
    )
    p1.max_num_issues_start_work = max(
        p1.max_num_issues_start_work, p2.max_num_issues_start_work
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        dupes = (
            Profile.objects.exclude(handle=None)
            .annotate(handle_lower=Lower("handle"))
            .values("handle_lower")
            .annotate(handle_lower_count=Count("handle_lower"))
