...
def get_bio(handle):
    try:
        ...
        return profile.data.get("location", "unknown"), profile.data.get(
            "bio", "unknown"
        )
    except Exception:
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "start_date",
            type=valid_date,
            help="Start of date range (inclusive) in YYYY/MM/DD format for activities to be collected",
        )
        parser.add_argument(
            "end_date",
            type=valid_date,
            help="End of date range (inclusive) in YYYY/MM/DD format for activities to be collected",
        )
    ...
    def extract_github_repo(self, url):
        ...
        if not match:
            self.stdout.write(
                self.style.WARNING(
                    "WARNING: malformed github url: %s, using value as is" % url
                )
            )
            return url
    ...
    def format_bounty(self, bounty):
        ...
        try:
            bounty_fulfillment = bounty.fulfillments.filter(accepted=True).latest(
                "created_on"
            )
    ...
    def handle(self, *args, **options):
        ...
        if has_rows:
            ...
            url = self.upload_to_s3(
                f"activity_report_{start}_{end}_generated_on_{now}.csv",
                csvfile.getvalue(),
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "Sent activity report from %s to %s to %s"
                    % (start, end, settings.CONTACT_EMAIL)
                )
            )
        else:
            self.stdout.write(
                self.style.WARNING("No activity from %s to %s to report" % (start, end))
            )
