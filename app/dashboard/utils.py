...
def getStandardBountiesContractAddresss(network, contract_version='1'):
    """Returns contract address based on network and contract_version
    ...
    if contract_version == '2':
        if network == 'mainnet':
            return to_checksum_address('0xe7f69ea2a79521136ee0bf3c50f6b5f1ea0ab0cd')
        elif network == 'rinkeby':
            return to_checksum_address('0xa53aadb09bd0612ee810ab8b4605c9ee45892169')
        raise UnsupportedNetworkException(network)
    else:
        if network == 'mainnet':
...
def getBountyContract(network, contract_version='1'):
    """Returns Bounty Contract based on network and contract_version
    network : 'mainnet', 'rinkeby'
...
def get_bounty(bounty_enum, network, contract_version='1'):
...
def get_bounty_id(issue_url, network, contract_version='1'):
...
def get_bounty_id_from_web3(issue_url, network, start_bounty_id, direction='up', contract_version='1'):
    issue_url = normalize_url(issue_url)
