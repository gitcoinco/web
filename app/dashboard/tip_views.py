...
def request_money(request):
    ...
    if request.method == "POST":
        ...
        if profiles.exists():
            ...
            messages.success(
                request, f"Stay tuned, {profile.handle} has been notified by email."
            )
        else:
    ...
    params = {
        ...
        "preferred_payout_address": request.user.profile.preferred_payout_address
...
def record_tip_activity(
    tip, github_handle, event_name, override_created=None, other_handle=None
):
    kwargs = {
    ...
    try:
        ...
        logger.debug(
            "error in record_tip_activity: %s - %s - %s - %s",
            e,
            event_name,
            tip,
            github_handle,
...
def receive_tip_v3(request, key, txid, network):
    ...
    ) | these_tips.filter(metadata__reference_hash_for_funder=key)
    tip = tips.first()
    if not tip:
    ...
    is_authed = (
        request.user.username.lower() == tip.username.lower()
        or request.user.username.lower() == tip.from_username.lower()
        or not tip.username
    )
    not_mined_yet = (
        get_web3(tip.network).eth.getBalance(
            Web3.toChecksumAddress(tip.metadata["address"])
        )
        == 0
    )
    did_fail = False
    ...
    is_redeemable = (
        not (tip.receive_txid and (num_redemptions >= max_redemptions)) and is_authed
    )
    ...
    if not is_redeemable:
        ...
        messages.info(
            request,
            f"This tx {tip.txid}, failed.  Please contact the sender and ask them to send the tx again.",
        )
    elif not_mined_yet:
        messages.info(
            request,
...
def send_tip_4(request):
    ...
    is_authenticated_for_this_via_login = (
        tip.from_username and tip.from_username == from_username
    )
...
def send_tip_2(request):
    ...
    if profile:
        ...
        if profile.avatar_baseavatar_related.exists():
            user["avatar_id"] = (
                profile.avatar_baseavatar_related.filter(active=True).first().pk
            )
            user["avatar_url"] = (
                profile.avatar_baseavatar_related.filter(active=True).first().avatar_url
            )
