...
def get_polkadot_txn_status(txnid, token_name):
    ...
    try:
        ...
        if token_name == "DOT":
            polkascan_url = (
                f"https://explorer-31.polkascan.io/polkadot/api/v1/extrinsic/{txnid}"
            )
        elif token_name == "KSM":
            polkascan_url = (
                f"https://explorer-31.polkascan.io/kusama/api/v1/extrinsic/{txnid}"
            )
...
def sync_polkadot_payout(fulfillment):
    if fulfillment.payout_tx_id:
        txn_status = get_polkadot_txn_status(
            fulfillment.payout_tx_id, fulfillment.token_name
        )
        if txn_status:
