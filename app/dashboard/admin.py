...
class BountyFulfillmentAdmin(admin.ModelAdmin):
    ...
    search_fields = ["fulfiller_address", "fulfiller_metadata", "fulfiller_github_url"]
    ordering = ["-id"]
...
class ActivityAdmin(admin.ModelAdmin):
    ...
    def response_change(self, request, obj):
        ...
        if "_make_nano_bounty" in request.POST:
            ...
            self.message_user(
                request,
                "Nano bounty made - You still need to make it public + edit amounts tho.",
            )
...
class BountyInvitesAdmin(admin.ModelAdmin):
    ...
    def bounty_url(self, obj):
        ...
        return format_html(
            "<a href={}>{}</a>", mark_safe(bounty.url), mark_safe(bounty.url)
        )
    ...
    def response_change(self, request, obj):
        ...
        if "_squelch_sybil" in request.POST:
            ...
            SquelchProfile.objects.create(
                profile=obj, comments=f"squelched by {request.user.username}"
            )
...
class TipAdmin(admin.ModelAdmin):
    ...
    def resend(self, instance):
        html = format_html(
            '<a href="/_administration/email/new_tip/resend?pk={}">resend</a>',
            instance.pk,
        )
        return html
    ...
    def response_change(self, request, obj):
        ...
        if "_reset_tip_redemption" in request.POST:
            if not obj.receive_txid:
                self.message_user(
                    request,
                    f"Cannot reset tip! This tip has not been marked as receieved",
                )
                return redirect(obj.admin_url)
            obj.receive_txid = ""
            ...
            self.message_user(
                request,
                f"This tip redemption has been reset.  Please make sure {addr} has enough ETH in it to pay gas, and send the new claim link to customer.",
            )
            return redirect(obj.admin_url)
        return super().response_change(request, obj)
    ...
    def img(self, instance):
        if instance.admin_override_org_logo:
            return format_html(
                ...
                mark_safe(instance.admin_override_org_logo.url),
            )
        if not instance.avatar_url:
            return "n/a"
        return format_html(
            "<img src={} style='max-width:30px; max-height: 30px'>",
            mark_safe(instance.avatar_url),
        )
...
class SponsorAdmin(admin.ModelAdmin):
    ...
    def img(self, instance):
        ...
        img_html = format_html(
            '<img src={} style="width: auto; max-height: 40px">', mark_safe(logo.url)
        )
        return img_html
...
class HackathonEventAdmin(admin.ModelAdmin):
    ...
    def img(self, instance):
        ...
        if not logo:
            return "n/a"
        img_html = format_html(
            '<img src={} style="max-width:30px; max-height: 30px">', mark_safe(logo.url)
        )
        return img_html
