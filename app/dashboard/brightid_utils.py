...
def get_brightid_status(brightid_uuid):
    brightIDUrl = "https://app.brightid.org/node/v5/verifications/Gitcoin/" + str(
        brightid_uuid
    )
    ...
    try:
        ...
        isVerified = (
            responseData.get("data", {}).get("unique", False)
            and responseData.get("data", {}).get("context", "") == "Gitcoin"
        )
