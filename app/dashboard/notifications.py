...
def github_org_to_twitter_tags(github_org):
    ...
    """
    twitter_handles = GithubOrgToTwitterHandleMapping.objects.filter(
        github_orgname__iexact=github_org
    )
...
def maybe_market_to_twitter(bounty, event_name):
    ...
    if event_name == "remarket_bounty":
        ...
        tweet_txts = ["Worked rejected on {} {} {}\n{}"]
    elif event_name == "worker_approved":
        tweet_txts = ["Worked approved on {} {} {}\n{}"]
    ...
    new_tweet = (
        new_tweet + " " + github_org_to_twitter_tags(bounty.org_name)
    )  # twitter tags
...
def maybe_market_kudos_to_email(kudos_transfer):
    ...
    if (
        not kudos_transfer
        or not kudos_transfer.txid
        or not kudos_transfer.amount
        or not kudos_transfer.kudos_token_cloned_from
    ):
    ...
    from_name = (
        "Someone"
        if not kudos_transfer.from_username
        else f"{kudos_transfer.from_username}"
    )
    on_network = (
        "" if kudos_transfer.network == "mainnet" else f"({kudos_transfer.network})"
    )
    subject = gettext(
...
def build_github_notification(bounty, event_name, profile_pairs=None):
    ...
    try:
        usdt_value = (
            f"({round(bounty.value_in_usdt_now, 2)} USD @ ${round(convert_token_to_usdt(bounty.token_name), 2)}/{bounty.token_name})"
            if bounty.value_in_usdt_now
            else ""
        )
    except Exception:
    ...
    bounty_owner = (
        f"@{bounty.bounty_owner_github_username}"
    ...
    crowdfund_msg = (
        f"* Want to chip in? Add your own contribution [here]({absolute_url})."
    )
    openwork_msg = (
        f"* ${amount_open_work} more funded OSS Work available on the "
    ...
    claim_msg = (
        f"* If you want to claim the bounty you can do so " f"[here]({absolute_url})"
    )
    learn_more_msg = f"* Learn more [on the Gitcoin Issue Details page]({absolute_url})"
    crowdfund_amount = (
        f"(plus a crowdfund of {bounty.additional_funding_summary_sentence})"
        if bounty.additional_funding_summary_sentence
        else ""
    )
    crowdfund_thx = ", ".join(
        f"@{tip.from_username}"
        for tip in bounty.tips.filter(is_for_bounty_fulfiller=True)
        if tip.from_username
    )
    funding_org = (
        f" as part of the {bounty.funding_organisation} fund"
    ...
    if event_name == "new_bounty":
        msg = (
            f"{status_header}__This issue now has a funding of {natural_value} {bounty.token_name} {usdt_value} "
    ...
    if event_name == "increased_bounty":
        msg = (
            f"{status_header}__The funding of this issue was increased to {natural_value} {bounty.token_name} {usdt_value} "
            f"{crowdfund_amount}.__\n\n "
            f"\n{claim_msg}\n{crowdfund_msg}\n{help_msg}\n{openwork_msg}\n"
        )
    elif event_name == "killed_bounty":
        msg = (
            f"{status_header}__The funding of {natural_value} {bounty.token_name} {crowdfund_amount} "
            f"{usdt_value} attached to this issue has been **cancelled** by the bounty submitter__\n\n "
            f"\n{help_msg}\n{openwork_msg}"
        )
    elif event_name == "rejected_claim":
        msg = (
            f"{status_header}__The work submission for {natural_value} {bounty.token_name} {usdt_value} {crowdfund_amount} "
        ...
        if bounty.fulfillments.exists():
            ...
            for bf in bounty.fulfillments.all():
                username = (
                    ...
                    else bf.fulfiller_address
                )
                link_to_work = (
                    f"[PR]({bf.fulfiller_github_url})"
                    if bf.fulfiller_github_url
        ...
        msg = (
            f"{status_header}__Work for {natural_value} {bounty.token_name} {usdt_value} has been submitted by__:\n"
            f"{profiles}{sub_msg}\n<hr>\n\n"
            f"{learn_more_msg}\n{crowdfund_msg}\n{help_msg}\n{openwork_msg}"
        )
    elif event_name == "work_done":
        ...
        for tip in bounty.tips.filter(is_for_bounty_fulfiller=False).exclude(txid=""):
            msg_tips += (
                f"* {tip.from_username} tipped {tip.amount} {tip.tokenName} "
                f"worth {tip.value_in_usdt_now} USD to {tip.username}.\n"
            )

        try:
            accepted_fulfillment = bounty.fulfillments.filter(accepted=True).latest(
                "fulfillment_id"
            )
            accepted_fulfiller = (
                f" to @{accepted_fulfillment.fulfiller_github_username}"
            )
            msg_body = (
                f"__The funding of {natural_value} {bounty.token_name} {usdt_value} "
...
def open_bounties():
    ...
    return (
        ...
        .filter(network="mainnet", idx_status__in=["open", "submitted"])
        .cache()
    )
...
def maybe_market_tip_to_github(tip):
    ...
    """
    if (
        not tip.is_notification_eligible(var_to_check=settings.GITHUB_CLIENT_ID)
        or not tip.github_url
    ):
        return False
    ...
    _comments = (
        ...
        f"{tip.comments_public}"
        if tip.comments_public
        else ""
    )
    try:
        value_in_usd = (
            f"({tip.value_in_usdt_now} USD @ ${round(convert_token_to_usdt(tip.tokenName), 2)}/{tip.tokenName})"
    ...
    if tip.username:
        msg = (
            f"⚡️ A tip worth {round(tip.amount, 5)} {warning} {tip.tokenName} {value_in_usd} has been "
            f"granted to {username} for this issue{_from}. ⚡️ {_comments}\n\nNice work {username}! "
        )
        redeem_instructions = (
            "To redeem your tip, login to Gitcoin at https://gitcoin.co/explorer and select "
        ...
        msg = (
            f"💰 A crowdfund contribution worth {round(tip.amount, 5)} {warning} {tip.tokenName} {value_in_usd} has "
...
def maybe_market_kudos_to_github(kt):
    ...
    """
    if (
        not kt.is_notification_eligible(var_to_check=settings.GITHUB_CLIENT_ID)
        or not kt.github_url
    ):
        return False
    ...
    _comments = (
        "\n\nThe sender had the following public comments: \n> " f"{kt.comments_public}"
        if kt.comments_public
        else ""
    )
    if kt.username:
        msg = (
            f"⚡️ A *{kt.kudos_token_cloned_from.humanized_name}* Kudos has been "
    ...
    image = (
        f"<a title='{kt.kudos_token_cloned_from.humanized_name}' href='{kt.kudos_token_cloned_from.url}'> "
...
def maybe_market_to_email(b, event_name):
    ...
    if event_name == "new_bounty" and not settings.DEBUG:
        ...
        try:
            accepted_fulfillment = b.fulfillments.filter(accepted=True).latest(
                "modified_on"
            )
        ...
        try:
            rejected_fulfillment = b.fulfillments.filter(accepted=False).latest(
                "modified_on"
            )
...
def maybe_post_on_craigslist(bounty):
    ...
    form.find("input", {"id": "remuneration"})[
        "value"
    ] = f"{bounty.get_natural_value()} {bounty.token_name}"
    try:
    ...
    emails = fetch_mails_since_id(
        settings.IMAP_EMAIL, settings.IMAP_PASSWORD, last_email_id
    )
    for _, content in emails.items():
        if "craigslist" in content["from"]:
            for link in re.findall(
                r"(?:https?:\/\/[a-zA-Z0-9%]+[.]+craigslist+[.]+org/[a-zA-Z0-9\/\-]*)",
                content.as_string(),
            ):
...
def maybe_notify_bounty_user_escalated_to_slack(
    bounty, username, last_heard_from_user_days
):
    if not bounty.is_notification_eligible(var_to_check=settings.SLACK_TOKEN):
        return False
...
def maybe_notify_user_escalated_github(
    bounty, username, last_heard_from_user_days=None
):
    if not bounty.is_notification_eligible(var_to_check=settings.GITHUB_CLIENT_ID):
        return False
...
{append_snooze_copy(bounty)}"""
    ...
    post_issue_comment(
        bounty.org_name, bounty.github_repo_name, bounty.github_issue_number, msg
    )
...
def maybe_notify_bounty_user_warned_removed_to_slack(
    bounty, username, last_heard_from_user_days=None
):
    if not bounty.is_notification_eligible(var_to_check=settings.SLACK_TOKEN):
        return False
