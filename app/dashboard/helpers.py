...
def get_bounty_view_kwargs(request):
    ...
    standard_bounties_id = request.GET.get("sb_id") or request.GET.get(
...
def handle_bounty_views(request):
    ...
    try:
        ...
        bounty = (
            Bounty.objects.current().filter(**bounty_kwargs).distinct().latest("id")
        )
...
def issue_details(request):
    ...
    if hackathon_slug:
        sponsor_profiles = (
            HackathonEvent.objects.filter(slug__iexact=hackathon_slug)
            .prefetch_related("sponsor_profiles")
            .values_list("sponsor_profiles__handle", flat=True)
        )
...
def bounty_did_change(bounty_id, new_bounty_details):
    ...
    try:
        ...
        old_bounties = (
            Bounty.objects.filter(standard_bounties_id=bounty_id, network=network)
            .nocache()
            .order_by("-created_on")
        )
        ...
        did_change = True
        print(
            f"asserting did change because got the following exception: {e}. args;"
    ...
    for fulfillment in fulfillments:
        ...
        try:
            ...
            old_bounty_fulfillments = BountyFulfillment.objects.filter(
                fulfillment_id=fulfillment_id, bounty=old_bounty
            ).nocache()
            ...
            if github_username:
                ...
                try:
                    kwargs["profile_id"] = Profile.objects.get(
                        handle=github_username.lower()
                    ).pk
                except Profile.MultipleObjectsReturned:
                    kwargs["profile_id"] = (
                        Profile.objects.filter(handle=github_username.lower())
                        .first()
                        .pk
                    )
...
def create_new_bounty(old_bounties, bounty_payload, bounty_details, bounty_id):
    ...
    with transaction.atomic():
        ...
        bounty_kwargs = {
            "is_open": True
            if (bounty_details.get("bountyStage") == 1 and not accepted)
            else False,
        ...
        if not latest_old_bounty:
            ...
            bounty_kwargs.update(
                {
                    ...
                    "payout_tx_id": "",
                    "admin_override_suspend_auto_approval": not schemes.get(
            ...
            if latest_old_bounty_dict.get("bounty_owner_profile"):
                latest_old_bounty_dict["bounty_owner_profile"] = Profile.objects.get(
...
def merge_bounty(latest_old_bounty, new_bounty, metadata, bounty_details, verbose=True):
    ...
    if bounty_invitees and not latest_old_bounty:
        ...
        inviter = Profile.objects.get(
            handle=new_bounty.bounty_owner_github_username.lower()
        )
        invite_url = get_bounty_invite_url(inviter, new_bounty.id)
        msg = (
            "Check out this bounty that pays out "
            + str(new_bounty.get_value_true)
            + new_bounty.token_name
            + invite_url
        )
    ...
    if bounty_reserved_for_user and release_to_public_after:
        ...
        if release_to_public_after == "3-days":
            new_bounty.reserved_for_user_expiration = (
                new_bounty.reserved_for_user_from + timezone.timedelta(days=3)
            )
        elif release_to_public_after == "1-week":
            new_bounty.reserved_for_user_expiration = (
                new_bounty.reserved_for_user_from + timezone.timedelta(weeks=1)
            )
        ...
        if new_bounty.bounty_reserved_for_user and new_bounty.status == "reserved":
            ...
            new_reserved_issue(
                "founders@gitcoin.co", new_bounty.bounty_reserved_for_user, new_bounty
            )

    # set cancel date of this bounty
    canceled_on = (
        latest_old_bounty.canceled_on
        if latest_old_bounty and latest_old_bounty.canceled_on
        else None
    )
    ...
    if fulfillments:
        ...
        for inactive in (
            Bounty.objects.filter(current_bounty=False, github_url=url)
            .nocache()
            .order_by("-created_on")
        ):
    ...
    new_bounty.is_featured = (
        True if latest_old_bounty and latest_old_bounty.is_featured is True else False
    )
...
def process_bounty_details(bounty_details):
    ...
    try:
        with semaphore:
            new_bounty = create_new_bounty(
                old_bounties, bounty_payload, bounty_details, bounty_id
            )
...
def record_bounty_activity(
    event_name, old_bounty, new_bounty, _fulfillment=None, override_created=None
):
        ...
        if event_name not in funder_actions:
            if not fulfillment:
                ...
                if event_name == "work_done":
                    fulfillment = new_bounty.fulfillments.filter(accepted=True).latest(
                        "fulfillment_id"
                    )
    ...
    if user_profile:
        ...
        return Activity.objects.create(
            ...
            bounty=new_bounty,
            metadata={
                "new_bounty": get_bounty_data_for_activity(new_bounty)
                if new_bounty
                else None,
                "old_bounty": get_bounty_data_for_activity(old_bounty)
                if old_bounty
                else None,
                "fulfillment": get_fulfillment_data_for_activity(fulfillment)
                if fulfillment
                else None,
            },
        )
...
def is_lowball_bounty(bounty_value_usdt):
    ...
    """
    return (
        bounty_value_usdt < settings.LOWBALL_BOUNTY_THRESHOLD
        if bounty_value_usdt
        else False
    )
...
def process_bounty_changes(old_bounty, new_bounty):
    ...
    is_blocked = any(
        [
            (ele.lower() in new_bounty.github_url.lower())
            for ele in BlockedURLFilter.objects.values_list("expression", flat=True)
        ]
    )
    if is_blocked:
    ...
    # get json diff
    json_diff = (
        diff(old_bounty.raw_data, new_bounty.raw_data)
        if (old_bounty and new_bounty)
        else None
    )
    ...
    if (
        not old_bounty
        or (not old_bounty and new_bounty and new_bounty.is_open)
        or (not old_bounty.is_open and new_bounty and new_bounty.is_open)
    ):
    ...
    try:
        if not old_bounty or old_bounty.value_in_usdt != new_bounty.value_in_usdt:
            if (
                is_lowball_bounty(new_bounty.value_in_usdt)
    ...
    if event_name != "unknown_event":
        ...
        did_post_to_github = maybe_market_to_github(
            new_bounty, event_name, profile_pairs
        )
