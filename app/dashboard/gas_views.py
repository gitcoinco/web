...
def gas(request):
    ...
    context = {
        ...
        "card_desc": _(
            "Real-time graph of Ethereum (ETH) Live Predicted Confirmation Times (x axis) vs Gas Usage"
        ),
...
def gas_heatmap(request):
    ...
    gas_histories[mins] = (
        JSONStore.objects.filter(view="gas_history", key=key)
        .order_by("-created_on")
        .first()
        .data
    )
...
def gas_faq(request):
    context = {
        ...
        "card_desc": _(
            "Frequently asked questions and answers about Ethereum (ETH) Gas"
        ),
        "hide_send_tip": True,
    }
...
def gas_faucet_list(request):
    ...
    context = {
        ...
        "card_desc": _(
            "Ethereum (ETH) Gas Faucet List including the Mainnet, Rinkeby Testnet and Ropsten Testnet"
        ),
        "hide_send_tip": True,
    }
...
def gas_history_view(request):
    ...
    context = {
        ...
        "card_desc": _(
            "See and comment on the Ethereum (ETH) Gas - Hourly History Graph"
        ),
        "max": max_y,
        "lines": lines,
        "gas_histories": gas_histories,
