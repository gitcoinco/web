...
class BountySerializer(serializers.HyperlinkedModelSerializer):
    ...
    bounty_owner_email = serializers.SerializerMethodField(
        "override_bounty_owner_email"
    )
    bounty_owner_name = serializers.SerializerMethodField("override_bounty_owner_name")

    def override_bounty_owner_email(self, obj):
        can_make_visible_via_api = bool(
            int(obj.privacy_preferences.get("show_email_publicly", 0))
        )
        default = "Anonymous"
    ...
    def override_bounty_owner_name(self, obj):
        can_make_visible_via_api = bool(
            int(obj.privacy_preferences.get("show_name_publicly", 0))
        )
        default = "Anonymous"
    ...
    class Meta:
        ...
        fields = (
            ...
            "web3_type",
            "can_submit_after_expiration_date",
            "github_issue_number",
            "github_org_name",
            "github_repo_name",
            "idx_status",
    ...
    def create(self, validated_data):
        ...
        for fulfillment_data in fulfillments_data:
            bounty_fulfillment = BountyFulfillment.objects.create(
                bounty=bounty, **fulfillment_data
            )
            bounty_invite = BountyInvites.objects.filter(
                bounty=bounty, invitee=bounty_fulfillment.profile.user
            ).first()
            if bounty_invite:
...
class HackathonProjectsViewSet(viewsets.ModelViewSet):
    queryset = (
        HackathonProject.objects.prefetch_related("bounty", "profiles")
        .all()
        .order_by("id")
    )
    ...
    def get_queryset(self):
        ...
        if hackathon_id:
            ...
            queryset = (
                ...
                .prefetch_related("profiles", "bounty")
                .order_by("-winner", order_by, "id")
            )

            if sponsor:
                queryset = queryset.filter(
                    Q(bounty__github_url__icontains=sponsor)
                    | Q(bounty__bounty_owner_github_username=sponsor)
                )
        elif sponsor:
            queryset = (
                HackathonProject.objects.filter(
                    Q(hackathon__sponsor_profiles__handle__iexact=sponsor)
                    | Q(bounty__bounty_owner_github_username=sponsor)
                )
                .exclude(status="invalid")
                .prefetch_related("profiles", "bounty")
        ...
        if skills:
            queryset = queryset.filter(Q(profiles__keywords__icontains=skills))

        if rating:
            queryset = queryset.filter(Q(rating__gte=rating))

        if "winners" in filters:
            queryset = queryset.filter(Q(winner=True))
        if "lfm" in filters:
            queryset = queryset.filter(Q(looking_members=True))
        if "submitted" in filters:
            queryset = queryset.filter(Q(bounty__bounty_state="work_submitted"))
    ...
    def get_queryset(self):
        ...
        if event_tag:
            if event_tag == "all":
                ...
                try:
                    evt = HackathonEvent.objects.filter(slug__iexact=event_tag).latest(
                        "id"
                    )
        ...
        if "reserved_for_user_handle" in param_keys:
            ...
            if handle:
                try:
                    ...
                    logger.warning(
                        f"reserved_for_user_handle: Unknown handle: ${handle}"
                    )
        ...
        if applicants == "0":
            queryset = queryset.annotate(interested_count=Count("interested")).filter(
                interested_count=0
            )
        elif applicants == "1-5":
            queryset = (
                queryset.annotate(interested_count=Count("interested"))
                .filter(interested_count__gte=1)
                .filter(interested_count__lte=5)
            )

        # filter by who is interested
        if "started" in param_keys:
            queryset = queryset.filter(
                interested__profile__handle__in=[
        ...
        if "fulfiller_github_username" in param_keys:
            queryset = queryset.filter(
                fulfillments__profile__handle__iexact=self.request.query_params.get(
                    "fulfiller_github_username"
                )
            )
        ...
        if "interested_github_username" in param_keys:
            queryset = queryset.filter(
                interested__profile__handle=self.request.query_params.get(
        ...
        if "misc" in param_keys:
            if self.request.query_params.get("misc") == "hiring":
                queryset = queryset.exclude(
                    attached_job_description__isnull=True
                ).exclude(attached_job_description="")
        ...
        if "keywords" in param_keys:
            for index, keyword in enumerate(
                self.request.query_params.get("keywords").split(",")
            ):
                if index == 0:
        ...
        if order_by and order_by != "null":
            if order_by == "recently_marketed":
                queryset = queryset.order_by(
                    F("last_remarketed").desc(nulls_last=True), "-web3_created"
                )
            else:
        ...
        if len(pks):
            ...
            increment_view_count.delay(
                pks, queryset[0].content_type, self.request.user.id, view_type
            )
...
class TribesTeamSerializer(serializers.ModelSerializer):
    ...
    def follow_count(self, instance):
        return (
            TribeMember.objects.filter(org=instance)
    ...
    def user_following(self, instance):
        ...
        if user_profile:
            return (
                len(
                    user_profile.tribe_members.filter(
                        org__handle=instance.handle.lower()
                    )
                )
                > 0
            )
