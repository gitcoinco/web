...
def oauth_connect(request, *args, **kwargs):
    active_user_profile = Profile.objects.filter(
...
def record_bounty_activity(bounty, user, event_name, interest=None, fulfillment=None):
    ...
    if event_name == "worker_applied":
        kwargs["metadata"]["approve_worker_url"] = bounty.approve_worker_url(
            user.profile
        )
    ...
    try:
        ...
        if (
            event_name in ["start_work", "worker_applied"]
            and interest
            and interest.issue_message
        ):
        ...
        logger.error(
            f"error in record_bounty_activity: {e} - {event_name} - {bounty} - {user}"
        )
...
def create_new_interest_helper(bounty, user, issue_message):
    ...
    record_bounty_activity(
        bounty,
        user,
        "start_work" if not approval_required else "worker_applied",
        interest=interest,
    )
    bounty.interested.add(interest)
    record_user_action(user, "start_work", interest)
    maybe_market_to_slack(
        bounty, "start_work" if not approval_required else "worker_applied"
    )
    ...
    if bounty.event and request.user.is_authenticated:
        is_registered = (
            HackathonRegistration.objects.filter(
                registrant=request.user.profile, hackathon_id=bounty.event.id
            )
            or None
        )
    else:
...
def new_interest(request, bounty_id):
    ...
    if access_token:
        ...
        github_user_data = get_github_user_data(access_token)
        profile = (
            Profile.objects.prefetch_related("bounty_set")
            .filter(handle=github_user_data["login"].lower())
            .first()
        )
    ...
    if currently_working_on >= max_num_issues:
        return JsonResponse(
            {
                "error": _(
                    f"You cannot work on more than {max_num_issues} issues at once"
                ),
                "success": False,
            },
            status=401,
    ...
    if profile.no_times_slashed_by_staff():
        return JsonResponse(
            {
                "error": _(
                    "Because a staff member has had to remove you from a bounty in the past, you are unable to start"
    ...
    try:
        ...
        bounty_ids = (
            bounty.interested.filter(profile_id=profile_id)
    ...
    if approval_required:
        msg = _(
            "You have applied to start work. If approved, you will be notified via email."
        )
        ...
        msg = _(
            "You have applied to start work, but the bounty is reserved for another user."
        )
        JsonResponse({"error": msg, "success": False}, status=401)
    return JsonResponse(
        {
            "success": True,
...
def post_comment(request):
    ...
    receiver_profile = Profile.objects.filter(
        handle=request.POST.get("review[receiver]").lower()
    ).first()
    fbAmount = FeedbackEntry.objects.filter(
        sender_profile=profile_id, receiver_profile=receiver_profile, bounty=bountyObj
    ).count()
    if fbAmount > 0:
        return JsonResponse(
            {
    ...
    feedback.save()
    return JsonResponse({"success": True, "msg": "Finished."})
...
def remove_interest(request, bounty_id):
    ...
    if access_token:
        ...
        profile = Profile.objects.filter(
            handle=github_user_data["login"].lower()
        ).first()
        profile_id = profile.pk
    ...
    try:
        ...
        interest_ids = (
            bounty.interested.filter(profile_id=profile_id, bounty=bounty)
            .values_list("id", flat=True)
            .order_by("-created")
        )
...
def cancel_reason(request):
    ...
    try:
        ...
        return JsonResponse({"errors": ["Bounty not found"]}, status=404)
...
def uninterested(request, bounty_id, profile_id):
    ...
    is_moderator = (
        request.user.profile.is_moderator if hasattr(request.user, "profile") else False
    )
    if not is_logged_in or (not is_funder and not is_staff and not is_moderator):
        return JsonResponse(
            {"error": "Only bounty funders are allowed to remove users!"}, status=401
        )
...
def onboard(request, flow=None):
    ...
    if (steps and "github" not in steps) or "github" not in onboard_steps:
        if (
            ...
            and not getattr(request.user, "profile", None)
        ):
            login_redirect = redirect("/login/github?next=" + request.get_full_path())
            return login_redirect

    if (
        request.POST.get("eth_address")
    ...
    params = {
        ...
        "3d_avatar_params": None
        if "avatar" not in steps
        else avatar3dids_helper(theme),
...
def users_fetch_filters(
    ...
    if len(bounties_completed) == 2:
        profile_list = profile_list.annotate(count=Count("fulfilled")).filter(
...
def set_project_winner(request):
    ...
    if not project_id:
        return JsonResponse({"message": "Invalid Project"})
    project = HackathonProject.objects.get(pk=project_id)

    if not request.user.is_authenticated and (
        request.user.is_staff
        or request.user.profile.handle == project.bounty.bounty_owner_github_username
    ):
...
def users_autocomplete(request):
    ...
    if q:
        ...
        sqs = SearchQuerySet().autocomplete(
            (SQ(first_name_auto=q) | SQ(last_name_auto=q) | SQ(handle_auto=q))
        )
...
def output_users_to_csv(request):
    ...
    export_search_to_csv.delay(
        body=request.body.decode("utf-8"), user_handle=request.user.profile.handle
    )

    return JsonResponse(
        {"message": "Your request is processing and will be delivered to your email"}
    )
...
def users_fetch(request):
    ...
    if q:
        profile_list = profile_list.filter(
            Q(handle__icontains=q) | Q(keywords__icontains=q)
        ).distinct()

    if tribe:

        profile_list = profile_list.filter(
            Q(follower__org__handle__in=[tribe])
            | Q(organizations_fk__handle__in=[tribe])
        ).distinct()
        ...
        if user_filter and user_filter != "all":
            if user_filter == "owners":
                ...
                profile_list = profile_list.exclude(
                    Q(organizations_fk__handle__in=[tribe])
                )
            elif user_filter == "hackers":
                profile_list = profile_list.filter(
                    Q(fulfilled__bounty__github_url__icontains=tribe)
                    | Q(
                        project_profiles__hackathon__sponsor_profiles__handle__in=[
                            tribe
                        ]
                    )
                    | Q(
                        hackathon_registration__hackathon__sponsor_profiles__handle__in=[
                            tribe
                        ]
                    )
                )
    ...
    if request.GET.get("type") == "explore_tribes":
        profile_list = Profile.objects.filter(is_org=True).order_by(
            "-follower_count", "id"
        )
        ...
        this_page = Profile.objects_full.filter(
            pk__in=[ele.pk for ele in this_page]
        ).order_by("-follower_count", "id")
        ...
        profile_list = (
            Profile.objects_full.filter(pk__in=[ele for ele in this_page])
            .order_by("-rank_coder", "id")
            .exclude(handle__iexact="gitcoinbot")
        )
    ...
    for user in this_page:
        ...
        is_following = (
            True if followers.filter(profile=current_profile).count() else False
        )
        profile_json = {
            k: getattr(user, k)
            for k in [
        ...
        if hackathon_id:
            registration = HackathonRegistration.objects.filter(
                hackathon_id=hackathon_id, registrant=user
            ).last()
            if registration:
        ...
        if user.is_org:
            ...
            profile_json["count_bounties_on_repo"] = profile_dict.get("as_dict").get(
                "count_bounties_on_repo"
            )
            sum_eth = profile_dict.get("as_dict").get("sum_eth_on_repos")
            profile_json["sum_eth_on_repos"] = round(
                sum_eth if sum_eth is not None else 0, 2
            )
            ...
            profile_json["job_status"] = (
                user.job_status_verbose if user.job_search_status else None
            )
            profile_json["previously_worked"] = False  # user.previous_worked_count > 0
            profile_json[
                "position_contributor"
            ] = user.get_contributor_leaderboard_index()
...
def bounty_mentor(request):
    ...
    if body["hackathon_id"]:
        ...
        if body["bounty_org"]:
            sponsor = hackathon_event.sponsor_profiles.get(
                handle__iexact=body["bounty_org"]
            )
        else:
            sponsor = None
        ...
        bounty_org_default_mentors = Group.objects.get_or_create(
            name=f"sponsor-org-{sponsor.handle.lower()}-mentors"
        )[0]
        message = f"Mentors Updated Successfully"
        if body["set_default_mentors"]:
            current_mentors = Profile.objects.filter(
                user__groups=bounty_org_default_mentors
            )
            mentors_to_remove = list(
                set([current.id for current in current_mentors])
                - set(body["new_default_mentors"])
            )
...
def get_user_bounties(request):
    ...
    all_bounties = Bounty.objects.current().filter(
        bounty_owner_github_username__iexact=profile.handle, network=network
    )
    ...
    open_bounties = all_bounties.exclude(idx_status="cancelled").exclude(
        idx_status="done"
    )
...
def dashboard(request):
    ...
    title = (
        keyword.title() + str(_(" Bounties ")) if keyword else str(_("Issue Explorer"))
    )
    params = {
        "active": "dashboard",
        "title": title,
        "avatar_url": request.build_absolute_uri(
            static("v2/images/twitter_cards/tw_cards-01 copy.png")
        ),
...
def invoice(request):
    ...
    params["tips"] = [
        tip
        for tip in bounty.tips.send_happy_path()
        if (
            (tip.username == request.user.username and tip.username)
            or (tip.from_username == request.user.username and tip.from_username)
    ...
    if bounty.fee_amount > 0:
        params["fee_value_in_usdt"] = (
            bounty.fee_amount * Decimal(bounty.get_value_in_usdt) / bounty.value_true
        )
...
def social_contribution_modal(request):
    ...
    params[
        "invite_url"
    ] = f"{settings.BASE_URL}issue/{get_bounty_invite_url(request.user.username, bounty.pk)}"
    promo_text = (
        str(_("Check out this bounty that pays out "))
        + f"{bounty.get_value_true} {bounty.token_name} {params['invite_url']}"
    )
...
def bulk_invite(request):
    ...
    bounties_completed = (
        request.POST.get("params[bounties_completed]", "").strip().split(",")
    )
    leaderboard_rank = (
        request.POST.get("params[leaderboard_rank]", "").strip().split(",")
    )
    ...
    profiles = users_fetch_filters(
        profiles, skills, bounties_completed, leaderboard_rank, rating, organisation
    )
    ...
    return JsonResponse({"status": 200, "msg": "email_sent"})
...
def fulfill_bounty(request):
    ...
    if bounty.event:
        params["prize_projects"] = HackathonProject.objects.filter(
            profiles=request.user.profile, hackathon=bounty.event, bounty=bounty
        )
...
def increase_bounty(request):
    ...
    params["FEE_PERCENTAGE"] = (
        request.user.profile.fee_percentage if request.user.is_authenticated else 10
    )
...
def helper_handle_admin_override_and_hide(request, bounty):
    ...
    if admin_override_and_hide:
        is_moderator = (
            request.user.profile.is_moderator
...
def helper_handle_admin_contact_funder(request, bounty):
    ...
    if admin_contact_funder_txt:
        ...
        if is_staff or is_moderator:
            ...
            messages.warning(
                request, _("Only moderators or the funder of this bounty may do this.")
            )


def helper_handle_mark_as_remarket_ready(request, bounty):
    admin_mark_as_remarket_ready = request.GET.get(
        "admin_toggle_as_remarket_ready", False
    )
    if admin_mark_as_remarket_ready:
        ...
        if is_staff or is_moderator:
            bounty.admin_mark_as_remarket_ready = (
                not bounty.admin_mark_as_remarket_ready
            )
            bounty.save()
        ...
        if is_staff:
            ...
            if admin_override_satatus not in valid_statuses:
                messages.warning(
                    ...
                    str(
                        _(
                            "Not a valid status choice.  Please choose a valid status (no quotes): "
                        )
                    )
                    + valid_statuses_str,
                )
                ...
                bounty.save()
                messages.success(
                    request, _(f'Status updated to "{admin_override_satatus}" ')
                )
        else:
            messages.warning(
                request, _("Only staff or the funder of this bounty may do this.")
            )
...
def helper_handle_snooze(request, bounty):
    ...
    if snooze_days:
        ...
        if is_funder or is_staff or is_moderator:
            ...
            bounty.save()
            messages.success(
                request, _(f"Warning messages have been snoozed for {snooze_days} days")
            )
        else:
            messages.warning(
                request, _("Only moderators or the funder of this bounty may do this.")
...
def helper_handle_approvals(request, bounty):
    ...
    mutate_worker_action_past_tense = (
        "approved" if mutate_worker_action == "approve" else "rejected"
    )
    ...
    if mutate_worker_action:
        if not request.user.is_authenticated:
            messages.warning(
                request,
                _(
                    "You must be logged in to approve or reject worker submissions. Please login and try again."
                ),
            )
            return
        ...
        if not worker:
            messages.warning(
                request,
                _(
                    "You must provide the worker's username in order to approve or reject them."
                ),
            )
            return
        ...
        if is_funder or is_staff:
            pending_interests = bounty.interested.select_related("profile").filter(
                profile__handle=worker.lower(), pending=True
            )
            ...
            if not pending_interests.exists():
                messages.warning(
                    request,
                    _(
                        "This worker does not exist or is not in a pending state. Perhaps they were already approved or rejected? Please check your link and try again."
                    ),
                )
                return
            ...
            if mutate_worker_action == "approve":
                ...
                record_bounty_activity(
                    bounty, request.user, "worker_approved", interest
                )
                ...
                record_bounty_activity(
                    bounty, request.user, "worker_rejected", interest
                )
                bounty.interested.remove(interest)
                interest.delete()
            ...
            messages.success(
                request, _(f"{worker} has been {mutate_worker_action_past_tense}")
            )
        else:
            messages.warning(
                request, _("Only the funder of this bounty may perform this action.")
            )
...
def helper_handle_release_bounty_to_public(request, bounty):
    ...
    if release_to_public:
        ...
        if is_bounty_status_reserved:
            is_staff = request.user.is_staff
            is_bounty_reserved_for_user = (
                bounty.reserved_for_user_handle == request.user.username.lower()
            )
            if is_staff or is_bounty_reserved_for_user:
                ...
                if success:
                    messages.success(
                        request,
                        _("You have successfully released this bounty to the public"),
                    )
                else:
                    messages.warning(
                        request,
                        _(
                            "An error has occurred whilst trying to release. Please try again later"
                        ),
                    )
            else:
                messages.warning(
                    request,
                    _(
                        "Only staff or the user that has been reserved can release this bounty"
                    ),
                )
        else:
            messages.warning(
                request, _("This functionality is only for reserved bounties")
            )
...
def bounty_invite_url(request, invitecode):
    ...
    try:
        ...
        bounty_invite = BountyInvites.objects.filter(
            bounty=bounty, inviter=inviter, invitee=request.user
        ).first()
        if bounty_invite:
            bounty_invite.status = "accepted"
            bounty_invite.save()
        else:
            bounty_invite = BountyInvites.objects.create(status="accepted")
            bounty_invite.bounty.add(bounty)
            bounty_invite.inviter.add(inviter)
...
def bounty_details(request, ghuser="", ghrepo="", ghissue=0, stdbounties_id=None):
    ...
    try:
        if ghissue:
            issue_url = (
                "https://github.com/" + ghuser + "/" + ghrepo + "/issues/" + ghissue
            )
            bounties = Bounty.objects.current().filter(github_url=issue_url)
            if not bounties.exists():
                issue_url = (
                    "https://github.com/" + ghuser + "/" + ghrepo + "/pull/" + ghissue
                )
                bounties = Bounty.objects.current().filter(github_url=issue_url)
        else:
    ...
    if issue_url:
        try:
            ...
            if bounties:
                ...
                if (
                    bounties.count() > 1
                    and bounties.filter(network="mainnet").count() > 1
                ):
                ...
                if bounty.title and bounty.org_name:
                    params[
                        "card_title"
                    ] = f"{bounty.title} | {bounty.org_name} Funded Issue Detail | Gitcoin"
                    params["title"] = clean(params["card_title"], strip=True)
                    params["card_desc"] = ellipses(
                        clean(bounty.issue_description_text, strip=True), 255
                    )
                    params["noscript"] = {
                        "title": clean(bounty.title, strip=True),
                        "org_name": bounty.org_name,
                        "issue_description_text": clean(
                            bounty.issue_description_text, strip=True
                        ),
                ...
                params["stdbounties_id"] = (
                    bounty.standard_bounties_id
                    if not stdbounties_id
                    else stdbounties_id
                )
                params["interested_profiles"] = bounty.interested.select_related(
                    "profile"
                ).all()
                ...
                if bounty.event:
                    ...
                    params["prize_projects"] = (
                        HackathonProject.objects.filter(
                            hackathon=bounty.event,
                            bounty__standard_bounties_id=bounty.standard_bounties_id,
                        )
...
def funder_payout_reminder_modal(request, bounty_network, stdbounties_id):
    bounty = (
        Bounty.objects.current()
        .filter(network=bounty_network, standard_bounties_id=stdbounties_id)
        .first()
    )
...
def funder_payout_reminder(request, bounty_network, stdbounties_id):
    ...
    has_fulfilled = bounty.fulfillments.filter(
        profile__handle=github_user_data["login"]
    ).count()
    if has_fulfilled == 0:
        return JsonResponse(
            {
...
def quickstart(request):
    ...
    activities = Activity.objects.filter(activity_type="new_bounty").order_by(
        "-created"
    )[:5]
    context = deepcopy(qs.quickstart)
    context["activities"] = activities
    context["avatar_url"] = request.build_absolute_uri(
...
def load_banners(request):
    ...
    response = {"status": 200, "banners": images}
...
def profile_details(request, handle):
    ...
    bounties = (
        Bounty.objects.current()
        .prefetch_related(
            "fulfillments", "interested", "interested__profile", "feedbacks"
        )
        .filter(
    ...
    if bounties:
        for bounty in bounties:
            ...
            _bounty = {
                ...
                "rating": [
                    feedback.rating
                    for feedback in bounty.feedbacks.all().distinct("bounty_id")
                ],
            }
...
def user_card(request, handle):
    ...
    if request.user.is_authenticated:
        is_following = (
            True
            if TribeMember.objects.filter(
                profile=request.user.profile, org=profile
            ).count()
            else False
        )
    else:
...
def profile_grants(request, handle):
    ...
    contributions = Contribution.objects.filter(
        subscription__contributor_profile=profile
    ).order_by("-pk")
    history = []
...
def profile_activity(request, handle):
    ...
    activities = list(
        profile.get_various_activities().values_list("created_on", flat=True)
    )
    ...
    for i in range(1, 12 * 30):
        ...
        count = len(
            [
                ...
                if (activity_date < prev_date and activity_date > date)
            ]
        )
        if count:
...
def profile_earnings(request, handle, direction="to"):
    ...
    response = """date,close"""
    earnings = list(
        earnings.order_by("created_on").values_list("created_on", "value_usd")
    )
    balances = {}
...
def profile_viewers(request, handle):
    ...
    response = """date,close"""
    items = list(
        profile.viewed_by.order_by("created_on").values_list("created_on", flat=True)
    )
    balances = {}
    for item in items:
...
def profile_job_opportunity(request, handle):
    ...
    try:
        ...
        profile.resume = (
            request.FILES.get("job_cv", profile.resume) if not error_response else None
        )
        profile.save()
    except (ProfileNotFoundException, ProfileHiddenException):
        raise Http404

    response = {"status": 200, "message": "Job search status saved"}
    return JsonResponse(response)
...
def profile_backup(request):
    ...
    keys = [
        ...
        "activities",
        "tips",
        "_tips.private_fields",
        "feedbacks",
        "_feedbacks.private_items",
        "custom_avatars",
    ] + list(profile_data.keys())

    if model == "custom avatar":  # custom avatar
        custom_avatars = get_custom_avatars(profile)
        data["custom_avatars"] = CustomAvatarExportSerializer(
            custom_avatars, many=True
        ).data
        ...
        for interest in interests:
            active_work = active_work | Bounty.objects.filter(
                interested=interest, current_bounty=True
            )
        ...
        data["activities"] = ActivityExportSerializer(
            profile.activities, many=True
        ).data
    elif model == "tips":  # tips
        data["tips"] = filtered_list_data(
            "tip", profile.tips, private_items=None, private_fields=False
        )
        data["_tips.private_fields"] = filtered_list_data(
            "tip", profile.tips, private_items=None, private_fields=True
        )
    elif model == "feedback":  # feedback
        feedbacks = FeedbackEntry.objects.filter(receiver_profile=profile).all()
        data["feedbacks"] = filtered_list_data(
            "feedback", feedbacks, private_items=False, private_fields=None
        )
        data["_feedbacks.private_items"] = filtered_list_data(
            "feedback", feedbacks, private_items=True, private_fields=None
        )
    else:
        data = profile_data

    response = {"status": 200, "data": data, "keys": keys}
...
def profile_tax_settings(request, handle):
    ...
    response = {"status": 200, "message": "Tax settings saved"}
    return JsonResponse(response)
...
def invalid_file_response(uploaded_file, supported):
    ...
    if not uploaded_file:
        response = {"status": 400, "message": "No File Found"}
    elif uploaded_file.size > 31457280:
        # 30MB max file size
        response = {"status": 413, "message": "File Too Large"}
    else:
        ...
        if file_mime not in supported:
            response = {"status": 415, "message": "Invalid File Type"}
        """
        try:
            forbidden = False
...
def get_profile_tab(request, profile, tab, prev_context):
    ...
    context["portfolio_count"] = (
        len(context["portfolio"]) + profile.portfolio_items.cache().count()
    )
    context["projects_count"] = (
        HackathonProject.objects.filter(profiles__id=profile.id).cache().count()
    )
    context["my_kudos"] = profile.get_my_kudos.cache().distinct(
        "kudos_token_cloned_from__name"
    )[0:7]
    # specific tabs
    if tab == "activity":
        all_activities = [
            ...
            "update_grant",
            "killed_grant",
            "new_grant_contribution",
            "new_grant_subscription",
        ...
        activity_tabs = [
            ...
            (
                _("Bounties"),
                ["new_bounty", "start_work", "work_submitted", "work_done"],
            ),
        ...
        if page:
            ...
            if activity_type == "currently_working":
                ...
                return TemplateResponse(
                    request, "profiles/profile_bounties.html", context, status=status
                )
                ...
                paginator = Paginator(
                    profile_filter_activities(
                        all_activities, activity_type, activity_tabs
                    ),
                    10,
                )
                ...
                context = {}
                context["activities"] = [
                    ele.view_props_for(request.user) for ele in paginator.get_page(page)
                ]

                return TemplateResponse(
                    request, "profiles/profile_activities.html", context, status=status
                )
        ...
        if request.method == "POST" and request.is_ajax():
            ...
            validated = (
                ...
                and request.user.username.lower() == profile.handle.lower()
            )
        ...
        if title:
            if request.POST.get("URL")[0:4] != "http":
                ...
                not request.user.is_authenticated
                or request.user.profile.pk != profile.pk
            ):
        ...
        context["earnings"] = Earning.objects.filter(
            to_profile=profile, network="mainnet", value_usd__isnull=False
        ).order_by("-created_on")
    elif tab == "spent":
        context["spent"] = Earning.objects.filter(
        ...
        context["feedbacks_sent"] = [
            fb for fb in profile.feedbacks_sent.all() if fb.visible_to(request.user)
        ]
        context["feedbacks_got"] = [
            fb for fb in profile.feedbacks_got.all() if fb.visible_to(request.user)
        ]
        ...
        context["unrated_contributed_bounties"] = (
            ...
            .filter(interested__status="okay")
            .filter(interested__pending=False)
            .filter(idx_status="submitted")
            .exclude(
                feedbacks__feedbackType="worker", feedbacks__sender_profile=profile
            )
            .distinct("pk")
            .nocache()
        )
        ...
        try:
            context["upcoming_calls"] = JSONStore.objects.get(
                key="brightid_verification_parties",
                view="brightid_verification_parties",
            ).data
...
def verify_user_twitter(request, handle):
    ...
    is_logged_in_user = (
        request.user.is_authenticated
        and request.user.username.lower() == handle.lower()
    )
    if not is_logged_in_user:
        return JsonResponse(
            {"ok": False, "msg": f"Request must be for the logged in user"}
        )

    profile = profile_helper(handle, True)
    if profile.is_twitter_verified:
        return JsonResponse({"ok": True, "msg": f"User was verified previously"})
    ...
    if twitter_handle == "":
        return JsonResponse(
            {"ok": False, "msg": f"Request must include a Twitter handle"}
        )
    ...
    try:
        ...
        if user.followers_count < MIN_FOLLOWER_COUNT:
            msg = (
                "Sorry, you must have at least "
                + str(MIN_FOLLOWER_COUNT)
                + " followers"
            )
        ...
        if age < timedelta(days=7 * MIN_ACCOUNT_AGE_WEEKS):
            msg = (
                "Sorry, your account must be at least "
                + str(MIN_ACCOUNT_AGE_WEEKS)
                + " weeks old"
            )
            return JsonResponse({"ok": False, "msg": msg})
    ...
    return JsonResponse(
        {"ok": True, "msg": full_text, "found": tweet_split, "expected": expected_split}
    )
...
def profile(request, handle, tab=None):
    ...
    try:
        ...
        if not handle:
            ...
            profile = None
            if not profile:
                profile = profile_helper(
                    handle.lower(), disable_cache=disable_cache, full_profile=True
                )
        else:
            if handle.endswith("/"):
                handle = handle[:-1]
            profile = profile_helper(
                handle.lower(), current_user=request.user, disable_cache=disable_cache
            )
        ...
        return TemplateResponse(
            request, "profiles/profile.html", context, status=status
        )
    ...
    if request.user.is_authenticated and hasattr(request.user, "profile"):
        context["is_on_tribe"] = request.user.profile.tribe_members.filter(
            org__handle=handle.lower()
        ).exists()
    else:
    ...
    if profile.is_org and profile.is_tribe:
        ...
        try:
            ...
            context["currentProfile"] = TribesSerializer(
            ...
            return TemplateResponse(
                request, "profiles/tribes-vue.html", context, status=status
            )
        except Exception as e:
    ...
    context["feedbacks_sent"] = [
        fb.pk for fb in profile.feedbacks_sent.all() if fb.visible_to(request.user)
    ]
    context["feedbacks_got"] = [
        fb.pk for fb in profile.feedbacks_got.all() if fb.visible_to(request.user)
    ]
    ...
    context["following"] = Paginator(
        context["all_following"], follow_page_size
    ).get_page(page_number)
    context["followers"] = Paginator(
        context["all_followers"], follow_page_size
    ).get_page(page_number)
    context["foltab"] = request.GET.get("sub", "followers")
    context["page_obj"] = (
        context["followers"]
        if context["foltab"] == "followers"
        else context["following"]
    )
...
def funders_mailing_list(request):
    profile_list = list(
        Profile.objects.filter(persona_is_funder=True)
        .exclude(email="")
...
def hunters_mailing_list(request):
    profile_list = list(
        Profile.objects.filter(persona_is_hunter=True)
        .exclude(email="")
        .values_list("email", flat=True)
    )
...
def sync_web3(request):
    ...
    """
    # setup
    result = {"status": "400", "msg": "bad request"}
    ...
    if issue_url and txid and network:
        ...
        if not has_tx_mined(txid, network):
            result = {"status": "400", "msg": "tx has not mined yet"}
        else:
            ...
            if not bounty_id:
                result = {"status": "400", "msg": "could not find bounty id"}
            else:
                # get/process bounty
                    ...
                    if new_bounty:
                        ...
                        try:
                            fund_ables = Activity.objects.filter(
                                ...
                                metadata__resource__contains={"provider": issue_url},
                            )
                            if fund_ables.exists():
                                ...
                                activity.save()
                                Comment.objects.create(
                                    profile=new_bounty.bounty_owner_profile,
                                    activity=activity,
                                    comment=comment,
                                )
            ...
            if hasattr(coin, "coinredemptionrequest"):
                ...
                tx = contract.functions.transfer(
                ...
                ).buildTransaction(
                    {
                        ...
                        "gas": 100000,
                        "gasPrice": recommend_min_gas_price_to_confirm_in_time(5)
                        * 10 ** 9,
                    }
                )
...
def new_bounty(request):
    ...
    if request.user.is_authenticated:
        ...
        suggested_developers = (
            BountyFulfillment.objects.prefetch_related("bounty")
            .filter(
                bounty__bounty_owner_github_username__iexact=request.user.profile.handle,
                bounty__idx_status="done",
            )
            .values("profile__handle", "profile__id")
            .annotate(fulfillment_count=Count("bounty"))
            .order_by("-fulfillment_count")[:5]
        )
    bounty_params = {
        "newsletter_headline": _("Be the first to know about new funded issues."),
        "issueURL": clean_bounty_url(
            request.GET.get("source") or request.GET.get("url", "")
        ),
    ...
    params["blocked_urls"] = json.dumps(
        list(BlockedURLFilter.objects.all().values_list("expression", flat=True))
    )
    params["FEE_PERCENTAGE"] = (
        request.user.profile.fee_percentage if request.user.is_authenticated else 10
    )
    ...
    params["avatar_url"] = request.build_absolute_uri(
        static("v2/images/twitter_cards/tw_cards-01.png")
    )
...
def new_hackathon_bounty(request, hackathon=""):
    ...
    try:
        hackathon_event = (
            HackathonEvent.objects.filter(slug__iexact=hackathon)
            .prefetch_related("sponsor_profiles")
            .latest("id")
        )
...
def get_suggested_contributors(request):
    ...
    if request.user.is_authenticated:
        previously_worked_developers = (
            BountyFulfillment.objects.prefetch_related("bounty", "profile")
            .filter(
                bounty__bounty_owner_github_username__iexact=request.user.profile.handle,
                bounty__idx_status="done",
            )
    ...
    recommended_developers = (
        ...
        .exclude(profile__handle__exact="")
        .distinct()[:10]
    )

    verified_developers = UserVerificationModel.objects.filter(verified=True).values(
        "user__profile__handle", "user__profile__id"
    )
    ...
    if invitees:
        ...
        users_invite = (
            Profile.objects.filter(invitees_filter)
...
def change_bounty(request, bounty_id):
    ...
    if request.body:
        ...
        if not can_change:
            return JsonResponse(
                {"error": _("The bounty can not be changed anymore.")}, status=405
            )
        ...
        if not is_funder and not is_staff:
            return JsonResponse(
                {"error": _("You are not authorized to change the bounty.")}, status=401
            )
            ...
            if value != 0:
                if key == "featuring_date":
                    value = timezone.make_aware(
                        timezone.datetime.fromtimestamp(int(value)), timezone=UTC
                    )
        ...
        if not bounty.is_bounties_network:
            ...
            if new_amount and current_amount != new_amount:
                ...
                try:
                    bounty.token_value_in_usdt = convert_token_to_usdt(
                        bounty.token_name
                    )
                    bounty.value_in_usdt = convert_amount(
                        bounty.value_true, bounty.token_name, "USDT"
                    )
                    bounty.value_in_usdt_now = bounty.value_in_usdt
                    bounty.value_in_eth = convert_amount(
                        bounty.value_true, bounty.token_name, "ETH"
                    )
        ...
        if new_reservation and bounty.bounty_reserved_for_user:
            new_reserved_issue(
                "founders@gitcoin.co", bounty.bounty_reserved_for_user, bounty
            )
    ...
    if request.is_ajax():
        ...
        for user in profiles:
            ...
            if user.avatar_baseavatar_related.filter(active=True).exists():
                profile_json["avatar_id"] = (
                    user.avatar_baseavatar_related.filter(active=True).first().pk
                )
                profile_json["avatar_url"] = (
                    user.avatar_baseavatar_related.filter(active=True)
                    .first()
                    .avatar_url
                )
        ...
        if not len(results) and add_non_gitcoin_users:
            ...
            for result in search_results:
                ...
                if profile_json["text"].lower() not in [
                    p["text"].lower() for p in profiles
                ]:
                    results.append(profile_json)
...
def get_kudos(request):
    ...
    if request.is_ajax():
        ...
        kudos_pks = (kudos_by_desc | kudos_by_name | kudos_by_tags).values_list(
            "pk", flat=True
        )
        kudos = Token.objects.filter(
            pk__in=kudos_pks, hidden=False, num_clones_allowed__gt=0
        ).order_by("name")
...
def hackathon_prizes(request, hackathon=""):
    ...
    if profile.is_staff:
        ...
        query_prizes = Bounty.objects.filter(
            event=hackathon_event, current_bounty=True
        ).filter(
            Q(bounty_owner_profile__in=profiles)
            | Q(admin_override_org_name__in=[sponsor.name, sponsor.handle])
        )
    elif funded_bounties.exists():
        query_prizes = funded_bounties.filter(current_bounty=True)
    else:
        raise Http404(
            "You need fund one prize in this hackathon to access the dashboard."
        )

    prizes = []
    for prize in query_prizes:
        paid = BountyFulfillment.objects.filter(bounty=prize, payout_status="done")
        total_submitted = BountyEvent.objects.filter(
            bounty=prize, event_type="submit_work"
        ).count()
        prize_in_json = {
            "pk": prize.pk,
            ...
            "paid": paid.exists(),
            "submissions": [
                project.to_json() for project in prize.project_bounty.all()
            ],
...
def dashboard_sponsors(request, hackathon="", panel="prizes"):
    ...
    if profile.is_staff:
        ...
        query_prizes = Bounty.objects.filter(
            event=hackathon_event, bounty_owner_profile=profile, current_bounty=True
        )
    else:
        raise Http404(
            "You need fund one prize in this hackathon to access the dashboard."
    ...
    avatar_url = (
        hackathon_event.logo.url
        if hackathon_event.logo
    ...
    hackathon_not_started = (
        timezone.now() < hackathon_event.start_date and not request.user.is_staff
    )
    org = {
    ...
    num_participants = BountyEvent.objects.filter(
        bounty__event_id=hackathon_event.id,
        event_type="express_interest",
        bounty__in=query_prizes,
    ).count()
    num_submissions = BountyEvent.objects.filter(
        bounty__event_id=hackathon_event.id,
        event_type="submit_work",
        bounty__in=query_prizes,
    ).count()
...
def hackathon(request, hackathon="", panel="prizes"):
    ...
    try:
        ...
        increment_view_count.delay(
            [hackathon_event.pk],
            hackathon_event.content_type,
            request.user.id,
            "individual",
        )
    except HackathonEvent.DoesNotExist:
    ...
    if request.user and hasattr(request.user, "profile"):
        profile = request.user.profile
        following_tribes = profile.tribe_members.filter(org__in=sponsors).values_list(
            "org__handle", flat=True
        )

        is_member = profile.organizations_fk.filter(
            pk__in=sponsors.values_list("id", flat=True)
        ).exists()
    ...
    if hasattr(request.user, "profile") == False:
        ...
        is_registered = (
            HackathonRegistration.objects.filter(
                registrant=request.user.profile, hackathon=hackathon_event
            )
    ...
    hacker_count = (
        HackathonRegistration.objects.filter(hackathon=hackathon_event)
        .distinct("registrant")
        .count()
    )
    projects_count = (
        HackathonProject.objects.filter(hackathon=hackathon_event).all().count()
    )
    view_tags = get_tags(request)
    active_tab = 0
    if panel == "prizes":
    ...
    params = {
        ...
        "hackathon_obj": HackathonEventSerializer(hackathon_event).data,
        "prize_founders": list(
            Bounty.objects.filter(event=hackathon_event)
            .values_list("bounty_owner_profile__handle", flat=True)
            .distinct()
        ),
    ...
    if hackathon_event.identifier == "grow-ethereum-2019":
        params[
            "card_desc"
        ] = "The ‘Grow Ethereum’ Hackathon runs from Jul 29, 2019 - Aug 15, 2019 and features over $10,000 in bounties"
    ...
    params[
        "chat_override_url"
    ] = f"{get_chat_url()}/hackathons/channels/{hackathon_event.chat_channel_id}"
...
def hackathon_onboard(request, hackathon=""):
    ...
    is_registered = False
    try:
        hackathon_event = HackathonEvent.objects.filter(slug__iexact=hackathon).latest(
            "id"
        )
        ...
        is_registered = (
            HackathonRegistration.objects.filter(
                registrant=profile, hackathon=hackathon_event
            )
            if profile
            else None
        )
        ...
        if hackathon_sponsors:
            ...
            for hackathon_sponsor in hackathon_sponsors:
                ...
                if sponsor.tribe:
                    sponsor_obj["members"] = TribeMember.objects.filter(
                        profile=sponsor.tribe
                    ).count()
    ...
    if Poll.objects.filter(hackathon=hackathon_event).exists():
        params["poll"] = (
            Poll.objects.filter(hackathon=hackathon_event).order_by("created_on").last()
        )
...
def hackathon_projects(request, hackathon="", specify_project=""):
    ...
    projects = (
        HackathonProject.objects.filter(hackathon=hackathon_event)
        .exclude(status="invalid")
        .prefetch_related("profiles")
        .order_by(order_by)
    ...
    if filters == "winners":
        projects = projects.filter(Q(badge__isnull=False))
    if specify_project:
...
def hackathon_get_project(request, bounty_id, project_id=None):
    ...
    try:
        ...
        projects = HackathonProject.objects.filter(
            bounty__standard_bounties_id=bounty.standard_bounties_id,
            profiles__id=profile.id,
        ).nocache()
...
def hackathon_save_project(request):
    ...
    error_response = invalid_file_response(
        logo, supported=["image/png", "image/jpeg", "image/jpg"]
    )
    ...
    if looking_members:
        ...
        has_other_contact_method = (
            request.POST.get("has_other_contact_method", "") == "on"
        )
    ...
    if project_id:
        try:
            ...
            project = HackathonProject.objects.filter(
                id=project_id, profiles__id=profile.id
            )

            kwargs.update({"logo": request.FILES.get("logo", project.first().logo)})
            ...
            return JsonResponse(
                {"error": _("Error trying to save project")}, status=401
            )
    else:
        ...
        project.profiles.add(
            *list(filter(lambda profile_id: profile_id > 0, map(int, profiles)))
        )
        invalidate_obj(project)

    return JsonResponse({"success": True, "msg": _("Project saved.")})
    ...
    params = {
        "project": {
            ...
            "url": reverse(
                ...
                args=[project.hackathon.slug, project_id, slugify(project.name)],
            ),
            "logo_url": project.logo.url
            if project.logo
            else staticfiles_storage.url(f"v2/images/projects/logos/${project.id}.png"),
            "prize": {
                "id": project.bounty.id,
                "title": project.bounty.title,
                "avatar": project.bounty.avatar_url,
                "org": project.bounty.org_display_name,
                "org_url": project.bounty.org_profile.url
                if project.bounty.org_profile
                else "",
...
def hackathon_project_page(request, hackathon, project_id, project_name, tab=""):
    ...
    params = {
        ...
        "is_member": project.profiles.filter(pk=profile.id).exists()
        if profile
        else False,
        "project_obj": {
            "name": project.name,
...
def hackathon_registration(request):
    ...
    try:
        ...
        if HackathonRegistration.objects.filter(
            hackathon=hackathon_event, registrant=profile
        ).exists():
        ...
        if poll:
            ...
            for entry in poll:
                ...
                if question.question_type == "SINGLE_OPTION":
                    answer, status = Answer.objects.get_or_create(
                        user=request.user, question=question, hackathon=hackathon_event
                    )
                    ...
                    if (
                        question.minimum_character_count
                        and not len(text) > question.minimum_character_count
                    ):
                        raise ValidationError(
                            f"{question.minimum_character_count} characters required"
                        )
                    answer, status = Answer.objects.get_or_create(
                        user=request.user,
                        open_response=text,
            ...
            for (question, choices) in set_questions.items():
                Answer.objects.filter(
                    user=request.user,
                    question__id=int(question),
                    hackathon=hackathon_event,
                ).exclude(choice__in=choices).delete()

    except Exception as e:
        logger.error("Error while saving registration", e)

    client = MailChimp(
        mc_api=settings.MAILCHIMP_API_KEY, mc_user=settings.MAILCHIMP_USER
    )
    mailchimp_data = {
        "email_address": email,
    ...
    try:
        client.lists.members.create_or_update(
            settings.MAILCHIMP_LIST_ID_HACKERS, user_email_hash, mailchimp_data
        )
    ...
    if referer and "/issue/" in referer and is_safe_url(referer, request.get_host()):
        messages.success(
            request,
            _(
                f"You have successfully registered to {hackathon_event.name}. Happy hacking!"
            ),
        )
        redirect = referer
    else:
...
def funder_dashboard_bounty_info(request, bounty_id):
    ...
    if bounty.status == "open":
        interests = (
            Interest.objects.prefetch_related("profile")
            .filter(status="okay", bounty=bounty)
            .all()
        )
        profiles = [
            {
        ...
        for f in fulfillments:
            profile = {
                "fulfiller_metadata": f.fulfiller_metadata,
    ...
    return JsonResponse({"id": bounty.id, "profiles": profiles})
...
def serialize_funder_dashboard_open_rows(bounties, interests):
    return [
        {
            "users_count": len(
                [i for i in interests if b.pk in [i_b.pk for i_b in i.bounties]]
            ),
            "title": b.title,
            "id": b.id,
...
def serialize_funder_dashboard_submitted_rows(bounties):
    return [
        {
            "users_count": b.fulfillments.count(),
            "title": b.title,
            "id": b.id,
            "token_name": b.token_name,
            "value_in_token": b.value_in_token,
    ...
    if bounty_type == "open":
        ...
        interests = list(
            Interest.objects.filter(
                bounty__pk__in=[b.pk for b in bounties], status="okay"
            )
        )
        return JsonResponse(
            clean_dupe(serialize_funder_dashboard_open_rows(bounties, interests)),
            safe=False,
        )
        ...
        bounties = (
            Bounty.objects.prefetch_related("fulfillments")
            .distinct("id")
            .filter(
        ...
        return JsonResponse(
            serialize_funder_dashboard_submitted_rows(bounties), safe=False
        )
...
def change_user_profile_banner(request):
    ...
    try:
        ...
        if filename[0:7] != "/static" or filename.split("/")[
            -1
        ] not in load_files_in_directory("wallpapers"):
            is_valid = False
        if not is_valid:
            return JsonResponse({"error": "Bad request"}, status=401)
    ...
    response = {"status": 200, "message": "User banner image has been updated."}
    return JsonResponse(response)
...
def choose_persona(request):
    ...
    if request.user.is_authenticated:
        profile = request.user.profile if hasattr(request.user, "profile") else None
        if not profile:
            return JsonResponse({"error": _("You must be authenticated")}, status=401)
...
def is_my_tribe_member(leader_profile, tribe_member):
    return any(
        [
            tribe_member.org.handle.lower() == org.lower()
...
def join_tribe(request, handle):
    if request.user.is_authenticated:
        ...
        try:
            try:
                TribeMember.objects.get(
                    profile=profile, org__handle=handle.lower()
                ).delete()
            except TribeMember.MultipleObjectsReturned:
                TribeMember.objects.filter(
                    profile=profile, org__handle=handle.lower()
                ).delete()
...
def tribe_leader(request):
    if request.user.is_authenticated:
        ...
        try:
            ...
            is_my_org = request.user.is_authenticated and any(
                [
                    tribemember.org.handle.lower() == org.lower()
                    for org in request.user.profile.organizations
                ]
            )
...
def save_tribe(request, handle):
    ...
    try:
        ...
        if request.FILES.get("cover_image"):
            ...
            error_response = invalid_file_response(
                cover_image, supported=["image/png", "image/jpeg", "image/jpg"]
            )
        ...
        if request.POST.get("tribe_priority"):
            ...
            if request.POST.get("publish_to_ts"):
                title = "updated their priority to " + request.POST.get(
                    "priority_html_text"
                )
                kwargs = {
                    "profile": tribe,
                    "activity_type": "status_update",
                    "metadata": {"title": title, "ask": "#announce"},
                }
                activity = Activity.objects.create(**kwargs)
        ...
        return JsonResponse(
            {"success": False, "is_leader": False, "message": "something went wrong"},
            status=500,
        )
...
def create_bounty_v1(request):
    ...
    """
    response = {"status": 400, "message": "error: Bad Request. Unable to create bounty"}
    ...
    bounty.bounty_owner_github_username = request.POST.get(
    ...
    bounty.admin_override_suspend_auto_approval = (
        False if request.POST.get("auto_approve_workers") == "true" else True
    )
    ...
    bounty.bounty_owner_name = request.POST.get(
        "bounty_owner_name", ""
    )  # ETC-TODO: REMOVE ?
    ...
    bounty.expires_date = timezone.make_aware(
        timezone.datetime.fromtimestamp(expires_date), timezone=UTC
    )
    ...
    if reserved_for_username:
        bounty.bounty_reserved_for_user = Profile.objects.get(
            handle=reserved_for_username.lower()
        )
        if bounty.bounty_reserved_for_user:
            ...
            if release_to_public_after == "3-days":
                bounty.reserved_for_user_expiration = (
                    bounty.reserved_for_user_from + timezone.timedelta(days=3)
                )
            elif release_to_public_after == "1-week":
                bounty.reserved_for_user_expiration = (
                    bounty.reserved_for_user_from + timezone.timedelta(weeks=1)
                )
    ...
    if activity_ref:
        try:
            ...
            activity.save()
            Comment.objects.create(
                profile=bounty.bounty_owner_profile, activity=activity, comment=comment
            )
...
def cancel_bounty_v1(request):
    ...
    """
    response = {"status": 400, "message": "error: Bad Request. Unable to cancel bounty"}
    ...
    if bounty.bounty_state in ["cancelled", "done"]:
        response["message"] = (
            "error: bounty in " + bounty.bounty_state + " state cannot be cancelled"
        )
        return JsonResponse(response)
...
def fulfill_bounty_v1(request):
    ...
    if bounty.bounty_state in ["cancelled", "done"]:
        response["message"] = (
            "error: bounty in " + bounty.bounty_state + " state cannot be fulfilled"
        )
        return JsonResponse(response)
    ...
    if payout_type == "fiat" and not fulfiller_identifier:
        ...
        payout_type == "qr" or payout_type == "polkadot-ext"
...
def payout_bounty_v1(request, fulfillment_id):
    ...
    """
    response = {"status": 400, "message": "error: Bad Request. Unable to payout bounty"}
    ...
    if bounty.bounty_state in ["cancelled", "done"]:
        response["message"] = (
            "error: bounty in " + bounty.bounty_state + " state cannot be paid out"
        )
        return JsonResponse(response)
    ...
    if payout_type not in ["fiat", "qr", "web3_modal", "polkadot_ext", "manual"]:
        response[
            "message"
        ] = "error: parameter payout_type must be fiat / qr / web_modal / polkadot_ext / manual"
        return JsonResponse(response)
    if payout_type == "manual" and not bounty.event:
        response[
            "message"
        ] = "error: payout_type manual is eligible only for hackathons"
        return JsonResponse(response)
    ...
    if payout_type == "fiat":
        ...
        if not payout_status:
            response[
                "message"
            ] = "error: missing parameter payout_status for fiat payment"
            return JsonResponse(response)
        ...
        if not funder_identifier:
            response[
                "message"
            ] = "error: missing parameter funder_identifier for fiat payment"
            return JsonResponse(response)
        ...
        if not funder_address:
            response[
                "message"
            ] = f"error: missing parameter funder_address for payout_type ${payout_type}"
            return JsonResponse(response)
    ...
    if payout_type in ["fiat", "manual"]:
        if not payout_tx_id:
            response[
                "message"
            ] = f"error: missing parameter payout_tx_id for payout_type ${payout_type}"
            return JsonResponse(response)
...
def close_bounty_v1(request, bounty_id):
    ...
    """
    response = {"status": 400, "message": "error: Bad Request. Unable to close bounty"}
    ...
    try:
        ...
        accepted_fulfillments = BountyFulfillment.objects.filter(
            bounty=bounty, accepted=True, payout_status="done"
        )
    except Bounty.DoesNotExist:
        response["message"] = "error: bounty not found"
        return JsonResponse(response)
    ...
    if bounty.bounty_state in ["cancelled", "done"]:
        response["message"] = (
            "error: bounty in " + bounty.bounty_state + " state cannot be closed"
        )
        return JsonResponse(response)
    ...
    bounty.is_open = (
        False  # TODO: fixup logic in status calculated property on bounty model
    )
    bounty.accepted = True
    bounty.save()

    response = {"status": 204, "message": "bounty is closed"}
...
def validate_number(user, twilio, phone, redis, delivery_method="sms"):
    ...
    hash_number = hashlib.pbkdf2_hmac(
        "sha256", phone.encode(), PHONE_SALT.encode(), 100000
    ).hex()

    if user.profile.sms_verification:
        return JsonResponse(
            {"success": False, "msg": "Your account has been validated previously."},
            status=401,
        )

    if (
        Profile.objects.filter(encoded_number=hash_number, sms_verification=True)
        .exclude(pk=user.profile.id)
        .exists()
    ):
        return JsonResponse(
            {
    ...
    validation = twilio.lookups.phone_numbers(phone).fetch(
        type=["caller-name", "carrier"]
    )
    country_code = (
        validation.carrier["mobile_country_code"]
        if validation.carrier["mobile_country_code"]
        else 0
    )
    pv = ProfileVerification.objects.create(
        profile=user.profile,
        caller_type=validation.caller_name["caller_type"]
        if validation.caller_name
        else "",
    ...
    if validation.caller_name and validation.caller_name["caller_type"] == "BUSINESS":
        ...
        pv.validation_comment = (
            "Only support consumer numbers, not business or cloud numbers."
        )
        pv.save()
        if validate_consumer_number:
            return JsonResponse(
                {"success": False, "msg": pv.validation_comment}, status=401
            )
...
def send_verification(request):
    ...
    has_previous_validation = (
        profile.last_validation_request
        and profile.last_validation_request.replace(tzinfo=pytz.utc)
    )
    ...
    if not has_previous_validation:
        ...
        if cooldown > datetime.now().replace(tzinfo=pytz.utc):
            return JsonResponse(
                {"success": False, "msg": "Wait a minute to try again."}, status=401
            )

        if validation_attempts > SMS_MAX_VERIFICATION_ATTEMPTS:
            return JsonResponse(
                {"success": False, "msg": "Attempt numbers exceeded."}, status=401
            )
...
def validate_verification(request):
    ...
    if has_previous_validation:
        ...
        if verification.status == "approved":
            ...
            return JsonResponse({"success": True, "msg": "Verification completed"})

        return JsonResponse(
            {"success": False, "msg": "Failed verification"}, status=401
        )

    return JsonResponse(
        {"success": False, "msg": "No verification process associated"}, status=401
    )
