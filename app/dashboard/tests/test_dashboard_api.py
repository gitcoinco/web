...
class ProfileFactory(factory.DjangoModelFactory):
    class Meta:
        model = Profile
    data = {},
    handle = factory.Faker('first_name'),
    email = factory.Faker('email')


class BountyFactory(factory.DjangoModelFactory):
    class Meta:
        model = Bounty
    title = factory.Sequence(lambda n: 'foo{0}'.format(n - 255))
    value_in_token = 3
    token_name = 'ETH'
...
class DashboardAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        BountyFactory.create_batch(255)

    def test_bounties_amounts(self):
