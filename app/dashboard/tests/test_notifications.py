...
class DashboardNotificationsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.usdt_value = (
            f"({round(self.bounty.value_in_usdt, 2)} USD)"
            if self.bounty.value_in_usdt
            else ""
        )
    ...
    def test_build_github_notification_new_bounty(self):
        ...
        assert (
            f"__This issue now has a funding of {self.natural_value} {self.bounty.token_name}"
            in message
        )
    ...
    def test_build_github_notification_killed_bounty(self):
        ...
        assert (
            f"__The funding of {self.natural_value} {self.bounty.token_name} {self.usdt_value}"
            in message
        )
    ...
    def test_build_github_notification_increased_bounty(self):
        ...
        assert (
            f"__The funding of this issue was increased to {self.natural_value} {self.bounty.token_name}"
            in message
        )
