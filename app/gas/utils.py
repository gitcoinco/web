    ...
    try:
        for minutes in [1, 11, 21, 31]:
            gp = (
                ...
                .distinct("gas_price")
                .order_by("gas_price")
                .values_list("gas_price", "mean_time_to_confirm_minutes")
            )
            if gp:
...
def gas_history(breakdown, mean_time_to_confirm_minutes):
    ...
    for gp in gas_profiles:
        ...
        if key not in results.keys():
            ...
            if (
                package["mean_time_to_confirm_minutes"]
                > key_package["mean_time_to_confirm_minutes"]
            ):
                results[key] = package
            elif (
                package["mean_time_to_confirm_minutes"]
...
def gas_advisories():
    gas_advisories = GasAdvisory.objects.filter(
        active=True, active_until__gt=timezone.now()
    )
