...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            if len(elements) < 10:
                logger.warning(
                    "In: sync_gas_prices - Malformed Response: (%s) - Status Code: {%s}",
                    response,
                    response.status_code,
                )
                print(
                    f"Malformed Response: ({response}) - Status Code: {response.status_code}"
                )
                return
            ...
            for element in elements:
                try:
                    ...
                    mean_time_to_confirm_minutes = str(
                        round(element["expectedTime"], 1)
                    )
                    _99confident_confirm_time_blocks = 0
                    _99confident_confirm_time_mins = str(
                        round(element["expectedTime"] * 2.5, 1)
                    )
                    ...
                    logger.warning(
                        "In: sync_gas_prices - Malformed response - Code: %s",
                        response.status_code,
                    )
