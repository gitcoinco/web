...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        max_gas = 50
        package = {}
        for gp in GasProfile.objects.filter(
            gas_price__lt=max_gas,
            ...
            new_pkg = [
                float(gp.gas_price),
                -1 * int(new_time.strftime("%s")),
                float(gp.mean_time_to_confirm_minutes),
            ]
        ...
        for j in range(num_items_to_show_at_a_time, len(arr)):
            ...
            fig = plt.figure(
                figsize=(16, 9), dpi=200, facecolor=facecolor, edgecolor="k"
            )
            ...
            for k in range(0, num_items_to_show_at_a_time):
                ...
                ax.plot_wireframe(
                    ...
                    cstride=10,
                    color=color,
                    alpha=1 - (k / num_items_to_show_at_a_time),
                )
