...
class Like(SuperModel):
    ...
    profile = models.ForeignKey(
        "dashboard.Profile", on_delete=models.CASCADE, related_name="likes", blank=True
    )
...
class Flag(SuperModel):
    ...
    profile = models.ForeignKey(
        "dashboard.Profile", on_delete=models.CASCADE, related_name="flags", blank=True
    )
    activity = models.ForeignKey(
        "dashboard.Activity",
        on_delete=models.CASCADE,
...
class Comment(SuperModel):
    ...
    )  # pks of users who like this post
    likes_handles = ArrayField(
        models.CharField(max_length=200, blank=True), default=list, blank=True
    )  # handles of users who like this post
...
def presave_comment(sender, instance, **kwargs):
    ...
    instance.likes_handles = list(
        Profile.objects.filter(pk__in=instance.likes).values_list("handle", flat=True)
    )
...
class OfferQuerySet(models.QuerySet):
    ...
    def current(self):
        ...
        return self.filter(
            valid_from__lte=timestamp, valid_to__gt=timestamp, public=True
        )
...
class Offer(SuperModel):
    ...
    style = models.CharField(
        max_length=50, db_index=True, choices=STYLES, default="announce1"
    )
    ...
    public = models.BooleanField(
        help_text="Is this available publicly yet?", default=True
    )
...
class OfferAction(SuperModel):
    ...
    offer = models.ForeignKey(
        "townsquare.Offer", on_delete=models.CASCADE, related_name="actions", blank=True
    )
        ...
        with transaction.atomic():
            ...
            for result in results:
                try:
                    ...
                    match_curve = clr.run_live_calc(
                        data, result["id"], 999999, total_pot
                    )
                    contributors = len(
                        set([ele[1] for ele in data if int(ele[0]) == profile.pk])
                    )
...
class MatchRanking(SuperModel):
    ...
    profile = models.ForeignKey(
        "dashboard.Profile",
        on_delete=models.CASCADE,
        related_name="match_rankings",
        blank=True,
    )
    round = models.ForeignKey(
        "townsquare.MatchRound",
        on_delete=models.CASCADE,
    ...
    contributions_total = models.DecimalField(
        default=0, decimal_places=2, max_digits=50
    )
...
def get_eligible_input_data(mr):
    ...
    earnings = Earning.objects.filter(
        created_on__gt=mr.valid_from, created_on__lt=mr.valid_to
    )
    ...
    earnings = earnings.filter(
        to_profile__isnull=False,
        from_profile__isnull=False,
        value_usd__isnull=False,
        network=network,
    )
    ...
    tips = list(
        Tip.objects.send_happy_path()
        .filter(
            Q(comments_priv__contains="activity:")
            | Q(comments_priv__contains="comment:")
            | Q(tokenName="ETH", amount__lte=0.05)
        )
        .values_list("pk", flat=True)
    )
    earnings = earnings.filter(source_id__in=tips)
    # filter out colluding profiles
    excluded_profiles = SquelchProfile.objects.filter(active=True).values_list(
...
class Favorite(SuperModel):
    ...
    activity = models.ForeignKey(
        "dashboard.Activity", on_delete=models.CASCADE, null=True
    )
...
class PinnedPost(SuperModel):
    ...
    user = models.ForeignKey(
        "dashboard.Profile", on_delete=models.CASCADE, related_name="pins"
    )
    activity = models.ForeignKey(
        "dashboard.Activity", on_delete=models.CASCADE, related_name="pin"
    )
...
class SquelchProfile(SuperModel):
    ...
    profile = models.ForeignKey(
        "dashboard.Profile", on_delete=models.CASCADE, related_name="squelches"
    )
