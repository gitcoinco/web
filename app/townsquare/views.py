...
def get_sidebar_tabs(request):
    ...
    hackathons = HackathonEvent.objects.filter(
        start_date__gt=timezone.now() - timezone.timedelta(days=10), end_date__gt=timezone.now())
    if hackathons.count():
...
def get_offers(request):
    ...
    if request.user.is_authenticated:
        available_offers = available_offers.exclude(
            actions__profile=request.user.profile, actions__what__in=['click', 'decline', 'go'])
...
def get_following_tribes(request):
    ...
    if request.user.is_authenticated:
        handles = request.user.profile.tribe_members.filter(
            org__data__type='Organization').values_list('org__handle', flat=True)
...
def api(request, activity_id):
    ...
    if request.GET.get('method') == 'comment':
        ...
        for comment in comments:
            ...
            comment_dict['likes'] = ", ".join(Profile.objects.filter(pk__in=comment.likes).values_list(
                'handle', flat=True)) if len(comment.likes) else "no one. Want to be the first?"
