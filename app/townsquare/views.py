...
def load_wallpapers(request):
    ...
    response = {"status": 200, "wallpapers": images}
...
def get_sidebar_tabs(request):
    ...
    if request.user.is_authenticated:
            ...
            new_tab = {
                ...
                "badge": max_of_ten(
                    get_specific_activities(
                        key, False, request.user, request.session.get(key, 0)
                    ).count()
                )
                if request.GET.get("tab") != key
                else 0,
            }
            tabs = [new_tab] + tabs
        ...
        if num_favorites:
            ...
            new_tab = {
                ...
                "badge": max_of_ten(activities) if request.GET.get("tab") != key else 0,
            }
            tabs = [new_tab] + tabs
            default_tab = "my_favorites"

        threads_last_24_hours = (
            max_of_ten(
                request.user.profile.subscribed_threads.filter(
                    pk__gt=request.session.get("my_threads", 0)
                ).count()
            )
            if request.GET.get("tab") != "my_threads"
            else 0
        )
    ...
    hackathons = HackathonEvent.objects.filter(
        start_date__lt=start_date, end_date__gt=end_date, visible=True
    )
    if hackathons.count():
...
def get_miniclr_info(request):
    ...
    matching_leaderboard = [
        {
            "i": obj.number,
            "following": request.user.profile == obj.profile
            ...
            "amount": f"{int(obj.contributions_total/1000)}k"
            if obj.contributions_total > 1000
            else round(obj.contributions_total, 2),
            "match_amount": obj.match_total,
            "you": obj.profile.pk == request.user.profile.pk
            if request.user.is_authenticated
            else False,
        }
...
def ignored_suggested_tribe(request, tribeId):
    ...
    return JsonResponse({"tribes": get_suggested_tribes(request)})
...
def get_suggested_tribes(request):
    ...
    if request.user.is_authenticated:
        ...
        handles = (
            TribeMember.objects.filter(profile=profile)
            .distinct("org")
            .values_list("org__handle", flat=True)
        )
        tribes = (
            Profile.objects.filter(is_org=True)
            .exclude(handle__in=list(handles))
...
def get_following_tribes(request):
    following_tribes = []
    if request.user.is_authenticated:
        handles = request.user.profile.tribe_members.filter(
            org__data__type="Organization"
        ).values_list("org__handle", flat=True)
...
def town_square(request):
    ...
    (
        ...
        is_direct_link,
        admin_link,
    ) = get_param_metadata(request, tab)
    ...
    context = {
        ...
        "REFER_LINK": f"https://gitcoin.co/townsquare/?cb=ref:{request.user.profile.ref_code}"
        if request.user.is_authenticated
        else None,
        ...
        "search": search,
        "tags": view_tags,
        "suggested_actions": SuggestedAction.objects.filter(active=True).order_by(
            "-rank"
        ),
    ...
    if request.user.is_authenticated:
        ...
        if email_subscriber:
            ...
            for key in request.POST.keys():
                email_subscriber.set_should_send_email_type_to(
                    key, bool(request.POST.get(key) == "true")
                )
...
def api(request, activity_id):
    ...
    if request.GET.get("method") == "comment":
        ...
        for comment in comments:
            ...
            counter += 1
            results[counter] += time.time() - start_time
            start_time = time.time()
            ...
            results[counter] += time.time() - start_time
            start_time = time.time()
            comment_dict["last_chat_status_title"] = comment_dict[
                "last_chat_status"
            ].title()
            counter += 1
            results[counter] += time.time() - start_time
            ...
            results[counter] += time.time() - start_time
            start_time = time.time()
            comment_dict["is_liked"] = request.user.is_authenticated and (
                request.user.profile.pk in comment.likes
            )
            counter += 1
            results[counter] += time.time() - start_time
            start_time = time.time()
            ...
            start_time = time.time()
            comment_dict["likes"] = (
                ", ".join(comment.likes_handles)
                if len(comment.likes)
                else "no one. Want to be the first?"
            )
            counter += 1
            results[counter] += time.time() - start_time
            start_time = time.time()
            comment_dict["name"] = (
                comment.profile.data.get("name", None) or comment.profile.handle
            )
            counter += 1
            results[counter] += time.time() - start_time
            start_time = time.time()
            # perf - 0.2 on a 150 comment thread
            comment_dict["default_match_round"] = (
                comment.profile.matchranking_this_round.default_match_estimate
                if comment.profile.matchranking_this_round
                else None
            )
            counter += 1
            results[counter] += time.time() - start_time
            ...
            results[counter] += time.time() - start_time
            start_time = time.time()
            comment_dict["sorted_match_curve"] = (
                comment.profile.matchranking_this_round.sorted_match_curve
                if comment.profile.matchranking_this_round
                else None
            )
            counter += 1
            ...
            comment_dict["redeem_link"] = (
                comment.redeem_link
                if is_authenticated
                and comment.tip
                and comment.tip.recipient_profile_id == profile.id
                else ""
            )
        ...
        if activity.tip:
            user_can_redeem = (
                request.user.profile.id == activity.tip.recipient_profile_id
            )
            response["has_tip"] = True
            response["tip_available"] = not activity.tip.recipient_profile
            response["can_redeem"] = (
                activity.tip.status == "PENDING" and user_can_redeem
            )
    ...
    if request.POST.get("method") == "vote":
        ...
        if not activity.has_voted(request.user):
            activity.metadata["poll_choices"][index]["answers"].append(
    ...
    # like request
        ...
        if request.POST["direction"] == "liked":
            already_likes = request.user.profile.likes.filter(
                activity=activity
            ).exists()
            if not already_likes:
    ...
    # award request
        ...
        if (
            request.user.profile.id == activity.profile.id
            and comment.activity_id == activity.id
        ):
    ...
    # favorite request
        ...
        if request.POST["direction"] == "favorite":
            already_likes = Favorite.objects.filter(
        ...
        if permission:
            if request.POST.get("direction") == "pin":
                pinned_post, created = PinnedPost.objects.update_or_create(
                    what=what,
                    defaults={"activity": activity, "user": request.user.profile},
                )
    ...
    # flag request
        ...
        if request.POST["direction"] == "flagged":
            ...
            is_hidden_by_staff = (
                activity.flags.filter(profile__user__is_staff=True).count() > 0
            )
            is_hidden_by_moderators = (
                activity.flags.filter(profile__user__groups__name="Moderators").count()
                > 0
            )
            is_hidden = (
                is_hidden_by_users or is_hidden_by_staff or is_hidden_by_moderators
            )
            if is_hidden:
        ...
        if "Just sent a tip of" not in comment:
            if not request.user.profile.is_blocked:
                comment = Comment.objects.create(
                    profile=request.user.profile, activity=activity, comment=comment
                )
...
def comment_v1(request, comment_id):
    response = {"status": 400, "message": "error: Bad Request."}
    ...
    try:
        ...
        response = {"status": 404, "message": "unable to find comment"}
        return JsonResponse(response)

    if comment.profile != profile:
        response = {"status": 401, "message": "user not authorized"}
        return JsonResponse(response)
    ...
    if method == "DELETE":
        comment.delete()
        response = {"status": 204, "message": "comment successfully deleted"}
        return JsonResponse(response)
    ...
    if method == "EDIT":
        ...
        comment.save()
        response = {"status": 203, "message": "comment successfully updated"}
        return JsonResponse(response)
...
def get_offer_and_create_offer_action(
    profile, offer_id, what, do_not_allow_more_than_one_offeraction=False
):
    offer = Offer.objects.current().get(pk=offer_id)
    if do_not_allow_more_than_one_offeraction and profile.offeractions.filter(
        what=what, offer=offer
    ):
...
def offer_go(request, offer_id, offer_slug):
    ...
    try:
        ...
        offer = get_offer_and_create_offer_action(
            request.user.profile, offer_id, "go", False
        )
...
def offer_decline(request, offer_id, offer_slug):
    ...
    try:
        ...
        if not request.user.is_authenticated:
            return redirect("/login/github?next=" + request.get_full_path())
        offer = get_offer_and_create_offer_action(
            request.user.profile, offer_id, "decline", False
        )
        return redirect("/")
    except:
        raise Http404
...
def offer_view(request, offer_id, offer_slug):
    ...
    try:
        ...
        if not request.user.is_authenticated:
            return redirect("/login/github?next=" + request.get_full_path())
        if (
            request.user.profile.offeractions.filter(what="go", offer=offer)
            and not is_debugging_offers
        ):
            raise Exception("already visited this offer")
        if not is_debugging_offers:
            OfferAction.objects.create(
                profile=request.user.profile, offer=offer, what="click"
            )
        # render page context
        context = {
            "title": offer.title,
...
def extract_metadata_page(request):
    ...
    return JsonResponse(
        {"status": "error", "message": "no url was provided"}, status=404
    )
