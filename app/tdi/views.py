...
def whitepaper_access(request, ratelimited=False):
    ...
    if ac.uses >= ac.maxuses:
        context["msg"] = _(
            "You have exceeded your maximum number of uses for this access code. Please contact founders@gitcoin.co"
        )
        return TemplateResponse(request, "whitepaper_accesscode.html", context)
    ...
    ).format(wa.invitecode, email__etc, wa.created_on.strftime("%Y-%m-%d %H:%M"), wa.ip)
    charlength = 3.5
