...
def bounty_request(request):
    ...
    if request.GET.get("code", False):
        ...
        ls = Link.objects.filter(
            shortcode=code, comments__icontains="Gitcoin Request Coin", uses__lt=5
        )
        if ls.exists():
            ls = ls.first()
...
def create_bounty_request_v1(request):
    ...
    if not user:
        response[
            "message"
        ] = "error: user needs to be authenticated to submit bounty request"
    ...
    try:
        ...
        response = {"status": 400, "message": "error: could not find tribe"}
        return JsonResponse(response)
    except Profile.MultipleObjectsReturned:
        response = {"status": 500, "message": "error: found multiple tribes"}
        return JsonResponse(response)
    ...
    try:
        ...
        kwargs = {"org": tribe_profile, "profile": profile}
        tribemember = TribeMember.objects.create(**kwargs)
        tribemember.save()

    response = {"status": 204, "message": "bounty request successfully created"}
...
def update_bounty_request_v1(request):
    ...
    if not user:
        response[
            "message"
        ] = "error: user needs to be authenticated to edit bounty request"
        return JsonResponse(response)
    ...
    try:
        ...
        response = {"status": 404, "message": "request bounty request does not exsist"}
        return JsonResponse(response)
    except BountyRequest.MultipleObjectsReturned:
        response = {"status": 500, "message": "error: found multiple bounty requests"}
        return JsonResponse(response)

    is_my_org = any(
        [
            bounty_request.tribe.handle.lower() == org.lower()
    ...
    response = {"status": 200, "message": "bounty request updated successfully"}
