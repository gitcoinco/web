...
class AppUrlsTestCase(TestCase):
    ...
    def test_sitemap_resolve(self):
        ...
        self.assertEqual(
            resolve("/sitemap.xml").view_name, "django.contrib.sitemaps.views.index"
        )
    ...
    def test_email_settings_reverse(self):
        ...
        self.assertEqual(
            reverse("email_settings", args=(priv_key,)), f"/settings/email/{priv_key}"
        )
    ...
    def test_leaderboard_reverse(self):
        ...
        self.assertEqual(
            reverse("leaderboard", args=("quarterly_earners",)),
            "/leaderboard/quarterly_earners",
        )
    ...
    def test_uniterested_reverse(self):
        ...
        self.assertEqual(
            reverse("uninterested", args=[1, 2]),
            "/actions/bounty/1/interest/2/uninterested/",
        )

    def test_uniterested_resolve(self):
        """Test the uninterested url and check the resolution"""
        self.assertEqual(
            resolve("/actions/bounty/1/interest/2/uninterested/").view_name,
            "uninterested",
        )
