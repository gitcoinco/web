...
def preprocess(request):
    ...
    profile = (
        ...
        if user_is_authenticated and hasattr(request.user, "profile")
        else None
    )
    ...
    context = {
        ...
        "last_chat_status": request.user.profile.last_chat_status
        if (hasattr(request.user, "profile") and user_is_authenticated)
        else False,
    ...
    if context["github_handle"]:
        ...
        if not settings.DEBUG:
            context["unclaimed_tips"] = (
                context["unclaimed_tips"].filter(network="mainnet").cache(timeout=60)
            )
            context["unclaimed_kudos"] = (
                context["unclaimed_kudos"].filter(network="mainnet").cache(timeout=60)
            )
