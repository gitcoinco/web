...
class PerformanceProfiler:
    ...
    def profile_time(self, name):
        ...
        print(
            f"pulled {name} in {round(this_time,2)} seconds (total: {round(total_time,2)} sec)"
        )
        self.last_time = time.time()
...
def get_bounty_history_at_date(statuses, date, keyword):
    ...
    try:
        ...
        amount += sum(
            ManualStat.objects.filter(key="bounty_gmv", date__lt=date).values_list(
                "val", flat=True
            )
        )
        return amount
...
def get_grants_history_at_date(date, keyword):
    try:
        ...
        amount = base_stats.filter(created_on__lte=date).first().val
        amount += sum(
            ManualStat.objects.filter(key="grants_gmv", date__lt=date).values_list(
                "val", flat=True
            )
        )
        return amount
...
def get_kudos_history_at_date(date, keyword):
    ...
    amount += sum(
        ManualStat.objects.filter(key="kudos_gmv", date__lt=date).values_list(
            "val", flat=True
        )
    )
    return amount
...
def get_ecosystem_history_at_date(date, keyword):
    ...
    if date > timezone.datetime(2018, 12, 23):
        amount += 51087.23
    amount += sum(
        ManualStat.objects.filter(key="ecosystem_gmv", date__lt=date).values_list(
            "val", flat=True
        )
    )
    return amount
...
def get_codefund_history_at_date(date, keyword):
    ...
    if date > timezone.datetime(2019, 10, 9):
        amount += sum(
            ManualStat.objects.filter(key="codefund_gmv", date__lt=date).values_list(
                "val", flat=True
            )
        )
    return amount
...
def get_tip_history_at_date(date, keyword):
    ...
    amount += sum(
        ManualStat.objects.filter(key="tip_gmv", date__lt=date).values_list(
            "val", flat=True
        )
    )
    return amount
...
def get_history(base_stats, copy, num_months=6):
    ...
    increment = 1000
    ticks = json.dumps(
        list(x * increment for x in range(0, int(today / increment) + 1))
    )
    history = [
        ["When", copy],
        ["Launch", 0],
    ...
    for i in _range:
        try:
            ...
            val = (
                base_stats.filter(created_on__lt=before_then)
                .order_by("-created_on")
                .first()
                .val
            )
            history = history + [
                [f"{i} month{plural} ago", val],
            ]
        except Exception:
            pass
...
def get_completion_rate(keyword):
    ...
    base_bounties = Bounty.objects.current().filter(
        network="mainnet", idx_status__in=["done", "expired", "cancelled"]
    )
    if keyword:
        base_bounties = base_bounties.filter(raw_data__icontains=keyword)
    eligible_bounties = base_bounties.filter(
        created_on__gt=(timezone.now() - timezone.timedelta(days=60))
    )
    ...
    not_completed_bounties = eligible_bounties.filter(
        idx_status__in=["expired", "cancelled"]
    ).count()
...
def get_funder_receiver_stats(keyword):
    ...
    all_value = earnings.exclude(value_usd__isnull=True).values_list(
        "value_usd", flat=True
    )
...
def get_hourly_rate_distribution(keyword, bounty_value_range=None, methodology=None):
    ...
    if bounty_value_range:
        base_bounties = base_bounties.filter(
            _val_usd_db__lt=bounty_value_range[1], _val_usd_db__gt=bounty_value_range[0]
        )
        hourly_rates = [
            ele.hourly_rate for ele in base_bounties if ele.hourly_rate is not None
        ]
        # print(bounty_value_range, len(hourly_rates))
    else:
        hourly_rates = [
            ele.hourly_rate
            for ele in base_bounties
            if is_valid_bounty_for_headline_hourly_rate(ele)
        ]
...
def get_bounty_median_turnaround_time(func="turnaround_time_started", keyword=None):
    base_bounties = get_base_done_bounties(keyword)
    eligible_bounties = base_bounties.exclude(idx_status="open").filter(
        created_on__gt=(timezone.now() - timezone.timedelta(days=60))
    )
    pickup_time_hours = []
...
def build_stat_results(keyword=None):
    ...
    context = {
        ...
        "title": _("Results"),
        "card_desc": _(
            "Gitcoin is transparent by design.  Here are some stats about our suite of OSS incentivization products."
        ),
    }
    ...
    if keyword:
        base_email_subscribers = EmailSubscriber.objects.filter(
        ...
        profile_usernames = list(profile_usernames) + list(
            [bounty.github_repo_name for bounty in base_bounties]
        )
        base_alumni = base_alumni.filter(profile__in=profile_pks)
        base_leaderboard = base_leaderboard.filter(
            github_username__in=profile_usernames
        )
    ...
    count_active_30d = (
        EmailEvent.objects.filter(
            event="open", created_on__gt=(timezone.now() - timezone.timedelta(days=30))
        )
        .distinct("email")
        .count()
    )
    count_active_90d = (
        EmailEvent.objects.filter(
            event="open", created_on__gt=(timezone.now() - timezone.timedelta(days=90))
        )
        .distinct("email")
        .count()
    )
    ...
    num_quests = 15
    top_quests = Quest.objects.filter(visible=True).order_by(
        "-ui_data__attempts_count"
    )[0:num_quests]
    context["top_quests"] = [
        {
            "title": quest.title,
    ...
    num_to_show = 30
    context["top_funders"] = (
        base_leaderboard.filter(active=True, leaderboard="quarterly_payers")
        .order_by("rank")
        .values_list("github_username", flat=True)[0:num_to_show]
    )
    ...
    context["top_coders"] = (
        base_leaderboard.filter(active=True, leaderboard="quarterly_earners")
        .order_by("rank")
        .values_list("github_username", flat=True)[0:num_to_show]
    )
    pp.profile_time("orgs")
    ...
    context["members_history"], context["slack_ticks"] = get_history(
        base_stats, "Members"
    )
    ...
    # jdi history
    key = (
        f"joe_dominance_index_30_{keyword}_value"
        if keyword
        else "joe_dominance_index_30_value"
    )
    base_stats = (
        Stat.objects.filter(
            key=key,
    ...
    num_months = int(
        (timezone.now() - timezone.datetime(2017, 10, 1).replace(tzinfo=pytz.UTC)).days
        / 30
    )
...
"""
    ...
    context["kudos_leaderboards"] = [
        [
            "Top Kudos Artists 👩‍🎨",
            get_kudos_leaderboard("kudos_token.artist"),
            "created",
        ],
        [
            "Top Kudos Collectors 🖼",
            get_kudos_leaderboard("kudos_kudostransfer.username"),
            "collected",
        ],
        [
            "Top Kudos Senders 💌",
            get_kudos_leaderboard("kudos_kudostransfer.from_username"),
            "sent",
        ],
    ]
    ...
    context["mau"] = (
        ManualStat.objects.filter(key="MAUs")
        .order_by("-pk")
        .values_list("val", flat=True)[0]
    )
    ...
    total_bounties_usd = sum(
        base_bounties.exclude(
            idx_status__in=["expired", "cancelled", "canceled", "unknown"]
        ).values_list("_val_usd_db", flat=True)
    )
    total_tips_usd = sum(
        [
            tip.value_in_usdt
            for tip in Tip.objects.filter(network="mainnet").send_happy_path()
    ...
    context["max_bounty_history"] = float(context["universe_total_usd"]) * 0.15
    context["bounty_abandonment_rate"] = bounty_abandonment_rate
    bounty_average_turnaround = round(
        get_bounty_median_turnaround_time("turnaround_time_submitted", keyword) / 24, 1
    )
    ...
    for val_range in usd_value_ranges:
        ...
        context["hourly_rate_distribution_by_range"][
            key
        ] = get_hourly_rate_distribution(keyword, val_range, "quartile")
    context["skill_rate_distribution_by_range"] = {}
    for programming_language in programming_languages:
        context["skill_rate_distribution_by_range"][
            programming_language
        ] = get_hourly_rate_distribution(programming_language, None, "quartile")
    ...
    context["kudos_tokens"] = KudosToken.objects.filter(
        num_clones_in_wild__gt=0
    ).order_by("-num_clones_in_wild")[0:25]
    ...
    context["title"] = (
        f"{total_gmv_rounded} in "
        + f"{keyword.capitalize() if keyword else ''} Results"
    )
    context["programming_languages"] = ["All"] + programming_languages

    try:
        context["pct_breakeven"] = ManualStat.objects.filter(
            key="pct_breakeven"
        ).values_list("val", flat=True)[0]
    except Exception as e:
        print(e)
    ...
    bh = (
        bounty_history[-1]
        if context["prev_month_name"] == bounty_history[-1][0]
        else bounty_history[-2]
    )
    bh[0] = 0
    context["last_month_amount"] = round(sum(bh) / 1000)
    context["last_month_amount_hourly"] = sum(bh) / 30 / 24
    context["last_month_amount_hourly_business_hours"] = (
        context["last_month_amount_hourly"] / 0.222
    )
    context["hackathons"] = [
        (ele, ele.stats)
        for ele in HackathonEvent.objects.filter(
            visible=True, start_date__lt=timezone.now()
        )
        .order_by("-start_date")
        .all()
    ]
    context["hackathon_total"] = sum(
        [ele[1]["total_volume"] for ele in context["hackathons"]]
    )
    from dashboard.models import FeedbackEntry

    reviews = (
        FeedbackEntry.objects.exclude(comment="")
        .filter(created_on__lt=(timezone.now() - timezone.timedelta(days=7)))
        .order_by("-created_on")[0:15]
    )
    ...
    context["hours"] = sum(
        BountyFulfillment.objects.filter(
            fulfiller_hours_worked__isnull=False,
            bounty__current_bounty=True,
            fulfiller_hours_worked__lt=1000,
        ).values_list("fulfiller_hours_worked", flat=True)
    )
    ...
    context["bounties_gmv"] = (
        Stat.objects.filter(key="bounties_done_value").order_by("-pk").first().val
    )
    context["bounties_gmv"] = (
        str(round((total_tips_usd + context["bounties_gmv"]) / 10 ** 6, 2)) + "m"
    )
    median_index = int(num_contributions / 2)
    context["median_contribution"] = round(
        Contribution.objects.order_by("subscription__amount_per_period_usdt")[
            median_index
    ...
    context["ads_served"] = (
        str(
            round(
                ManualStat.objects.filter(key="ads_served").order_by("-pk").first().val
                / 10 ** 6,
                1,
            )
        )
    ...
    context["privacy_violations"] = (
        ManualStat.objects.filter(key="privacy_violations").order_by("-pk").first().val
    )
