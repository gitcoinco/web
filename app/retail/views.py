...
@cached_as(
    Activity.objects.select_related('bounty').filter(bounty__network='mainnet').order_by('-created'), timeout=120
)
...
def contributor_landing(request, tech_stack):
    ...
    context = {
        ...
        'title':
            tech_stack.title() + str(_(" Open Source Opportunities")) if tech_stack else "Open Source Opportunities",
        'slides': slides,
        'slideDurationInMs': 6000,
        'active': 'home',
...
def how_it_works(request, work_type):
    ...
    context = {'active': f'how_it_works_{work_type}', }
...
def robotstxt(request):
    ...
    context = {'settings': settings, 'hidden_profiles': hidden_profiles, }
...
def about(request):
    core_team = [(
        static("v2/images/team/kevin-owocki.png"), "Kevin Owocki", "All the things", "owocki", "owocki",
        "The Community", "Avocado Toast"
    ),
                 (
                     static("v2/images/team/alisa-march.jpg"), "Alisa March", "User Experience Design",
                     "PixelantDesign", "pixelant", "Tips", "Apple Cider Doughnuts"
                 ),
                 (
                     static("v2/images/team/justin-bean.jpg"), "Justin Bean", "Engineering", "StareIntoTheBeard",
                     "justinbean", "Issue Explorer", "Sushi"
                 ),
                 (
                     static("v2/images/team/mark-beacom.jpg"), "Mark Beacom", "Engineering", "mbeacom", "mbeacom",
                     "Start/Stop Work", "Dolsot Bibimbap"
                 ),
                 (
                     static("v2/images/team/eric-berry.jpg"), "Eric Berry", "OSS Funding", "coderberry", "ericberry",
                     "Chrome/Firefox Extension", "Pastel de nata"
                 ),
                 (
                     static("v2/images/team/vivek-singh.jpg"), "Vivek Singh", "Community Buidl-er", "vs77bb",
                     "vivek-singh-b5a4b675", "Gitcoin Requests", "Tangerine Gelato"
                 ),
                 (
                     static("v2/images/team/aditya-anand.jpg"), "Aditya Anand M C", "Engineering", "thelostone-mc",
                     "aditya-anand-m-c-95855b65", "The Community", "Cocktail Samosa"
                 ),
                 (
                     static("v2/images/team/saptaks.jpg"), "Saptak Sengupta", "Engineering", "saptaks", "saptaks",
                     "Everything Open Source", "daab chingri"
                 ),
                 (
                     static("v2/images/team/scott.jpg"), "Scott Moore", "Biz Dev", "ceresstation",
                     "scott-moore-a2970075", "Issue Explorer", "Teriyaki Chicken"
                 ), ]
    exclude_community = ['kziemiane', 'owocki', 'mbeacom']
    community_members = []
    leadeboardranks = LeaderboardRank.objects.filter(
        active=True, leaderboard='quarterly_earners'
    ).exclude(github_username__in=exclude_community).order_by('-amount').cache()[0:15]
...
def vision(request):
    ...
    context = {
        ...
        'card_desc':
            _("Gitcoin's Vision for a web3 world is to make it easy for developers to find paid work in open source."),
    }
...
def not_a_token(request):
    ...
    context = {
        ...
        'card_desc':
            _(
                "We didn't do a token because we felt it wasn't the right way to align incentives with our user base.  Read more about the future of monetization in web3."
            ),
    }
...
def activity(request):
    ...
    context = {'p': p, 'page': p.get_page(page), 'title': _('Activity Feed'), }
        ...
        'Product': [
            {
                ...
                'a':
                    _(
                        "The <a href=https://gitcoin.co/explorer>Funded Issue Explorer</a> contains a handful of issues that are ready to be paid out as soon as they are turned around. Check out the developer guide at <a href=https://gitcoin.co/help/dev>https://gitcoin.co/help/dev</a>."
                    )
            },
            {
                ...
                'a':
                    _(
                        "The best way to get started is to post a funded issue on a task you need done.  Check out the repo maintainers guide at <a href=https://gitcoin.co/help/repo>https://gitcoin.co/help/repo</a>."
                    )
            },
            {
...
<li>
     Create delightful experiences
            ...
            },
            {
                ...
                'a':
                    _(
                        "We'd love to help!  Please email <a href='mailto:founders@gitcoin.co'>founders@gitcoin.co</a> or join <a href=/slack>Gitcoin Community Slack</a>."
                    )
            },
            {
        ...
        'General': [{
            ...
            'a':
                _(
                    "Yes, all of Gitcoin's core software systems are open source and available at <a href=https://github.com/gitcoinco/>https://github.com/gitcoinco/</a>.  Please see the liscense.txt file in each repo for more details."
                )
        },
                    {
...
"""
    ...
    tutorials = [{
        ...
        'title': _('Leverage Gitcoin’s Firehose of Talent to Do More Faster'),
                 ...
                 },
                 {
                     ...
                     'url':
                         'https://medium.com/gitcoin/tutorial-send-a-tip-to-any-github-user-in-60-seconds-2eb20a648bc8',
    ...
    context = {'active': 'help', 'title': _('Help'), 'faq': faq, 'tutorials': tutorials, }
    return TemplateResponse(request, 'help.html', context)


def presskit(request):

    brand_colors = [("Cosmic Teal", "#25e899", "37, 232, 153"), ("Dark Cosmic Teal", "#0fce7c", "15, 206, 124"),
                    ("Milky Way Blue", "#15003e", "21, 0, 62"), ("Stardust Yellow", "#FFCE08", "255,206, 8"),
                    ("Polaris Blue", "#3E00FF", "62, 0, 255"), ("Vinus Purple", "#8E2ABE", "142, 42, 190"),
                    ("Regulus Red", "#F9006C", "249, 0, 108"), ("Star White", "#FFFFFF", "23, 244, 238"), ]

    context = {'brand_colors': brand_colors, 'active': 'get', 'title': _('Presskit'), }
    return TemplateResponse(request, 'presskit.html', context)


def get_gitcoin(request):
    context = {'active': 'get_gitcoin', 'title': _('Get Started'), }
...
def error(request, code):
    context = {'active': 'error', 'code': code}
...
def ios(request):
    ...
    context = {
        ...
        'card_desc':
            'Gitcoin aims to make it easier to grow open source from anywhere in the world,\
...
def newtoken(request):
    context = {'active': 'newtoken', 'msg': None, }
    ...
    if request.POST:
        ...
        if validtion_passed:
            ...
            context['msg'] = str(
                _('Your token has been submitted and will be listed within 2 business days if it is accepted.')
            )
...
def increase_funding_limit_request(request):
    ...
    if request.body:
        if not user or not profile or not profile.handle:
            return JsonResponse({'error': _('You must be Authenticated via Github to use this feature!')}, status=401)
