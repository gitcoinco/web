...
MARKETING_EMAILS = [
    ...
    (
        "grant_recontribute",
        _("Recontribute to previously funded grants"),
        _("Quarterly"),
    ),
]
...
TRANSACTIONAL_EMAILS = [
    ...
    (
        "faucet",
        _("Faucet Notification Emails"),
        _("Only when you are sent a faucet distribution"),
    ),
    (
        "bounty",
        _("Bounty Notification Emails"),
        _("Only when you're active on a bounty"),
    ),
    (
        "bounty_match",
        _("Bounty Match Emails"),
        _("Only when you've posted a open bounty and you have a new match"),
    ),
    (
        "bounty_feedback",
        _("Bounty Feedback Emails"),
        _("Only after a bounty you participated in is finished."),
    ),
    (
        "bounty_expiration",
        _("Bounty Expiration Warning Emails"),
        _("Only after you posted a bounty which is going to expire"),
    ),
    (
        "featured_funded_bounty",
        _("Featured Funded Bounty Emails"),
        _("Only when you've paid for a bounty to be featured"),
    ),
    ...
    (
        "grant_updates",
        _("Grant Update Emails"),
        _("Updates from Grant Owners about grants you've funded."),
    ),
    (
        "grant_txn_failed",
        _("Grant Transaction Failed Emails"),
        _("Notifies Grant contributors when their contribution txn has failed."),
    ),
]
...
def render_featured_funded_bounty(bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/funded_featured_bounty.html", params)
    )
...
def render_nth_day_email_campaign(to_email, nth, firstname):
    ...
    response_html = premailer_transform(
        render_to_string(f"emails/campaigns/email_campaign_day_{nth}.html", params)
    )
    response_txt = render_to_string(
        f"emails/campaigns/email_campaign_day_{nth}.txt", params
    )
...
def render_new_grant_email(grant):
    params = {"grant": grant}
    response_html = premailer_transform(
        render_to_string("emails/grants/new_grant.html", params)
    )
...
def render_new_supporter_email(grant, subscription):
    ...
    response_html = premailer_transform(
        render_to_string("emails/grants/new_supporter.html", params)
    )
...
def render_thank_you_for_supporting_email(grant, subscription):
    params = {"grant": grant, "subscription": subscription}
    response_html = premailer_transform(
        render_to_string("emails/grants/thank_you_for_supporting.html", params)
    )
    response_txt = render_to_string(
        "emails/grants/thank_you_for_supporting.txt", params
    )
...
def render_support_cancellation_email(grant, subscription):
    params = {"grant": grant, "subscription": subscription}
    response_html = premailer_transform(
        render_to_string("emails/grants/support_cancellation.html", params)
    )
...
def render_grant_cancellation_email(grant, subscription):
    params = {"grant": grant, "subscription": subscription}
    response_html = premailer_transform(
        render_to_string("emails/grants/grant_cancellation.html", params)
    )
...
def render_subscription_terminated_email(grant, subscription):
    params = {"grant": grant, "subscription": subscription}
    response_html = premailer_transform(
        render_to_string("emails/grants/subscription_terminated.html", params)
    )
    response_txt = render_to_string("emails/grants/subscription_terminated.txt", params)
    subject = _(
        "Your subscription on Gitcoin Grants has been cancelled by the Grant Creator"
    )
    return response_html, response_txt, subject
...
def render_successful_contribution_email(grant, subscription, contribution):
    ...
    response_html = premailer_transform(
        render_to_string("emails/grants/successful_contribution.html", params)
    )
...
def successful_contribution(request):
    ...
    response_html, __, __ = render_successful_contribution_email(
...
def render_tip_email(to_email, tip, is_new):
    ...
    params = {
        ...
        "show_expires": not already_redeemed
        and tip.expires_date < (timezone.now() + timezone.timedelta(days=365))
        and tip.expires_date,
        "is_new": is_new,
        "warning": warning,
        "subscriber": get_or_save_email_subscriber(to_email, "internal"),
...
def render_request_amount_email(to_email, request, is_new):
    ...
    response_html = premailer_transform(
        render_to_string("emails/request_funds.html", params)
    )
    response_txt = render_to_string("emails/new_tip.txt", params)
...
def render_kudos_email(
    to_email, kudos_transfer, is_new, html_template, text_template=None
):
    """Summary
    ...
    params = {
        ...
        or kudos_transfer.kudos_token_cloned_from,
...
render_new_kudos_email = partial(
    render_kudos_email,
    html_template="emails/new_kudos.html",
    text_template="emails/new_kudos.txt",
)
render_sent_kudos_email = partial(
    render_kudos_email,
    html_template="emails/new_kudos.html",
    text_template="emails/new_kudos.txt",
)
render_kudos_accepted_email = partial(
    render_kudos_email,
    html_template="emails/new_kudos.html",
    text_template="emails/new_kudos.txt",
)
render_kudos_mint_email = partial(
    render_kudos_email, html_template="emails/kudos_mint.html", text_template=None
)
render_kudos_mkt_email = partial(
    render_kudos_email, html_template="emails/kudos_mkt.html", text_template=None
)
...
def render_match_email(bounty, github_username):
    ...
    response_html = premailer_transform(
        render_to_string("emails/new_match.html", params)
    )
...
def render_quarterly_stats(to_email, platform_wide_stats):
    ...
    response_html = premailer_transform(
        render_to_string("emails/quarterly_stats.html", params)
    )
...
def render_tax_report(to_email, tax_year):
    ...
    response_html = premailer_transform(
        render_to_string("emails/tax_report.html", params)
    )
...
def render_funder_payout_reminder(**kwargs):
    kwargs["bounty_fulfillment"] = (
        kwargs["bounty"]
        .fulfillments.filter(profile__handle=kwargs["github_username"])
        .last()
    )
    response_html = premailer_transform(
        render_to_string("emails/funder_payout_reminder.html", kwargs)
    )
    response_txt = ""
    return response_html, response_txt
...
def render_match_distribution(mr):
    ...
    response_html = premailer_transform(
        render_to_string("emails/match_distribution.html")
    )
    response_txt = ""
    return response_html, response_txt
...
def render_no_applicant_reminder(bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/bounty/no_applicant_reminder.html", params)
    )
...
def render_bounty_feedback(bounty, persona="submitter", previous_bounties=[]):
    if persona == "fulfiller":
        ...
        github_username = (
            " @" + accepted_fulfillments.first().fulfiller_github_username
            if accepted_fulfillments.exists()
            and accepted_fulfillments.first().fulfiller_github_username
            else ""
        )
        txt = f"""
hi{github_username},
...
"""
    ...
    params = {"txt": txt, "email_type": "bounty_feedback"}
...
"""
    params = {"txt": txt, "email_type": "admin_contact_funder"}
...
def get_notification_count(profile, days_ago, from_date):
    ...
    try:
        notifications_count = Notification.objects.filter(
            to_user=profile.user.id,
            is_read=False,
            created_on__range=[to_date, from_date],
        ).count()
...
def render_new_bounty(
    ...
    from townsquare.utils import (
        is_email_townsquare_enabled,
        is_there_an_action_available,
    )
        ...
        upcoming_events = upcoming_events + [
            {
                ...
                if hackathon.logo
                else f"{settings.STATIC_URL}v2/images/emails/hackathons-neg.png",
                "url": hackathon.url,
    ...
    params = {
        ...
        "show_action": is_email_townsquare_enabled(to_email)
        and is_there_an_action_available(),
    }

    response_html = premailer_transform(
        render_to_string("emails/new_bounty.html", params)
    )
...
def render_unread_notification_email_weekly_roundup(
    to_email, from_date=date.today(), days_ago=7
):
    subscriber = get_or_save_email_subscriber(to_email, "internal")
    from dashboard.models import Profile
    from inbox.models import Notification
    ...
    notifications = Notification.objects.filter(
        to_user=profile.user.id, is_read=False, created_on__range=[to_date, from_date]
    ).count()
    ...
    response_html = premailer_transform(
        render_to_string(
            "emails/unread_notifications_roundup/unread_notification_email_weekly_roundup.html",
            params,
        )
    )
    response_txt = render_to_string(
        "emails/unread_notifications_roundup/unread_notification_email_weekly_roundup.txt",
        params,
    )
...
def render_weekly_recap(to_email, from_date=date.today(), days_back=7):
    ...
    _sections = []
    activity_types_def = {
        "start_work": {"css-class": "status-open", "text": "Started work"},
        "stop_work": {"css-class": "status-cancelled", "text": "Work stopped"},
        "worker_approved": {"css-class": "status-open", "text": "Worker got approved"},
        "worker_applied": {"css-class": "status-submitted", "text": "Worker applied"},
        "new_bounty": {"css-class": "status-open", "text": "New created bounties"},
        "work_submitted": {
    ...
    params = {
        ...
        "select_params": {"from": from_date, "to": to_date},
        "debug": activity_types,
        "email_type": "roundup",
    }

    response_html = premailer_transform(
        render_to_string("emails/recap/weekly_founder_recap.html", params)
    )
...
def render_gdpr_reconsent(to_email):
    sub = get_or_save_email_subscriber(to_email, "internal")
    params = {"subscriber": sub, "email_type": "roundup"}

    response_html = premailer_transform(
        render_to_string("emails/gdpr_reconsent.html", params)
    )
...
def render_share_bounty(
    to_email, msg, from_profile, invite_url=None, kudos_invite=False
):
    ...
    response_html = premailer_transform(
        render_to_string("emails/share_bounty_email.html", params)
    )
...
def render_new_work_submission(to_email, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/new_work_submission.html", params)
    )
...
def render_new_bounty_acceptance(to_email, bounty, unrated_count=0):
    ...
    response_html = premailer_transform(
        render_to_string("emails/new_bounty_acceptance.html", params)
    )
...
def render_new_bounty_rejection(to_email, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/new_bounty_rejection.html", params)
    )
...
def render_grant_update(to_email, activity):
    ...
    response_html = premailer_transform(
        render_to_string("emails/grant_update.html", params)
    )
...
def render_grant_recontribute(
    ...
    match_pool="175k",
):  # Round 5: 3/23/2020 — 4/7/2020; Round 6: 6/15/2020 — 6/29/2020 175k
    email_style = 27
    ...
    for subscription in subscriptions:
        ...
        contributions_count = subscription.subscription_contribution.filter(
            success=True,
            created_on__gte=datetime.datetime(*prev_round_start),
            created_on__lte=datetime.datetime(*prev_round_end),
        ).count()
        total_contribution_to_grant = (
            subscription.amount_per_period * contributions_count
        )
            ...
            prev_grants.append(
                {
                    ...
                    "url": grant.url[1:],
                    "title": grant.title,
                    "image_url": grant.logo.url
                    if grant.logo
                    else f"{settings.STATIC_URL}v2/images/emails/grants-symbol-pos.png",
    ...
    params = {
        ...
        "base_url": settings.BASE_URL,
        "bulk_add_url": "https://gitcoin.co/grants/cart/bulk-add/"
        + ",".join(str(grant["id"]) for grant in prev_grants),
        "hide_bottom_logo": True,
    }

    response_html = premailer_transform(
        render_to_string("emails/grant_recontribute.html", params)
    )
...
def render_grant_txn_failed(contribution):
    ...
    if contribution.tx_id:
        ...
        contributions = Contribution.objects.filter(
            split_tx_id=contribution.split_tx_id
        )
    ...
    params = {
        ...
        "bulk_add_url": "https://gitcoin.co/grants/cart/bulk-add/"
        + ",".join([str(ele.id) for ele in grants]),
        "email_style": email_style,
        "hide_bottom_logo": True,
    }

    response_html = premailer_transform(
        render_to_string("emails/grant_txn_failed.html", params)
    )
...
def render_wallpost(to_email, activity):
    ...
    response_html = premailer_transform(
        render_to_string("emails/wall_post.html", params)
    )
...
def render_bounty_changed(to_email, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/bounty_changed.html", params)
    )
...
def render_bounty_hypercharged(to_email, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/bounty_hypercharged.html", params)
    )
...
def render_bounty_expire_warning(to_email, bounty):
    ...
    fulfiller_emails = list(
        bounty.fulfillments.annotate(lower_email=Lower("fulfiller_email")).values_list(
            "lower_email"
        )
    )
    ...
    response_html = premailer_transform(
        render_to_string("emails/new_bounty_expire_warning.html", params)
    )
...
def render_bounty_startwork_expire_warning(to_email, bounty, interest, time_delta_days):
    ...
    response_html = premailer_transform(
        render_to_string("emails/bounty_startwork_expire_warning.html", params)
    )
    response_txt = render_to_string(
        "emails/bounty_startwork_expire_warning.txt", params
    )
...
def render_bounty_unintersted(to_email, bounty, interest):
    ...
    response_html = premailer_transform(
        render_to_string("emails/bounty_uninterested.html", params)
    )
...
def render_faucet_rejected(fr):
    ...
    response_html = premailer_transform(
        render_to_string("emails/faucet_request_rejected.html", params)
    )
...
def render_faucet_request(fr):
    ...
    response_html = premailer_transform(
        render_to_string("emails/faucet_request.html", params)
    )
...
def render_bounty_startwork_expired(to_email, bounty, interest, time_delta_days):
    ...
    response_html = premailer_transform(
        render_to_string("emails/render_bounty_startwork_expired.html", params)
    )
    response_txt = render_to_string(
        "emails/render_bounty_startwork_expired.txt", params
    )
...
def render_gdpr_update(to_email):
    ...
    response_html = premailer_transform(
        render_to_string("emails/gdpr_update.html", params)
    )
...
def render_reserved_issue(to_email, user, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/reserved_issue.html", params)
    )
...
def render_bounty_request(to_email, model, base_url):
    ...
    response_html = premailer_transform(
        render_to_string("emails/bounty_request.html", params)
    )
...
def render_start_work_approved(interest, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/start_work_approved.html", params)
    )
...
def render_start_work_rejected(interest, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/start_work_rejected.html", params)
    )
...
def render_start_work_new_applicant(interest, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/start_work_new_applicant.html", params)
    )
...
def render_start_work_applicant_about_to_expire(interest, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/start_work_applicant_about_to_expire.html", params)
    )
    response_txt = render_to_string(
        "emails/start_work_applicant_about_to_expire.txt", params
    )
...
def render_start_work_applicant_expired(interest, bounty):
    ...
    response_html = premailer_transform(
        render_to_string("emails/start_work_applicant_expired.html", params)
    )
...
def render_new_bounty_roundup(to_email):
    ...
    if highlight_kudos_ids:
        ...
        ).order_by("-created_on")[:num_kudos_to_show]
    ...
    if args.kudos:
        for requested_kudos in args.kudos:
            try:
                ...
                kudos.airdrop = requested_kudos.get(
                    "airdrop", f"https://gitcoin.co/kudos/{kudos.pk}/"
                )
...
def unread_notification_email_weekly_roundup(request):
    response_html, _, _ = render_unread_notification_email_weekly_roundup(
        "sebastian.tharakan97@gmail.com"
    )
    return HttpResponse(response_html)
...
def new_kudos(request):
    ...
    response_html, _ = render_new_kudos_email(
        settings.CONTACT_EMAIL, kudos_transfer, True
    )
...
def kudos_mint(request):
    ...
    response_html, _ = render_kudos_mint_email(
        settings.CONTACT_EMAIL, kudos_transfer, True
    )
...
def kudos_mkt(request):
    ...
    kudos_transfer = KudosTransfer.objects.last()
    response_html, _ = render_kudos_mkt_email(
        settings.CONTACT_EMAIL, kudos_transfer, True
    )
...
def new_match(request):
    ...
    response_html, _ = render_match_email(
        Bounty.objects.exclude(title="").last(), "owocki"
    )
...
def new_bounty_rejection(request):
    ...
    response_html, _ = render_new_bounty_rejection(
        settings.CONTACT_EMAIL, Bounty.objects.last()
    )
    return HttpResponse(response_html)
...
def grant_update(request):
    ...
    response_html, _ = render_grant_update(
        settings.CONTACT_EMAIL,
        Activity.objects.filter(activity_type="wall_post", grant__isnull=False).last(),
    )
    return HttpResponse(response_html)
...
def grant_txn_failed(request):
    failed_contrib = (
        Contribution.objects.filter(
            subscription__contributor_profile__user__email=settings.CONTACT_EMAIL
        )
        .exclude(validator_passed=True)
        .first()
    )
...
def wallpost(request):
    ...
    response_html, _ = render_wallpost(
        settings.CONTACT_EMAIL,
        Activity.objects.filter(activity_type="wall_post").last(),
    )
    return HttpResponse(response_html)
...
def new_bounty_acceptance(request):
    ...
    response_html, _ = render_new_bounty_acceptance(
        settings.CONTACT_EMAIL, Bounty.objects.last()
    )
    return HttpResponse(response_html)
...
def bounty_feedback(request):
    ...
    response_html, _ = render_bounty_feedback(
        Bounty.objects.current().filter(idx_status="done").last(), "foo"
    )
    return HttpResponse(response_html)
...
def funder_payout_reminder(request):
    ...
    response_html, _ = render_funder_payout_reminder(
        bounty=bounty, github_username=github_username
    )
    return HttpResponse(response_html)
...
def bounty_expire_warning(request):
    ...
    response_html, _ = render_bounty_expire_warning(
        settings.CONTACT_EMAIL, Bounty.objects.last()
    )
    return HttpResponse(response_html)
...
def start_work_expired(request):
    ...
    response_html, _ = render_bounty_startwork_expired(
        settings.CONTACT_EMAIL, Bounty.objects.last(), Interest.objects.all().last(), 5
    )
    return HttpResponse(response_html)
...
def start_work_expire_warning(request):
    ...
    response_html, _ = render_bounty_startwork_expire_warning(
        settings.CONTACT_EMAIL, Bounty.objects.last(), Interest.objects.all().last(), 5
    )
    return HttpResponse(response_html)
...
def share_bounty(request):
    ...
    handle = request.GET.get("handle")
    profile = Profile.objects.filter(handle=handle).first()
    response_html, _ = render_share_bounty(
        settings.CONTACT_EMAIL, "This is a sample message", profile
    )
    return HttpResponse(response_html)
