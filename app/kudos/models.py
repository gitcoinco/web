...
class Token(SuperModel):
    ...
    num_clones_available_counting_indirect_send = models.IntegerField(
    ...
    def artist_others(self):
        ...
        return Token.objects.filter(
            artist=self.artist, num_clones_allowed__gt=1, hidden=False
        )
    ...
    def owners_profiles(self):
        ...
        related_kudos_transfers = KudosTransfer.objects.filter(
            kudos_token_cloned_from=self.pk
        ).send_happy_path()
        related_profiles_pks = related_kudos_transfers.values_list(
            "recipient_profile_id", flat=True
        )
        related_profiles = Profile.objects.filter(
            pk__in=related_profiles_pks
        ).distinct()
    ...
    def owners_handles(self):
        ...
        related_kudos_transfers = KudosTransfer.objects.filter(
            kudos_token_cloned_from=self.pk
        ).exclude(recipient_profile__isnull=True)
        related_kudos_transfers = (
            related_kudos_transfers.send_success()
            | related_kudos_transfers.send_pending()
        )
        related_kudos_transfers = related_kudos_transfers.distinct("id")

        return related_kudos_transfers.values_list(
            "recipient_profile__handle", flat=True
        )
    ...
    def __str__(self):
        ...
        return (
            f"{self.contract.network} Gen {self.gen} Kudos Token: {self.humanized_name}"
        )
    ...
    def as_img(self):
        ...
        if (
            ...
            and settings.AWS_STORAGE_BUCKET_NAME in self.image
        ):
    ...
    def is_available(self):
        return (
            self.num_clones_allowed > 0
            and self.num_clones_available_counting_indirect_send > 0
        )
    ...
    def send_enabled_for(self, user):
        ...
        is_enabled_for_this_user = (
            hasattr(user, "profile")
            and TransferEnabledFor.objects.filter(
                profile=user.profile, token=self
            ).exists()
        )
        is_enabled_because_staff = user.is_authenticated and user.is_staff
        return (
            is_enabled_for_this_user
            or is_enabled_for_user_in_general
            or is_enabled_because_staff
        )


@receiver(pre_save, sender=Token, dispatch_uid="psave_token")
def psave_token(sender, instance, **kwargs):
    instance.num_clones_available_counting_indirect_send = (
        instance._num_clones_available_counting_indirect_send
    )
...
class KudosTransfer(SendCryptoAsset):
    ...
    kudos_token_cloned_from = models.ForeignKey(
        "kudos.Token",
        related_name="kudos_token_cloned_from",
        on_delete=models.SET_NULL,
        null=True,
    )
    ...
    def receive_url_for_recipient(self):
        ...
        try:
            ...
            return (
                f"{settings.BASE_URL}kudos/receive/v3/{key}/{self.txid}/{self.network}"
            )
...
def psave_kt(sender, instance, **kwargs):
    ...
    if token:
        all_transfers = KudosTransfer.objects.filter(
            kudos_token_cloned_from=token
        ).send_happy_path()
        token.popularity = all_transfers.count()
        token.popularity_week = all_transfers.filter(
            created_on__gt=(timezone.now() - timezone.timedelta(days=7))
        ).count()
        token.popularity_month = all_transfers.filter(
            created_on__gt=(timezone.now() - timezone.timedelta(days=30))
        ).count()
        token.popularity_quarter = all_transfers.filter(
            created_on__gt=(timezone.now() - timezone.timedelta(days=90))
        ).count()
        token.save()
...
class BulkTransferCoupon(SuperModel):
    ...
    @property
    def paid_until(self):
        return self.created_on + timezone.timedelta(
            minutes=self.make_paid_for_first_minutes
        )
...
def psave_BulkTransferCoupon(sender, instance, **kwargs):
    is_owned_by_gitcoin = (
        instance.token.owner_address.lower()
...
class BulkTransferRedemption(SuperModel):
    ...
    coupon = models.ForeignKey(
        "kudos.BulkTransferCoupon",
        related_name="bulk_transfer_redemptions",
...
class TokenRequest(SuperModel):
    ...
    gas_price_overide = models.IntegerField(
        default=0,
        help_text=(
            "If non-zero, then the celery task will use this gas price to mint hte kudos"
        ),
    )
    ...
    def mint(self, gas_price_gwei=None):
        ...
        from kudos.management.commands.mint_all_kudos import (
            mint_kudos,
        ...
        gas_price_gwei = (
            recommend_min_gas_price_to_confirm_in_time(1) * 2
        ...
        tx_id = mint_kudos(
            ...
            private_key,
            gas_price_gwei,
            mint_to=self.to_address,
            live=True,
            dont_wait_for_kudos_id_return_tx_hash_instead=True,
        )
        self.processed = True
        self.approved = True
        self.save()
...
class TransferEnabledFor(SuperModel):
    ...
    token = models.ForeignKey(
        "kudos.Token",
        related_name="transfers_enabled",
        on_delete=models.CASCADE,
    )
    profile = models.ForeignKey(
        "dashboard.Profile",
