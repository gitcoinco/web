...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "network",
            type=str,
            choices=["localhost", "rinkeby", "mainnet"],
            help="ethereum network to use",
        )
        parser.add_argument(
            "syncmethod",
            type=str,
            choices=["filter", "id", "block", "opensea"],
    ...
    def opensea_sync(self, kudos_contract, start_id):
        if kudos_contract.network == "rinkeby":
            ...
            raise RuntimeError(
                "The Open Sea API is only supported for contracts on rinkeby and mainnet."
            )
        ...
        for token_id in token_ids:
            ...
            try:
                ...
                transaction_hash = r.json()["asset_events"][0]["transaction"][
                    "transaction_hash"
                ]
                logger.info(f"token_id: {asset_token_id}, txid: {transaction_hash}")
                kudos_contract.sync_db(
                    kudos_id=int(asset_token_id), txid=transaction_hash
                )
            except IndexError:
                continue
            except HTTPError:
    ...
    def filter_sync(self, kudos_contract, fromBlock):
        event_filter = kudos_contract._contract.events.Transfer.createFilter(
            fromBlock=fromBlock
        )
        ...
        for event in event_filter.get_all_entries():
            ...
            kudos_contract.sync_db(
                kudos_id=event.args._tokenId, txid=event.transactionHash.hex()
            )
    ...
    def handle(self, *args, **options):
        ...
        if syncmethod == "filter":
            if start:
                if start.isdigit():
                    raise RuntimeError(
                        "This option is unstable if not on web3py 4.7.2.  May crash testrpc."
                    )
        ...
        if start:
            ...
            start_id = Token.objects.filter(
                contract__address=kudos_contract.address
            ).aggregate(Max("token_id"))["token_id__max"]
