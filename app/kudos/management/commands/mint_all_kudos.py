...
def mint_kudos(
    ...
    if not image_path:
        ...
        if image_name:
            ...
            if kudos_contract.network == "rinkeby":
                image_path = (
                    f"https://ss.gitcoin.co/static/v2/images/kudos/{image_name}"
                )
    ...
    if is_live:
        try:
            ...
            print(
                "Live run - Name: ", readable_name, " - Account: ", account, "Minted!"
            )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "yaml_file", help="absolute path to kudos.yaml file", type=str
        )
        parser.add_argument("--mint_to", help="address to mint the kudos to", type=str)
        parser.add_argument(
            "--gas_price_gwei", help="the gas price for mining", type=int
        )
        ...
        parser.add_argument(
            "--debug", help="turn on debug statements", action="store_true"
        )
        parser.add_argument(
            "--live",
        ...
        parser.add_argument(
            "--filter_svg", default="", type=str, dest="kudos_filter_svg"
        )
    ...
    def handle(self, *args, **options):
        ...
        for __, kudos in enumerate(all_kudos):
            is_match = (kudos_filter and kudos_filter in kudos["name"]) or (
                kudos_filter_svg and kudos_filter_svg in kudos.get("image", "")
            )
