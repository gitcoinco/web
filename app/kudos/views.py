...
def get_profile(handle):
    ...
    try:
        ...
        to_profile = (
            Profile.objects.filter(handle=handle.lower())
            .order_by("-created_on")
            .first()
        )
...
def about(request):
    ...
    activity_limit = 5
    listings = (
        Token.objects.select_related("contract")
        .filter(
...
def marketplace(request):
    ...
    token_list = (
        Token.objects.select_related("contract").visible().filter(**query_kwargs)
    )
    ...
    if len(pks):
        increment_view_count.delay(
            pks, token_list.first().content_type, request.user.id, "index"
        )
...
def details_by_address_and_token_id(request, address, token_id, name=None):
    kudos = get_token(
        token_id=token_id, network=settings.KUDOS_NETWORK, address=address
    )
...
def details(request, kudos_id, name):
    ...
    context = {
        ...
        "avatar_url": request.build_absolute_uri(
            static("v2/images/kudos/assets/kudos-image.png")
        ),
        "kudos": kudos,
        "pinned": pinned,
    ...
    if kudos:
        ...
        # increment view counts
        increment_view_count.delay(
            [token.pk], token.content_type, request.user.id, "individual"
        )
...
def get_primary_from_email(params, request):
    ...
    access_token = (
        request.user.profile.get_access_token() if request.user.is_authenticated else ""
    )
...
def send_2(request):
    ...
    if username:
        ...
        if profiles.exists():
            ...
            if profile.avatar_baseavatar_related.exists():
                ...
                user[
                    "avatar_url"
                ] = profile.avatar_baseavatar_related.first().avatar_url
    ...
    params = {
        ...
        "class": "send2",
        "recommend_gas_price": recommend_min_gas_price_to_confirm_in_time(
...
def send_3(request):
    ...
    access_token = (
        request.user.profile.get_access_token()
        if is_user_authenticated and request.user.profile
        else ""
    )
...
def send_4(request):
    ...
    is_authenticated_via_login = (
        kudos_transfer.from_username and kudos_transfer.from_username == from_username
    )
    ...
    if not is_authed:
        return JsonResponse(
            {"status": "error", "message": _("Permission Denied")}, status=401
        )
    ...
    if is_direct_to_recipient:
        record_kudos_activity(kudos_transfer, kudos_transfer.username, "receive_kudos")
...
def record_kudos_email_activity(kudos_transfer, github_handle, event_name):
    kwargs = {
        ...
        "metadata": {
            ...
            "received_on": str(kudos_transfer.received_on)
            if kudos_transfer.received_on
            else None,
        },
    }
    try:
        ...
        logger.warning(
            f"error in record_kudos_email_activity: profile with github name {github_handle} not found"
        )
        return
    try:
    ...
    try:
        ...
        logger.debug(
            f"error in record_kudos_email_activity: {e} - {event_name} - {kudos_transfer} - {github_handle}"
        )
...
def record_kudos_activity(kudos_transfer, github_handle, event_name):
    ...
    try:
        ...
        logging.error(
            f"error in record_kudos_activity: profile with github name {github_handle} not found"
        )
        return
    ...
    try:
        ...
        logging.error(
            f"error in record_kudos_activity: {e} - {event_name} - {kudos_transfer} - {github_handle}"
        )
...
def receive(request, key, txid, network):
    ...
    """
    these_kudos_transfers = KudosTransfer.objects.filter(
        web3_type="v3", txid=txid, network=network
    )
    ...
    not_mined_yet = (
        get_web3(kudos_transfer.network).eth.getBalance(
            Web3.toChecksumAddress(kudos_transfer.metadata["address"])
        )
        == 0
    )
    did_fail = False
    ...
    if kudos_transfer.receive_txid:
        ...
        try:
            ...
            record_user_action(
                kudos_transfer.from_username, "receive_kudos", kudos_transfer
            )
            record_kudos_email_activity(
                kudos_transfer, kudos_transfer.username, "receive_kudos"
            )
            record_kudos_activity(
                kudos_transfer, kudos_transfer.username, "receive_kudos"
            )
    ...
    params = {
        ...
        "kudos_transfer": kudos_transfer,
        "title": f"Receive {kudos_transfer.kudos_token_cloned_from.humanized_name} Kudos"
        if kudos_transfer and kudos_transfer.kudos_token_cloned_from
        else _("Receive Kudos"),
        "avatar_url": kudos_transfer.kudos_token_cloned_from.img_url
        if kudos_transfer and kudos_transfer.kudos_token_cloned_from
        else None,
        "card_desc": f"You've received a {kudos_transfer.kudos_token_cloned_from.humanized_name} kudos!"
        if kudos_transfer and kudos_transfer.kudos_token_cloned_from
        else _("You've received a kudos"),
        "key": key,
        "is_authed": is_authed,
        "disable_inputs": kudos_transfer.receive_txid or not_mined_yet or not is_authed,
        "tweet_text": urllib.parse.quote_plus(
...
def redeem_bulk_coupon(
    ...
    private_key = (
        settings.KUDOS_PRIVATE_KEY if not coupon.sender_pk else coupon.sender_pk
    )
    kudos_owner_address = (
        settings.KUDOS_OWNER_ACCOUNT
        if not coupon.sender_address
        else coupon.sender_address
    )
    ...
    gas_price = int(
        int(
            recommend_min_gas_price_to_confirm_in_time(gas_price_confirmation_time)
            * 10 ** 9
        )
...
def receive_bulk(request, secret):
    ...
    tweet_text = (
        f"I just got a {coupon.token.humanized_name} Kudos on @gitcoin.  "
        if not request.GET.get("tweet", None)
        else request.GET.get("tweet")
    )
    gas_amount = round(
        0.00035 * 1.3 * float(recommend_min_gas_price_to_confirm_in_time(1)), 4
    )
    params = {
        "title": title,
        "card_title": title,
        ...
        "tweet_url": coupon.token.url
        if not request.GET.get("tweet_url")
        else request.GET.get("tweet_url"),
    }
    ...
    if request.POST:
        ...
        if validation_passed:
            ...
            response = s3.Bucket(settings.MEDIA_BUCKET).put_object(
                Key=key, Body=img, ACL="public-read", ContentType="image/svg+xml"
            )
            artwork_url = (
                f"https://{settings.MEDIA_BUCKET}.s3-us-west-2.amazonaws.com/{key}"
            )
            ...
            context["msg"] = str(
                _(
                    "Your Kudos has been submitted and will be listed within 2 business days if it is accepted."
                )
            )
