...
def redeem_bulk_coupon(coupon, profile, address, ip_address, save_addr=False, submit_later=False, exit_after_sending_tx=False, max_gas_price_we_are_willing_to_pay_gwei=15):
    ...
    if not profile.is_brightid_verified and not profile.is_twitter_verified and not profile.is_duniter_verified and not profile.trust_profile and profile.github_created_on > (timezone.now() - timezone.timedelta(days=7)):
