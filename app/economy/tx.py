...
SEARCH_METHOD_TRANSFER = (
    "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
)
SEARCH_METHOD_DEPOSIT = (
    "0xaef05ca429cf234724843763035496132d10808feeac94ee79441c83b6dd519a"
)
SEARCH_METHOD_APPROVAL = (
    "0x7c3bc83eb61feb549a19180bb8de62c55c110922b2a80e511547cf8deda5b25a"
)
...
check_amount = (
    lambda amount: int(amount[75:], 16)
    if len(amount) == 138
    else print(
        f"{bcolors.FAIL}{bcolors.UNDERLINE} {index_transaction} txid: {transaction_tax[:10]} -> status: 0 False - amount was off by 0.001 {bcolors.ENDC}"
    )
)
...
check_event_transfer = lambda contract_address, search, txid: w3.eth.filter(
    {"address": contract_address, "topics": [search, txid]}
)
get_decimals = lambda contract: int(contract.functions.decimals().call())

# BulkCheckout parameters
bulk_checkout_address = (
    "0x7d655c57f71464B6f83811C55D84009Cd9f5221C"  # same address on mainnet and rinkeby
)
...
def transaction_status(transaction, txid):
    ...
    try:
        ...
        approve_event = check_event_transfer(
            contract.address, SEARCH_METHOD_APPROVAL, txid
        )
        transfer_event = check_event_transfer(
            contract.address, SEARCH_METHOD_TRANSFER, txid
        )
        deposit_event = check_event_transfer(
            contract.address, SEARCH_METHOD_DEPOSIT, txid
        )
        ...
        human_readable_value = (
            Decimal(int(contract_value)) / Decimal(10 ** decimals) if decimals else None
        )
...
def check_transaction_contract(transaction_tax):
    ...
    if transaction is not None:
        ...
        if (
            token_address is not False
            and not token_address == "0x0000000000000000000000000000000000000000"
        ):
...
def get_token(token_symbol, network):
    ...
    """
    token = (
        Token.objects.filter(network=network, symbol=token_symbol, approved=True)
        .first()
        .to_dict
    )
    if token_symbol == "ETH":
...
def grants_transaction_validator_v2(contribution, w3):
    ...
    bulk_checkout_contract = w3.eth.contract(
        address=bulk_checkout_address, abi=bulk_checkout_abi
    )
    ...
    if status == "success":
        ...
        if not is_bulk_checkout:
            ...
            response["validation"][
                "comment"
            ] = f"This function only validates transactions through the BulkCheckout contract, but this transaction was sent to {to_address}"
            return response
        ...
        receipt = w3.eth.getTransactionReceipt(tx_hash)
        parsed_logs = bulk_checkout_contract.events.DonationSent().processReceipt(
            receipt
        )
        ...
        if len(parsed_logs) == 0:
            response["validation"][
                "comment"
            ] = "No DonationSent events found in this BulkCheckout transaction"
            return response
        ...
        expected_token = get_token(token_symbol, network)[
            "addr"
        ].lower()  # we compare by token address
        ...
        response["validation"][
            "comment"
        ] = "DonationSent event with expected recipient, amount, and token was not found in transaction logs"
        return response
    ...
    try:
        ...
        if receipt.status == 0:
            ...
            response["validation"][
                "comment"
            ] = "Transaction failed. See Etherscan for more details"
            return response
        ...
        response["validation"][
            "comment"
        ] = "Transaction receipt not found. Transaction may still be pending or was dropped"
        return response
...
def grants_transaction_validator(contribution, w3):
    ...
    for tx in tx_list:
        ...
        if status == "success":
            ...
            if (
                ...
                and contribution.subscription.grant.admin_address
                and contribution.subscription.token_address
            ):
    ...
    if not token_transfer:
        ...
        if token_transfer["token_name"] != contribution.subscription.token_symbol:
            validation[
                "comment"
            ] = f"Tokens do not match, {token_transfer['token_name']} != {contribution.subscription.token_symbol}"
            validation["passed"] = False
            ...
            _transfers = get_token_originators(
                recipient_address,
                token_address,
                from_address=from_address,
                return_what="transfers",
            ...
            failsafe = (
                _transfers["token_name"] == contribution.subscription.token_symbol
            )
            if failsafe:
            ...
            threshold = float(
                float(abs(contribution.subscription.amount_per_period_minus_gas_price))
                * float(validation_threshold_pct)
            )
            validation["passed"] = (
                abs(delta1) <= threshold or abs(delta2) <= threshold
            ) or (
            ...
            validation[
                "comment"
            ] = f"Transfer Amount is off by {round(delta1, 2)} / {round(delta2, 2)}"
...
def get_token_recipient_senders(recipient_address, token_address):
    ...
    transfers = contract.events.Transfer.getLogs(
        fromBlock=0, toBlock="latest", argument_filters={"to": recipient_address}
    )

    return [trim_null_address(transfer.args["from"]) for transfer in transfers]
...
def get_token_originators(
    to_address, token, from_address="", return_what="transfers", tx_id="", amounts=[]
):
    address = to_address
    ...
    url = (
        f"https://api.aleth.io/v1/{endpoint}?filter[to]="
        + address
        + "&filter[token]="
        + token
        + "&page%5Blimit%5D=100"
    ...
    if token == "0x0":
        url = (
            f"https://api.aleth.io/v1/{endpoint}?filter[account]="
            + address
            + "&page%5Blimit%5D=100"
        )
    if from_address:
    ...
    transfers = requests.get(url, headers=headers).json()

    # NEW: PARSE EVENT LOGS TO SEE WHAT'S GOING ON

    if (
        transfers.get("message")
        == "API rate limit exceeded. Please upgrade your account."
    ):
