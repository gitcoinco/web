    ...
    try:
        ...
        if rs_json["message"] != "OK":
            if type(rs_json["result"]) == list:
                logger.info(
                    f"No {params['action']} found in blocks {params['startblock']} to {params['endblock']}"
                )
            else:
                logger.error(
...
def save_incoming_tx(network, record):
    if not DigitalGoodPurchase.objects.filter(txid__iexact=record["hash"]).exists():
        decimals = (
            18.0
            if len(record["contractAddress"]) == 0
            else float(record["tokenDecimal"])
        )
        dgp = DigitalGoodPurchase.objects.create(
            emails=[],
            tokenName="ETH"
            if len(record["contractAddress"]) == 0
            else record["tokenSymbol"],
            tokenAddress=record["contractAddress"],
            amount=(int(record["value"]) / decimals)
            if decimals > 0
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--end-block", default=99999999999, type=int, help="End at block number"
        )
    ...
    def handle(self, *args, **options):
        ...
        dgp_records = (
            DigitalGoodPurchase.objects.filter(
                receive_address__iexact=options["account_address"],
                tokenName__iexact="ETH",
            )
            .order_by("-id")
        ...
        if len(dgp_records) > 0:
            ...
            logger.info(
                f"ETH Tx Records were found. Continuing from block {params['startblock']}"
            )
        ...
        if len(dgp_records) > 0:
            params["startblock"] = int(dgp_records[0]["metadata"]["blockNumber"]) + 1
            logger.info(
                f"Token Tx Records were found. Continuing from block {params['startblock']}"
            )
