...
def create_channel_if_not_exists(options):
    try:
        ...
        try:
            chat_driver.login()
            new_channel = chat_driver.channels.create_channel(
                options={
                    "team_id": options["team_id"],
                    "name": options["channel_name"],
                    "purpose": options["channel_purpose"]
                    if "channel_purpose" in options
                    else "",
                    "header": options["channel_header"]
                    if "channel_header" in options
                    else "",
                    "display_name": options["channel_display_name"],
                    "type": options["channel_type"]
...
def update_chat_notifications(profile, notification_key, status):
    ...
    patch_chat_user.delay(
        query_opts=query_opts, update_opts={"notify_props": notify_props}
    )
...
def chat_notify_default_props(profile):
    return {
        "email": "false"
        if should_suppress_notification_email(profile.user.email, "chat")
        else "true",
        "push": "mention",
        "comments": "never",
        "desktop": "all",
...
def associate_chat_to_profile(profile):
    ...
    try:
        ...
        if (
            profile.gitcoin_chat_access_token is ""
            or profile.gitcoin_chat_access_token is None
        ):
            try:
                profile_access_tokens = chat_driver.users.get_user_access_token(
                    profile.chat_id
                )
                ...
                logger.info(str(e))
                try:
                    profile_access_token = chat_driver.users.create_user_access_token(
        ...
        if not profile.chat_id:
            create_user_response = chat_driver.users.create_user(
                ...
                params={"tid": settings.GITCOIN_CHAT_TEAM_ID},
            )
...
def create_channel(self, options, bounty_id=None, retry: bool = True) -> None:
    ...
    """
    with redis.lock(
        "tasks:create_channel:%s" % options["channel_name"], timeout=LOCK_TIMEOUT
    ):
...
@app.shared_task(bind=True, max_retries=3)
def hackathon_chat_sync(
    self, hackathon_id: str, profile_handle: str = None, retry: bool = True
) -> None:
    try:
        chat_driver.login()
        ...
        if profile_handle is None:
            regs_to_sync = HackathonRegistration.objects.filter(
                hackathon__id=hackathon_id
            ).select_related("registrant")
        ...
        for hack_sponsor in hackathon.sponsor_profiles.all():
            created, new_channel_details = create_channel_if_not_exists(
                {
                    "team_id": settings.GITCOIN_HACK_CHAT_TEAM_ID,
                    "channel_display_name": f"company-{slugify(hack_sponsor.name)}"[
                        :60
                    ],
        ...
        for channel_id in channels_to_connect:
            try:
                current_channel_members = chat_driver.channels.get_channel_members(
                    channel_id
                )
            except Exception as e:
                continue
            current_channel_users = [
                member["user_id"] for member in current_channel_members
            ]
...
@app.shared_task(bind=True, max_retries=3)
def hackathon_project_chat_sync(
    self,
    hackathon_id: str = None,
    bounty_owner_handle: str = None,
    project_id: str = None,
    retry: bool = True,
) -> None:
    try:
...
@app.shared_task(bind=True, max_retries=3)
def add_to_channel(
    self, channel_details, chat_user_ids: list, retry: bool = True
) -> None:
    """
        ...
        for chat_user_id in chat_user_ids:
            try:
                if chat_user_id is "" or chat_user_id is None:
                    continue
                chat_driver.channels.add_user(
                    channel_details["id"], options={"user_id": chat_user_id}
                )
            except Exception as e:
                logger.info(str(e))
                continue
...
def patch_chat_user(self, query_opts, update_opts, retry: bool = True) -> None:
    ...
    with redis.lock(
        "tasks:update_user:%s" % query_opts["handle"], timeout=LOCK_TIMEOUT
    ):
        chat_driver.login()
        ...
        try:
            ...
            if query_opts["chat_id"] is None and query_opts["handle"]:
                try:
                    chat_user = chat_driver.users.get_user_by_username(
                        query_opts["handle"]
                    )
                    chat_id = chat_user["id"]
                    user_profile = Profile.objects.filter(
                        handle=query_opts["handle"].lower()
                    )
