    ...
    def get_interested_actions(self):
        with open(
            os.path.join(
                ...
                "github_api_issue_with_pr_call.json",
            )
        ) as f:
            return json.load(f)
...
class TestSyncPullRequestWithBountyFulfillments(TestCase):
    ...
    def setUp(self):
        ...
        fulfiller_profile = Profile.objects.create(
            data={}, handle="samplegitcoindeveloper1", email="fred@bar.com"
        )
    ...
    def test_handle_simple(self, mocked_requests):
        ...
        mock.patch(
            "marketing.management.commands.sync_pull_request_with_bounty_fulfillments.get_gh_issue_state",
            lambda x, y, z: "closed",
        ).start()
        mock.patch(
            "marketing.management.commands.sync_pull_request_with_bounty_fulfillments.get_interested_actions",
            lambda x, y: GitHubTestAPI().get_interested_actions(),
        ).start()
        ...
        with open(settings.BASE_DIR + "/assets/v2/css/lib/typography.css") as f:
            ...
            marketing.management.commands.sync_pull_request_with_bounty_fulfillments.Command().handle(
                live=False
            )

            assert (
                BountyFulfillment.objects.last().funder_last_notified_on >= start_time
            )
