...
class TestSendUnreadNotificationEmailsRoundup(TestCase):
    ...
    @patch("time.sleep")
    @patch(
        "marketing.management.commands.send_unread_notification_emails_roundup.unread_notification_email_weekly_roundup"
    )
    def test_handle_no_options(
        self, mock_unread_notification_email_weekly_roundup, *args
    ):
    ...
    def test_handle_with_options(
        self, mock_unread_notification_email_weekly_roundup, *args
    ):
        """Test command roundup which various options."""
        Command().handle(
            exclude_startswith="f", filter_startswith="jack", start_counter=0, live=True
        )

        assert mock_unread_notification_email_weekly_roundup.call_count == 1

        mock_unread_notification_email_weekly_roundup.assert_called_once_with(
            ["jackson@bar.com"]
        )
