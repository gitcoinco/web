...
class Command(BaseCommand):
    ...
    help = (
        "pulls github pull request event associated with unaccepted BountyFulfillments"
    )
    help += (
        " then notifies the funder by email the issue is open awaiting their review."
    )
    help += (
        " Also escalates bounties that are being held open even with an accepted PR."
    )
    ...
    def _get_pr_urls(self, actions, cross_referenced_action_github_username):
        ...
        for cross_referenced_action in actions:
            if cross_referenced_action["event"] == "cross-referenced":
                if (
                    cross_referenced_action["actor"]["login"]
                    == cross_referenced_action_github_username
                ):
                    for referenced_action in actions:
                        if referenced_action["event"] == "referenced":
                            if (
                                cross_referenced_action["pr_url"]
                                == referenced_action["pr_url"]
                            ):
    ...
    def _notify_funder(
        self, bounty, bounty_fulfillment, live, deadline, escalated_deadline
    ):
        try:
    ...
    def sync_pull_requests_with_bounty_fulfillments(self, options):
        ...
        bounty_fulfillments = (
            bounties_fulfilled_last_time_period
            | fulfillments_notified_before_last_time_period
        )
