...
class Command(BaseCommand):
    ...
    def sync_pull_requests_with_bounty_fulfillments(self):
        ...
        bounties_fulfilled_last_time_period = list(BountyFulfillment.objects.filter(accepted=False).filter(created_on__gt=start))
        fulfillments_notified_before_last_time_period = list(BountyFulfillment.objects.filter(accepted=False).filter(funder_last_notified_on__lt=start))
        bounty_fulfillments = bounties_fulfilled_last_time_period + fulfillments_notified_before_last_time_period

        bounty_fulfillments = [BountyFulfillment.objects.get(pk=263)]
        for bounty_fulfillment in bounty_fulfillments:
            ...
            if (closed_issue == 'closed'):
                ...
                if pr_ref_commit_url and pr_merged_commit_url and pr_ref_commit_url == pr_merged_commit_url:
                    notified = self.notify_funder(bounty.bounty_owner_email, bounty, bounty_fulfillment)
                ...
                print('Sending payment reminder: ' + bounty.github_org_name + '/' + bounty.github_repo_name + ' ' + bounty.github_issue_number + ' Emailed: ' + notified)
                if notified:
                    bounty_fulfillment.funder_last_notified_on = timezone.now()
                    bounty_fulfillment.save()


    def notify_funder(self, to_email, bounty, bounty_fulfillment):
        return funder_payout_reminder(to_email=to_email, bounty=bounty, bounty_fulfillment=bounty_fulfillment)

    def handle(self, *args, **options):
        self.sync_pull_requests_with_bounty_fulfillments()
