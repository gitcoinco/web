...
def pull_to_db():
    ...
    profiles = (
        Profile.objects.exclude(email="").filter(modified_on__gt=before_timestamp).all()
    )
    ...
    for email in Match.objects.filter(modified_on__gt=before_timestamp).values_list(
        "email", flat=True
    ):
    ...
    for email in Subscription.objects.filter(
        modified_on__gt=before_timestamp
    ...
    for b in (
        Bounty.objects.filter(modified_on__gt=before_timestamp)
    ...
    for email in WhitepaperAccess.objects.filter(
        modified_on__gt=before_timestamp
    ).values_list("email", flat=True):
        process_email(email, "whitepaperaccess")

    for email in WhitepaperAccessRequest.objects.filter(
        modified_on__gt=before_timestamp
    ).values_list("email", flat=True):
    ...
    for es in eses.values_list("email", flat=True):
        ...
        try:
            client.lists.members.create(
                list_id, {"email_address": email, "status": "subscribed"}
            )
...
def push_to_mailchimp():
    ...
    eses_funder = EmailSubscriber.objects.filter(
        active=True, created_on__gt=created_after, profile__persona_is_funder=True
    ).order_by("-pk")
    print("funder emails")
    ...
    eses_hunter = EmailSubscriber.objects.filter(
        active=True, created_on__gt=created_after, profile__persona_is_hunter=True
    ).order_by("-pk")
    print("hunter emails")
