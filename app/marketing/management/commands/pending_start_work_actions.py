...
def helper_execute(threshold, func_to_execute, action_str):
    ...
    interests = Interest.objects.filter(
        pending=True, created__gt=start_time, created__lt=end_time
    )
    print(f"{interests.count()} {action_str}")
    for interest in interests:
        bounty = interest.bounties.first()
        if bounty:
            has_approved_worker_already = bounty.interested.filter(
                pending=False
            ).exists()
            if (
                bounty.admin_override_suspend_auto_approval
            ):  # skip bounties where this flag is set
            ...
            if is_bounty_in_terminal_state or is_bounty_already_submitted:
                print(
                    "skipped bc of is_bounty_in_terminal_state | is_bounty_already_submitted"
                )
                continue
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # warnings
        helper_execute(
            THRESHOLD_HOURS_AUTO_APPROVE_WARNING,
        ...
        # auto approval
        helper_execute(
            THRESHOLD_HOURS_AUTO_APPROVE,
            start_work_applicant_expired_executer,
            "auto approval",
        )
