...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for day in days:
            ...
            interests = (
                Interest.objects.select_related("profile")
                .filter(
                ...
                for bounty in bounties:
                    ...
                    try:
                        ...
                        if not actions:
                            ...
                            delta_now_vs_last_action = (
                                timezone.now() - snooze_time - last_action_by_user
                            )
                            last_heard_from_user_days = delta_now_vs_last_action.days

                            # decide action params
                            should_warn_user = (
                                last_heard_from_user_days >= num_days_back_to_warn
                            )
                            should_delete_interest = (
                                last_heard_from_user_days
                                >= num_days_back_to_delete_interest
                            )
                            should_ignore = (
                                last_heard_from_user_days
                                >= num_days_back_to_ignore_bc_mods_got_it
                            )
