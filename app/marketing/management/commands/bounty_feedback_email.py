...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for bounty in bounties_last_timeperiod:
            ...
            previous_bounties = Bounty.objects.filter(modified_on__lt=start_time, idx_status__in=statues, bounty_owner_email=submitter_email)
            has_been_sent_before_to_persona = previous_bounties.count()
            if not has_been_sent_before_to_persona:
                bounty_feedback(bounty, 'submitter')
