...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        bounties_cancelled_last_timeperiod = (
            Bounty.objects.current()
            .filter(
                network="mainnet",
        ...
        for bounty in bounties_to_process:
            ...
            if accepted_fulfillments.exists() and bounty.status == "done":
                ...
                fulfillment_pks = BountyFulfillment.objects.filter(
                    accepted=True, fulfiller_email=fulfiller_email
                ).values_list("pk", flat=True)
                previous_bounties = (
                    Bounty.objects.current()
                    .filter(
                        idx_status__in=statues, fulfillments__pk__in=fulfillment_pks
                    )
                ...
                if (
                    not has_been_sent_before_to_persona
                    and not is_fulfiller_and_funder_same_person
                ):
