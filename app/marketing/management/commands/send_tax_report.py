...
MISC_1099_COPY_1_TEMPLATE_PATH = os.path.join(
    MISC_1099_TEMPLATE_PATH, "copy_1_template.pdf"
)
MISC_1099_COPY_2_TEMPLATE_PATH = os.path.join(
    MISC_1099_TEMPLATE_PATH, "copy_2_template.pdf"
)
MISC_1099_COPY_B_TEMPLATE_PATH = os.path.join(
    MISC_1099_TEMPLATE_PATH, "copy_b_template.pdf"
)
MISC_1099_COPY_C_TEMPLATE_PATH = os.path.join(
    MISC_1099_TEMPLATE_PATH, "copy_c_template.pdf"
)
MISC_1099_TEMPLATES = [
...
def create_csv_record(
    profiles_obj, wt_obj, worker_type, us_workers, value_usd, b_ff=None
):
    ...
    if worker_type == BOUNTY:
        counter_party_gh_username = (
            b_ff.fulfiller_github_username
            if b_ff.fulfiller_github_username
            else NO_USERNAME
        )
        ...
        if counter_party:
            counter_party_gh_username = (
                counter_party.username if counter_party.username else NO_USERNAME
            )
        else:
...
def create_1099(form, recipient_path, template_path):
    ...
    pdfrw.PdfWriter().write(
        os.path.join(recipient_path, template_path[1]), template_pdf
    )
        ...
        for e in total_earning:
            ...
            # Tips
                ...
                try:
                    ...
                    continue
                record, funders[user]["us_workers"] = create_csv_record(
                    profiles_all, t_obj, TIP, funders[user]["us_workers"], e.value_usd
                )
                funders[user]["csv_record"].append(record)
            # Grants
                ...
                try:
                    ...
                    continue
                record, funders[user]["us_workers"] = create_csv_record(
                    profiles_all, g_obj, GRANT, funders[user]["us_workers"], e.value_usd
                )
                funders[user]["csv_record"].append(record)
            else:
                    ...
                    if recipient_profiles.exists():
                        ...
                        if usd_value > 600:
                            recipient_path = os.path.join(
                                misc_path, recipient_profile.username
                            )
                            ...
                            form_data[RECIPIENT][LOCATION] = get_profile_location(
                                recipient_profile
                            )[0]
