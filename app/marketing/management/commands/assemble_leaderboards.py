...
def bounty_to_location(bounty):
    ...
    fulfiller_usernames = list(
        bounty.fulfillments.filter(accepted=True).values_list(
...
def grant_to_location(grant):
    return profile_to_location(
        grant.subscription.contributor_profile.handle
    ) + profile_to_location(grant.subscription.grant.admin_profile.handle)
...
def grant_to_country(grant):
    return list(
        set(
            ele["country_name"]
            for ele in grant_to_location(grant)
            if ele and ele.get("country_name")
        )
    )
...
def grant_to_continent(grant):
    return list(
        set(
            ele["continent_name"]
            for ele in grant_to_location(grant)
            if ele and ele.get("continent_name")
        )
    )
...
def grant_to_city(grant):
    return list(
        set(ele["city"] for ele in grant_to_location(grant) if ele and ele.get("city"))
    )
...
def tip_to_country(tip):
    return list(
        set(
            ele["country_name"]
            for ele in tip_to_location(tip)
            if ele and ele.get("country_name")
        )
    )
...
def tip_to_city(tip):
    return list(
        set(ele["city"] for ele in tip_to_location(tip) if ele and ele.get("city"))
    )
...
def bounty_to_country(bounty):
    return list(
        set(
            ele["country_name"]
            for ele in bounty_to_location(bounty)
            if ele and ele.get("country_name")
        )
    )
...
def bounty_to_city(bounty):
    return list(
        set(
            ele["city"] for ele in bounty_to_location(bounty) if ele and ele.get("city")
        )
    )
...
def sum_bounty_helper(b, time, index_term, val_usd):
    fulfiller_index_terms = list(
        b.fulfillments.filter(accepted=True).values_list("profile__handle", flat=True)
    )
    ...
    if index_term.lower() in (k.lower() for k in b.keywords_list):
        is_github_org_name = Bounty.objects.filter(
            github_url__icontains=f"https://github.com/{index_term}"
        ).exists()
        is_github_repo_name = Bounty.objects.filter(
            github_url__icontains=f"/{index_term}/"
        ).exists()
...
def sum_grant_helper(gc, time, index_term, val_usd):
    ...
    add_element(f"{time}_{FULFILLED}", index_term, val_usd)
    if (
        gc.subscription.grant.admin_profile.handle.lower() == index_term
        and index_term not in IGNORE_EARNERS
    ):
        add_element(f"{time}_{EARNERS}", index_term, val_usd)
...
def do_leaderboard_feed():
    ...
    for _type in [PAYERS, EARNERS, ORGS]:
        ...
        lrs = LeaderboardRank.objects.active().filter(
            leaderboard=key, rank__lte=max_rank, product="all"
        )
        print(key, lrs.count())
        for lr in lrs:
            metadata = {
    ...
    for _type in [PAYERS, EARNERS, ORGS, CITIES, TOKENS]:
        ...
        lrs = (
            ...
            .filter(leaderboard=key, rank__lte=max_rank, product="all")
            .order_by("rank")[0:10]
        )
        ...
        for lr in lrs:
            profile_link = (
                f"<a href=/{lr.profile}>@{lr.profile}</a>"
                if _type not in [CITIES, TOKENS]
                else f"<strong>{lr.github_username}</strong>"
            )
        ...
        Activity.objects.create(
            profile=profile,
            activity_type="consolidated_leaderboard_rank",
            metadata=metadata,
        )
    ...
    for product in products:
        ...
        with transaction.atomic():
            ...
            for key, rankings in ranks.items():
                rank = 1
                for index_term, amount in sorted(
