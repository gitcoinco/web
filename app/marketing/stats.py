...
def chat_users():
    ...
    try:
        ...
        stats_request = chat_driver.system.get_analytics(
            {"name": "standard", "team_id": ""}
        )
        ...
        active_user_count = (
            hack_team_stats["active_member_count"]
            + core_team_stats["active_member_count"]
        )
...
def user_actions():
    ...
    action_types = UserAction.objects.distinct("action").values_list(
        "action", flat=True
    )
...
def firefox_ext_users():
    ...
    html_response = requests.get(url)
    soup = BeautifulSoup(html_response.text, "html.parser")
    eles = soup.findAll("div", {"class": "AddonMeta"})[0].findAll(
        "dt", {"class": "MetadataCard-title"}
    )
...
def medium_subscribers():
    ...
    num_users = data["payload"]["references"]["Collection"]["d414fce43ce1"]["metadata"][
        "followerCount"
    ]
    # print(num_users)
    Stat.objects.create(
...
def bounties_hourly_rate():
    ...
    for bounty in bounties:
        try:
            hours += (
                bounty.fulfillments.filter(accepted=True).first().fulfiller_hours_worked
            )
...
def bounties_by_status_and_keyword(created_before=timezone.now()):
    ...
    statuses = Bounty.objects.distinct("idx_status").values_list(
        "idx_status", flat=True
    )
    days_back = 9999
    created_after = created_before - timezone.timedelta(days=days_back)
    for status in statuses:
        for keyword in keywords:
            eligible_bounties = Bounty.objects.current().filter(
                network="mainnet",
                web3_created__gt=created_after,
                web3_created__lt=created_before,
            )
            if keyword:
                eligible_bounties = eligible_bounties.filter(
                    raw_data__icontains=keyword
                )
...
def joe_dominance_index(created_before=timezone.now()):
    ...
    joe_addresses = [
        "0x4331B095bC38Dc3bCE0A269682b5eBAefa252929".lower(),
        "0xe93d33CF8AaF56C64D23b5b248919EabD8c3c41E".lower(),
    ]  # kevin
    joe_addresses = joe_addresses + [
        "0x28e21609ca8542Ce5A363CBf339529204b043eDe".lower()
    ]  # eric
    joe_addresses = joe_addresses + [
        "0x60206c1F2B51Ac470cB0f71323474f7f9e4772e1".lower()
    ]  # vivek
    joe_addresses = joe_addresses + [
        "0x93d0deF1d76B510e2a7A6d01Cf18c54ec23f4253".lower()
    ]  # mark beacom
    joe_addresses = joe_addresses + [
        "0x58dC037f0A5c6C03D0f9477aea3198648CF0D263".lower()
    ]  # alisa
    joe_addresses = joe_addresses + [
        "0x5Df67Bdf139ABc95b0B3A5851875a73A5e722c0E".lower()
    ]  # alisa
    joe_addresses = joe_addresses + [
        "0x0A998a744351604887c70D2BA633d2e68021A8E1".lower()
    ]  # saptak
    joe_addresses = joe_addresses + [
        "0x5DA565AD870ee827608fC764f76ab8055B3E8474".lower()
    ]  # justin
    joe_addresses = joe_addresses + [
        "0x5cdb35fADB8262A3f88863254c870c2e6A848CcA".lower()
    ]  # aditya
    joe_addresses = joe_addresses + [
        "0x00de4b13153673bcae2616b67bf822500d325fc3".lower()
    ]  # kevin
    joe_addresses = joe_addresses + [
        "0xe317C793ebc9d4A3732cA66e5a8fC4ffc213B989".lower()
    ]  # dan
    ...
    for days in [7, 30, 90, 360]:
        ...
        for keyword in keywords:
            ...
            val_val = int(
                ...
                * sum(
                    [
                        (b.value_in_usdt_now if b.value_in_usdt_now else 0)
                        for b in joe_bounties
                    ]
                )
                / sum(
                    [
...
def avg_time_bounty_turnaround():
    ...
    for days in [7, 30, 90, 360]:
        ...
        turnaround_times = [
            b.turnaround_time_submitted
            for b in all_bounties
            if b.turnaround_time_submitted
        ]
        ...
        turnaround_times = [
            b.turnaround_time_accepted
            for b in all_bounties
            if b.turnaround_time_accepted
        ]
        val = int(statistics.median(turnaround_times) / 60 / 60)  # seconds to hours
        ...
        turnaround_times = [
            b.turnaround_time_started for b in all_bounties if b.turnaround_time_started
        ]
        val = int(statistics.median(turnaround_times) / 60 / 60)  # seconds to hours
...
def bounties_open():
    ...
    Stat.objects.create(
        ...
        val=(
            ...
            .filter(network="mainnet", idx_status="open")
            .count()
        ),
    )
...
def bounties_fulfilled():
    ...
    Stat.objects.create(
        ...
        val=(
            Bounty.objects.current()
            .filter(network="mainnet", idx_status="done")
            .count()
        ),
    )
...
def tips_received():
    ...
    Stat.objects.create(
        key="tips_received",
        val=(
            Tip.objects.filter(network="mainnet")
            .send_success()
            .receive_success()
            .count()
        ),
...
def subs_active():
    ...
    for key in email_keys:
        kwargs = {f"preferences__suppression_preferences__{key}": True}
