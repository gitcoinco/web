...
class Command(BaseCommand):
    ...
    def getTokenList(self):
        ...
        for transaction in r.json()["result"]:
            if int(transaction["balance"]) > 0:
                ...
                exchangeAddress = self.factoryContract.functions.getExchange(
                ...
                if exchangeAddress != "0x0000000000000000000000000000000000000000":
                    ...
                    logger.info(
                        "Token Name: "
                        + tokenList[address]["tokenName"]
                        + " and Token Symbol is: "
                        + tokenList[address]["tokenSymbol"]
                    )
    ...
    def sell_token(self, exchangeAddress, tokenSymbol):
        ...
        failure_count = (
            ...
            .exclude(transaction_result="success")
            .count()
        )
        if failure_count == 0:

            exchangeContract = self.web3.eth.contract(
                address=exchangeAddress, abi=self.exchangeABI
            )
            ...
            tokenContract = self.web3.eth.contract(
                address=tokenAddress, abi=self.tokenABI
            )
            walletBalance = tokenContract.functions.balanceOf(
                settings.FEE_ADDRESS
            ).call()
            ...
            numerator = (
                walletBalance
                * outputReserve
                * (1 - settings.UNISWAP_LIQUIDITY_FEE)
                * 1000
            )
            denominator = (
                inputReserve * 1000
                + walletBalance * (1 - settings.UNISWAP_LIQUIDITY_FEE) * 1000
            )
            outputAmount = numerator / denominator
            deadline = (
                self.web3.eth.getBlock("latest")["timestamp"]
                + settings.UNISWAP_TRADE_DEADLINE
            )
            logger.info(
                "Current token balance is : "
                + str(self.web3.fromWei(walletBalance, "ether"))
            )
            logger.info(
                "Amount of ETH to be bought is :"
                + str(self.web3.fromWei(outputAmount, "ether"))
            )
            logger.info(
                "Exchange rate is : " + str(outputAmount / walletBalance) + " ETH/token"
            )
            ...
            txn_dict = exchangeContract.functions.approve(
                settings.FEE_ADDRESS, self.web3.toWei(walletBalance, "wei")
            ).buildTransaction(
                {
                    "chainId": chain,
                    "gas": 300000,
                    "gasPrice": self.web3.toWei(
                        recommend_min_gas_price_to_confirm_in_time(
                            settings.UNISWAP_TRADE_DEADLINE / 60
                        ),
                        "gwei",
                    ),
                    "nonce": nonce,
            ...
            signed_txn = self.web3.eth.account.signTransaction(
                txn_dict, private_key=settings.FEE_ADDRESS_PRIVATE_KEY
            )
            ...
            nonce = self.web3.eth.getTransactionCount(settings.FEE_ADDRESS)
            txn_dict = exchangeContract.functions.tokenToEthSwapInput(
                self.web3.toWei(walletBalance, "wei"),
                self.web3.toWei(outputAmount * (1 - settings.SLIPPAGE), "wei"),
                deadline=deadline,
            ).buildTransaction(
                {
                    "chainId": chain,
            ...
            if tx_receipt["status"] == 1:
                ...
                transaction_record = CurrencyConversion.objects.create(
                    transaction_date=now(),
                    from_amount=walletBalance,
                    to_amount=outputAmount,
                    conversion_rate=outputAmount / walletBalance,
                    txid=self.web3.toHex(tx_receipt["transactionHash"]),
                    from_token_addr=tokenAddress,
                    from_token_symbol=tokenSymbol,
                    to_token_symbol="ETH",
                ...
                mail = Mail(
                    ...
                    "Failed fee conversion",
                    Email(settings.CONTACT_EMAIL),
                    Content("text/plain", tokenSymbol + " conversion to ETH failed"),
                )
            ...
            mail = Mail(
                ...
                "Token in Fee Wallet with previous failed fee conversion",
                Email(settings.CONTACT_EMAIL),
                Content(
                    "text/plain",
                    tokenSymbol
                    + " conversion to ETH failed previously so no conversion was attempted.",
                ),
            )
            response = self.sg.client.mail.send.post(request_body=mail.get())
    ...
    def handle(self, **options):
        ...
        self.web3 = Web3(
            Web3.HTTPProvider(
                settings.WEB3_HTTP_PROVIDER + "/v3/" + settings.INFURA_V3_PROJECT_ID
            )
        )
        ...
        with open(
            "feeswapper/management/commands/factoryABI.json", "r"
        ) as factoryABIfile:
            self.factoryABI = json.load(factoryABIfile)
        self.factoryContract = self.web3.eth.contract(
            address=self.factoryAddress, abi=self.factoryABI
        )
        ...
        with open(
            "feeswapper/management/commands/exchangeABI.json", "r"
        ) as exchangeABIfile:
