...
class feeSwapperTest(TestCase):
    ...
    @mock.patch(
        "feeswapper.management.commands.feeSwapper.requests.get", return_value=resp
    )
    @mock.patch(
        "feeswapper.management.commands.feeSwapper.Command.factoryContract",
        return_value="0x0000000000000000000000000000000000000001",
    )
    @mock.patch(
        "feeswapper.management.commands.feeSwapper.Command.web3", return_value="1"
    )
    def test_getTokenList(self, response, web3_response, toWei_response):
        ...
        assert (
            tokenList["0xda5b056cfb861282b4b59d29c9b395bcc238d29b"]["tokenSymbol"]
            == "BAT"
        )
    ...
    def test_getTokenList_with_zero_balance_token(
        self, response, web3_response, toWei_response
    ):
    ...
    @mock.patch(
        "feeswapper.management.commands.feeSwapper.Command.sg", return_value="1"
    )
    ...
    def test_sell_token_with_successful_trade(self, web3_response):
        ...
        assert (
            CurrencyConversion.objects.filter(from_token_symbol="BAT")[0].to_amount
            == 499.248873309965
        )
