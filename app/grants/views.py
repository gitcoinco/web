...
def get_stats(round_type):
    ...
    key_titles = [
        ...
        (
            "_admt1",
            "Estimated Matching Amount (in cents) / Twitter Followers",
            "-positive_round_contributor_count",
            "grants",
        ),
    ...
    for ele in key_titles:
        ...
        if ele[3] == "grants":
            top_grants = (
                Grant.objects.filter(active=True, grant_type__name=round_type)
                .exclude(pk=86)
                .order_by(order_by)[0:50]
            )
        ...
        if ele[3] == "profile":
            ...
            keys = list(
                Stat.objects.filter(
                    created_on__gt=created_on, key__startswith=startswith
                ).values_list("key", flat=True)
            )
        charts.append(
            {
                "title": f"{title} Over Time ({round_type.title()} Round)",
                "db": Stat.objects.filter(
                    key__in=keys,
                    created_on__gt=created_on,
                    created_on__minute__lt=minute,
                ),
            }
        )
    results = []
    ...
    for chart in charts:
        try:
            ...
            cht = PivotChart(
                datasource=rankdata,
                series_options=[
                    {"options": {"type": "line", "stacking": False}, "terms": ["val"]}
                ],
                chart_options={
...
def get_fund_reward(request, grant):
    token = (
        Token.objects.filter(
            ...
            owner_address__iexact="0x6239FF1040E412491557a7a02b2CBcC5aE85dc8F",
        )
        .order_by("?")
        .first()
    )
    ...
    request.session[
        "msg_html"
    ] = f"You have received a new {token.ui_name} for your contribution to {grant.title}"
...
def get_keywords():
    ...
    return json.dumps(
        [
            str(key)
            for key in Keyword.objects.all().cache().values_list("keyword", flat=True)
        ]
    )
...
def grants_addr_as_json(request):
    _grants = Grant.objects.filter(network="mainnet", hidden=False)
...
def get_collections(
    ...
    if only_contributions:
        contributions = user.profile.grant_contributor.filter(
            subscription_contribution__success=True
        ).values("grant_id")
    ...
    if user.is_authenticated and user.profile.handle == keyword:
        _collections = _collections.filter(
            Q(profile=user.profile) | Q(curators=user.profile)
        )
    else:
...
def get_grants(request):
    ...
    if grant_type == "collections":
        ...
        collections = GrantCollection.objects.filter(
            grants__in=Subquery(_grants.values("id"))
        ).distinct()[:12]
    ...
    if request.user.is_authenticated:
        contributions = Contribution.objects.filter(
            id__in=request.user.profile.grant_contributor.filter(
                subscription_contribution__success=True
            ).values("subscription_contribution__id")
    ...
    return JsonResponse(
        {
            "grant_types": get_grant_clr_types(clr_round, _grants, network)
            if clr_round
            else get_grant_type_cache(network),
...
def build_grants_by_type(
    ...
    if omit_my_grants and profile:
        grants_id = list(profile.grant_teams.all().values_list("pk", flat=True)) + list(
            profile.grant_admin.all().values_list("pk", flat=True)
        )
        ...
        contributions = profile.grant_contributor.filter(
            subscription_contribution__success=True
        ).values("grant_id")
...
def get_grant_clr_types(clr_round, active_grants=None, network="mainnet"):
    ...
    for _grant_type in _grant_types:
        count = (
            active_grants.filter(grant_type=_grant_type, network=network).count()
            if active_grants
            else 0
        )
...
def grants_by_grant_type(request, grant_type):
    ...
    _grants = None
    try:
        _grants = build_grants_by_type(
            request, grant_type, sort, network, keyword, state, category
        )
    except Exception as e:
        print(e)
        return redirect("/grants")

    all_grants_count = Grant.objects.filter(network=network, hidden=False).count()

    partners = (
        MatchPledge.objects.filter(active=True, pledge_type=grant_type)
        if grant_type
        else MatchPledge.objects.filter(active=True)
    )
    ...
    if len(pks):
        increment_view_count.delay(
            pks, grants[0].content_type, request.user.id, "index"
        )
    ...
    collections = []
    if request.user.is_authenticated:
        grants_following = Favorite.objects.filter(
            user=request.user, activity=None
        ).count()
        # KO 9/10/2020
    ...
    if active_rounds:
        for active_round in active_rounds:
            ...
            total_clr_pot = (
                total_clr_pot + clr_round_amount if total_clr_pot else clr_round_amount
            )
    ...
    if total_clr_pot:
        ...
        live_now = (
            f"❇️ LIVE NOW! Up to ${int_total_clr_pot} Matching Funding on Gitcoin Grants"
            if total_clr_pot > 0
            else ""
        )
    ...
    params = {
        ...
        "avatar_url": request.build_absolute_uri(
            static("v2/images/twitter_cards/grants7.png")
        ),
        ...
        "styles": {"bg": bg, "bg_size": bg_size, "bg_color": bg_color},
        "bg": bg,
...
def get_grant_sybil_profile(
    grant_id=None, days_back=None, grant_type=None, index_on=None
):
    grant_id_sql = f"= {grant_id}" if grant_id else "IS NOT NULL"
    days_back_sql = (
        f"grants_subscription.created_on > now() - interval '{days_back} hours'"
        if days_back
        else "true"
    )
    grant_type_sql = f"grant_type_id = '{grant_type.id}'" if grant_type else "true"
    order_sql = (
        f"ORDER BY {index_on} ASC"
        if index_on != "grants_contribution.originated_address"
        else "ORDER BY number_contriibutors DESC"
    )
    having_sql = (
        f""
        if index_on != "grants_contribution.originated_address"
        else "HAVING count(distinct grants_subscription.contributor_profile_id) >= 2"
    )
    query = f"""
        SELECT
            DISTINCT {index_on},
...
def grant_details(request, grant_id, grant_slug):
    ...
    try:
        ...
        increment_view_count.delay(
            [grant.pk], grant.content_type, request.user.id, "individual"
        )
        ...
        if tab == "sybil_profile" and request.user.is_staff:
            items = [
                ...
                "dashboard_profile.sms_verification",
            ...
            for item in items:
                ...
                sybil_profiles += [
                    [
                        f"THIS {title} Summary Last 90 days",
                        get_grant_sybil_profile(grant.pk, 90 * 24, index_on=item),
                    ],
                    [
                        f"{grant.grant_type.name} {title} Summary Last 90 Days",
                        get_grant_sybil_profile(
                            None, 90 * 24, grant.grant_type, index_on=item
                        ),
                    ],
                    [
                        f"All {title} Summary Last 90 Days",
                        get_grant_sybil_profile(None, 90 * 24, None, index_on=item),
                    ],
                ]
        _contributions = Contribution.objects.filter(
            subscription__grant=grant, subscription__is_postive_vote=True
        ...
        user_subscription = grant.subscriptions.filter(
            contributor_profile=profile, active=True
        ).first()
        user_non_errored_subscription = grant.subscriptions.filter(
            contributor_profile=profile, active=True, error=False
        ).first()
    ...
    is_admin = (
        (grant.admin_profile.id == profile.id)
        if profile and grant.admin_profile
        else False
    )
    if is_admin:
    ...
    if request.method == "POST" and (is_team_member or request.user.is_staff):
        ...
        if "grant_cancel_tx_id" in request.POST:
            ...
            if (
                "edit-twitter_account" in request.POST
                and request.POST.get("edit-twitter_account") != grant.twitter_handle_1
            ):
    ...
    is_unsubscribed_from_updates_from_this_grant = (
        ...
        and request.user.profile.pk in grant.metadata.get(key, [])
    )
    if request.GET.get("unsubscribe") and request.user.is_authenticated:
        ...
        grant.save()
        messages.info(
            request, _("You have been unsubscribed from the updates from this grant.")
        )
    ...
    params = {
        ...
        "options": [
            (
                f"Email Grant Funders ({grant.contributor_count})",
                "bullhorn",
    ...
    if tab == "stats":
        ...
        params["stats_history"] = grant.stats.filter(
...
def grant_new_whitelabel(request):
    ...
    params = {
        ...
        "card_player_thumb_override": request.build_absolute_uri(
...
def grant_new(request):
    ...
    if request.method == "POST":
        if "title" in request.POST:
            ...
            grant_kwargs = {
                ...
                "contract_owner_address": request.POST.get(
            ...
            messages.info(
                request,
                _(
                    "Thank you for posting this Grant.  Share the Grant URL with your friends/followers to raise your first tokens."
                ),
            )
            grant.save()
    ...
    params = {
        ...
        "title": _("New Grant"),
        "card_desc": _(
            "Provide sustainable funding for Open Source with Gitcoin Grants"
        ),
        "profile": profile,
        "grant": {},
        "keywords": get_keywords(),
...
def grant_fund(request, grant_id, grant_slug):
    ...
    if not grant.configured_to_receieve_funding:
        messages.info(
            request,
            _(
                "This grant is not configured to accept funding at this time.  Please contact founders@gitcoin.co if you believe this message is in error!"
            ),
        )
        logger.error(
    ...
    for (index, grant_id) in enumerate(grant_ids_list):
            ...
            failures.append(
                {
                    ...
                    "text": _("This Grant cannot be funded"),
                    "subtext": _(
                        "Grant team members cannot contribute to their own grant."
                    ),
                    "success": False,
                }
            )
            ...
            failures.append(
                {
                    ...
                    "grant": grant_id,
                    "text": _(
                        "You already have an active subscription for this grant."
                    ),
                    "success": False,
                }
            )
            ...
            failures.append(
                {
                    ...
                    "grant": grant_id,
                    "text": _(
                        "This Grant is not configured to accept funding at this time."
                    ),
                    "subtext": _(
                        "Grant is not properly configured for funding.  Please set grant.contract_address on this grant, or contact founders@gitcoin.co if you believe this message is in error!"
                    ),
                    "success": False,
                }
            )
        ...
        try:
            ...
            payload = {
                ...
                "gitcoin_donation_address": request.POST.get(
                ...
                "splitter_contract_address": request.POST.get(
                ...
                "amount_per_period": request.POST.get("amount_per_period").split(",")[
                    index
                ],
                "comment": request.POST.get("comment").split(",")[index],
                "confirmed": request.POST.get("confirmed").split(",")[index],
                "contract_address": request.POST.get("contract_address").split(",")[
                    index
                ],
                "contract_version": request.POST.get("contract_version").split(",")[
                    index
                ],
                "denomination": request.POST.get("denomination").split(",")[index],
                "gitcoin-grant-input-amount": request.POST.get(
                    "gitcoin-grant-input-amount"
                ).split(",")[index],
                "grant_id": request.POST.get("grant_id").split(",")[index],
                "sub_new_approve_tx_id": request.POST.get(
        ...
        successes.append(
            {
                ...
                "grant": grant_id,
                "text": _(
                    "Funding for this grant was successfully processed and saved."
                ),
                "success": True,
            }
        )
...
def zksync_set_interrupt_status(request):
    ...
    try:
        ...
        JSONStore.objects.create(
            key=user_address, view="zksync_checkout", data=deposit_tx_hash
        )

    return JsonResponse({"success": True, "deposit_tx_hash": deposit_tx_hash})
...
def get_replaced_tx(request):
    ...
    if "Replaced" in p.text:  # check if it's a replaced tx
        ...
        return JsonResponse({"success": True, "tx_hash": q.text})
    else:
        return JsonResponse({"success": True, "tx_hash": tx_hash})
...
def subscription_cancel(request, grant_id, grant_slug, subscription_id):
    ...
    if request.method == "POST" and (
        ...
        record_subscription_activity_helper(
            "killed_grant_contribution", subscription, profile
        )

        value_usdt = subscription.get_converted_amount()
        if value_usdt:
            grant.monthly_amount_subscribed -= (
                subscription.get_converted_monthly_amount()
            )
        ...
        messages.info(
            request,
            _(
                "Your subscription has been canceled. We hope you continue to support other open source projects!"
            ),
        )
        return redirect(reverse("grants:details", args=(grant.pk, grant.slug)))
...
def grants_bulk_add(request, grant_str):
    ...
    title = (
        f"{prefix}{len(grants)} Grants in Shared Cart {by_whom} : Viewed {views} times"
    )
    ...
    response = TemplateResponse(
        request, "grants/bulk_add_to_cart.html", context=context
    )
    return response
...
def leaderboard(request):
    ...
    return redirect(
        "https://gitcoin.co/leaderboard/payers?cadence=quarterly&keyword=all&product=grants"
    )
...
def record_grant_activity_helper(
    activity_type, grant, profile, amount=None, token=None
):
...
def new_matching_partner(request):
    ...
    if not request.user.is_authenticated:
        messages.info(request, _("Please login to submit this form."))
    elif request.POST:
...
def invoice(request, contribution_pk):
    p_contribution = Contribution.objects.prefetch_related(
        "subscription", "subscription__grant"
    )
    ...
    has_view_privs = (
        ...
        or request.user.profile == contribution.subscription.contributor_profile
    )
...
def grant_activity(request, grant_id=None):
    ...
    _ca = CartActivity.objects.create(
        grant=grant,
        profile=request.user.profile,
        action=action,
    ...
    for ca in CartActivity.objects.filter(
        profile=request.user.profile, latest=True, pk__lt=_ca.pk
    ):
        ca.latest = False
        ca.save()
...
def grants_clr(request):
    ...
    response = {"status": 200, "grants": grants}
    return JsonResponse(response)
...
def verify_grant(request, grant_id):
    ...
    if grant.twitter_verified:
        return JsonResponse({"ok": True, "msg": "Grant was verified previously"})
    ...
    try:
        ...
        last_tweet = api.user_timeline(
            screen_name=grant.twitter_handle_1,
            count=1,
            tweet_mode="extended",
            include_rts=False,
...
def save_collection(request):
    ...
    if len(grant_ids) == 0:
        return JsonResponse(
            {"ok": False, "msg": "We can't create empty collections"}, status=422
        )
    ...
    if collection_id:
        ...
        grant_ids = grant_ids + list(
            collection.grants.all().values_list("id", flat=True)
        )
    else:
        kwargs = {
            "title": title,
...
def get_collection(request, collection_id):
    ...
    curators = [
        {"url": curator.url, "handle": curator.handle, "avatar_url": curator.avatar_url}
...
def remove_grant_from_collection(request, collection_id):
    ...
    grants = [
        grant.repr(request.user, request.build_absolute_uri)
        for grant in collection.grants.all()
    ]
