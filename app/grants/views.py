...
def grant_details(request, grant_id, grant_slug):
    ...
    try:
        ...
        is_within_payout_period_for_most_recent_round = timezone.now() < timezone.datetime(2021, 4, 15, 12, 0).replace(tzinfo=pytz.utc)
        is_staff = request.user.is_authenticated and request.user.is_staff

        # Check if this grant needs to complete KYC before claiming match funds
        clr_matches = grant.clr_matches.filter(clr__round_num=9, clr__customer_name='ethereum')
