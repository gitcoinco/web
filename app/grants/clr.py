        ...
        for c in g.get("contributions"):
            ...
            if profile_id:
                val = (
                    ...
                    + [c.get("id")]
                    + [verification_status]
                    + [c.get("sum_of_each_profiles_contributions")]
                )
        ...
        for k1, v1 in contribz.items():
            ...
            for k2, v2 in contribz.items():
                ...
                if k2 > k1 and all(i in sms_verified_list for i in [k2, k1]):
                    tot += ((v1 * v2) ** 0.5) / (
                        pair_totals[k1][k2] / (v_threshold * 1.05) + 1
                    )
                # both bright
                elif k2 > k1 and all(i in bright_verified_list for i in [k2, k1]):
                    tot += ((v1 * v2) ** 0.5) / (
                        pair_totals[k1][k2] / (v_threshold * 1.2) + 1
                    )
                # both none
                elif k2 > k1 and not any(
                    i in sms_verified_list + bright_verified_list for i in [k2, k1]
                ):
                    tot += ((v1 * v2) ** 0.5) / (pair_totals[k1][k2] / uv_threshold + 1)
                # one bright or sms, one none
                elif k2 > k1 and (
                    (
                        (k2 in sms_verified_list + bright_verified_list)
                        and (k1 not in sms_verified_list + bright_verified_list)
                    )
                    or (
                        (k1 in sms_verified_list + bright_verified_list)
                        and (k2 not in sms_verified_list + bright_verified_list)
                    )
                ):
                    tot += ((v1 * v2) ** 0.5) / (pair_totals[k1][k2] / uv_threshold + 1)
                # one bright, one sms
                    ...
                    tot += ((v1 * v2) ** 0.5) / (
                        pair_totals[k1][k2] / (v_threshold * 1.125) + 1
                    )
...
def run_clr_calcs(grant_contribs_curr, v_threshold, uv_threshold, total_pot):
    ...
    # clr calcluation
    totals = calculate_clr(
        curr_agg, ptots, sms_list, bright_list, v_threshold, uv_threshold, total_pot
    )

    return totals


def calculate_clr_for_donation(
    ...
    grants_clr = run_clr_calcs(
        _grant_contributions_curr, v_threshold, uv_threshold, total_pot
    )
...
def fetch_data(clr_round, network="mainnet"):
    ...
    contributions = Contribution.objects.prefetch_related("subscription").filter(
        match=True,
    ...
    phantom_funding_profiles = PhantomFunding.objects.filter(
        created_on__gte=clr_start_date, created_on__lte=clr_end_date
    )
...
def populate_data_for_clr(grants, contributions, phantom_funding_profiles, clr_round):
    ...
    for grant in grants:
        ...
        # contributions
        contribs = copy.deepcopy(contributions).filter(
            subscription__grant_id=grant.id,
        ...
        # phantom funding
        grant_phantom_funding_contributions = phantom_funding_profiles.filter(
            grant_id=grant.id,
            created_on__gte=clr_start_date,
            created_on__lte=clr_end_date,
        )
        ...
        sms_verified_contribution_ids = [
            ele.pk for ele in contribs if ele.profile_for_clr.sms_verification
        ]
        sms_verified_phantom_funding_contribution_ids = [
            ele.profile_id
            for ele in grant_phantom_funding_contributions
            if ele.profile.sms_verification
        ]
        sms_verified_profile = list(
            set(
                sms_verified_contribution_ids
                + sms_verified_phantom_funding_contribution_ids
            )
        ...
        brightid_verified_contribution_ids = [
            ele.pk for ele in contribs if ele.profile_for_clr.is_brightid_verified
        ]
        brightid_verified_phantom_funding_contribution_ids = [
            ele.profile_id
            for ele in grant_phantom_funding_contributions
            if ele.profile.is_brightid_verified
        ]
        brightid_verified_profile = list(
            set(
                brightid_verified_contribution_ids
                + brightid_verified_phantom_funding_contribution_ids
            )
        ...
        # combine
        contributing_profile_ids = list(
            set(
                [c.identity_identifier(mechanism) for c in contribs]
                + [p.profile_id for p in grant_phantom_funding_contributions]
            )
        )
        ...
        if len(contributing_profile_ids) > 0:
            for profile_id in contributing_profile_ids:
                ...
                sum_of_each_profiles_contributions = float(
                    sum(
                        [
                            c.subscription.amount_per_period_usdt
                            * clr_round.contribution_multiplier
                            for c in profile_contributions
                            if c.subscription.amount_per_period_usdt
                        ]
                    )
                )
                phantom_funding = grant_phantom_funding_contributions.filter(
                    profile_id=profile_id
                )
                if phantom_funding.exists():
                    sum_of_each_profiles_contributions = (
                        sum_of_each_profiles_contributions
                        + phantom_funding.first().value
                    )
                ...
                summed_contributions.append(
                    {
                        ...
                        "is_sms_verified": True
                        if profile_id in sms_verified_profile
                        else False,
                        "is_brightid_verified": True
                        if profile_id in brightid_verified_profile
                        else False,
                    }
                )

            contrib_data_list.append(
                {"id": grant_id, "contributions": summed_contributions}
            )
    ...
    for grant in grants:
        ...
        if save_to_db:
            ...
            can_estimate = (
                True
                if base
                or _grant.clr_prediction_curve[1][1]
                or _grant.clr_prediction_curve[2][1]
                or _grant.clr_prediction_curve[3][1]
                else False
            )

            if can_estimate:
                _grant.clr_prediction_curve = [
                    [ele[0], ele[1], ele[1] - base]
                    for ele in _grant.clr_prediction_curve
                ]
            else:
            ...
            try:
                if _grant.clr_prediction_curve[0][1]:
                    ...
                    max_twitter_followers = max(
                        _grant.twitter_handle_1_follower_count,
                        _grant.twitter_handle_2_follower_count,
                    )
                    if max_twitter_followers:
                        Stat.objects.create(
                            ...
                            val=int(
                                100
                                * _grant.clr_prediction_curve[0][1]
                                / max_twitter_followers
                            ),
                        )
        ...
        debug_output.append(
            {
                "grant": grant.id,
                "clr_prediction_curve": (potential_donations, potential_clr),
                "grants_clr": grants_clr,
            }
        )
