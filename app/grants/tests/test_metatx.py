...
class MetatxTest(TestCase):
    def setUp(self):
        self.monthly_subscription = Subscription(
            real_period_seconds=31 * 24 * 60 * 60, network="test", grant=Grant()
        )
        self.monthly_subscription.get_subscription_hash_arguments = lambda: {
            "periodSeconds": self.monthly_subscription.real_period_seconds,
            "gasPrice": 0,
        }

        self.daily_subscription_free = Subscription(
            real_period_seconds=24 * 60 * 60, network="test", grant=Grant()
        )
        self.daily_subscription_free.get_subscription_hash_arguments = lambda: {
            "periodSeconds": self.daily_subscription_free.real_period_seconds,
            "gasPrice": 0,
        }

        self.daily_subscription_paid = Subscription(
            real_period_seconds=24 * 60 * 60, network="test", grant=Grant()
        )
        self.daily_subscription_paid.get_subscription_hash_arguments = lambda: {
            "periodSeconds": self.daily_subscription_paid.real_period_seconds,
            "gasPrice": 1000000,
        }

        subscriptions = [
            self.monthly_subscription,
            self.daily_subscription_free,
            self.daily_subscription_paid,
        ]
