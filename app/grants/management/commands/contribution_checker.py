...
tx_list_url = (
    "http://api.etherscan.io/api?module=account&action=txlist&address={}&startblock=0&endblock=99999999&sort=asc&apikey="
    + api_key
)
...
def verify_tx(contribution):
    ...
    for each_tx in r_list_data["result"]:
        ...
        if (
            contribution.created_on <= tx_datetime
            and tx_datetime <= contribution.created_on + dt.timedelta(minutes=10)
        ):
            if (
                each_tx["to"].lower() == splitter_address.lower()
                and each_tx["isError"] == "1"
            ):
...
class Command(BaseCommand):
    ...
    help = (
        "checks for approval transactions followed up by failed splitter transactions"
    )
