...
def update_grant_metadata(self, grant_id, retry: bool = True) -> None:
    ...
    instance.positive_round_contributor_count = instance.get_contributor_count(
        round_start_date, True
    )
    instance.negative_round_contributor_count = instance.get_contributor_count(
        round_start_date, False
    )
    ...
    for subscription in instance.subscriptions.all():
        ...
        if subscription.num_tx_processed <= subscription.num_tx_approved and value_usdt:
            if subscription.num_tx_approved != 1:
                instance.monthly_amount_subscribed += (
    ...
    instance.amount_received_with_phantom_funds = Decimal(
        round(instance.get_amount_received_with_phantom_funds(), 2)
    )
    instance.sybil_score = (
        instance.sybil_score / instance.positive_round_contributor_count
        if instance.positive_round_contributor_count
        else -1
    )
    max_sybil_score = 5
    ...
    try:
        ...
        instance.weighted_risk_score = float(ss ** 2) * float(
            math.sqrt(float(instance.clr_prediction_curve[0][1]))
        )
...
@app.shared_task(bind=True, max_retries=1)
def process_grant_contribution(
    self, grant_id, grant_slug, profile_id, package, retry: bool = True
) -> None:
    """
    :param self:
    :param grant_id:
        ...
        if "comment" in package:
            ...
            if comment and activity:
                ...
                comment = Comment.objects.create(
                    profile=_profile, activity=activity, comment=comment
                )
