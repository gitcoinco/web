...
class GrantType(SuperModel):
    ...
    categories = models.ManyToManyField(
        GrantCategory, help_text="Grant Categories associated with Grant Type"
    )
    ...
    def active_clrs(self):
        return GrantCLR.objects.filter(
            is_active=True, grant_filters__grant_type=str(self.pk)
        )
...
class GrantCLR(SuperModel):
    ...
    is_active = models.BooleanField(
        default=False, db_index=True, help_text="Is CLR Round currently active"
    )
    ...
    verified_threshold = models.DecimalField(
        help_text="Verfied CLR Threshold", default=25.0, decimal_places=2, max_digits=5
    )
    unverified_threshold = models.DecimalField(
        help_text="Unverified CLR Threshold",
        default=5.0,
        decimal_places=2,
        max_digits=5,
    )
    total_pot = models.DecimalField(
        help_text="CLR Pot", default=0, decimal_places=2, max_digits=10
    )
...
class Grant(SuperModel):
    ...
    grant_type = models.ForeignKey(
        GrantType, on_delete=models.CASCADE, null=True, help_text="Grant Type"
    )
    title = models.CharField(
        default="", max_length=255, help_text=_("The title of the Grant.")
    )
    slug = AutoSlugField(populate_from="title")
    description = models.TextField(
        default="", blank=True, help_text=_("The description of the Grant.")
    )
    description_rich = models.TextField(
    ...
    github_project_url = models.URLField(
        blank=True, help_text=_("Grant Github Project URL")
    )
    ...
    contract_owner_address = models.CharField(
        ...
        default="0x0",
        help_text=_(
            "The wallet address that owns the subscription contract and is able to call endContract()"
        ),
    )
    ...
    metadata = JSONField(
        ...
        blank=True,
        help_text=_(
            "The Grant metadata. Includes creation and last synced block numbers."
        ),
    )
    ...
    backup_clr_prediction_curve = ArrayField(
        ...
        default=list,
        help_text=_(
            "backup 5 point curve to predict CLR donations - used to store a secondary backup of the clr prediction curve, in the case a new identity mechanism is used"
        ),
    )
    activeSubscriptions = ArrayField(
    ...
    positive_round_contributor_count = models.PositiveIntegerField(
        blank=True, default=0
    )
    negative_round_contributor_count = models.PositiveIntegerField(
        blank=True, default=0
    )
    ...
    twitter_handle_1 = models.CharField(
        default="", max_length=255, help_text=_("Grants twitter handle"), blank=True
    )
    twitter_handle_2 = models.CharField(
        default="", max_length=255, help_text=_("Grants twitter handle"), blank=True
    )
    ...
    funding_info = models.CharField(
        default="",
        blank=True,
        null=True,
        max_length=255,
        help_text=_("Is this grant VC funded?"),
    )
    ...
    in_active_clrs = models.ManyToManyField(
        GrantCLR, help_text="Active Grants CLR Round"
    )
    is_clr_active = models.BooleanField(
        default=False, help_text=_("CLR Round active or not? (auto computed)")
    )
    clr_round_num = models.CharField(
        default="",
        max_length=255,
        help_text=_("the CLR round number thats active"),
        blank=True,
    )

    twitter_verified = models.BooleanField(
        default=False, help_text="The owner grant has verified the twitter account"
    )
    twitter_verified_by = models.ForeignKey(
        "dashboard.Profile",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        help_text="Team member who verified this grant",
    )
    twitter_verified_at = models.DateTimeField(
        blank=True,
        null=True,
        help_text="At what time and date what verified this grant",
    )
    ...
    def updateActiveSubscriptions(self):
        ...
        handles = []
        for handle in (
            Subscription.objects.filter(grant=self, active=True, is_postive_vote=True)
            .distinct("contributor_profile")
            .values_list("contributor_profile__handle", flat=True)
        ):
    ...
    def contributions(self):
        ...
        for subscription in self.subscriptions.all():
            pks += list(
                subscription.subscription_contribution.values_list("pk", flat=True)
            )
    ...
    def get_contributor_count(self, since=None, is_postive_vote=True):
        ...
        for sub in self.subscriptions.filter(is_postive_vote=is_postive_vote):
            for contrib in sub.subscription_contribution.filter(
                success=True, created_on__gt=since
            ):
    ...
    def history_by_month(self):
        ...
        for sub in subs:
            ...
            for contrib in contribs:
                ...
                quarter = math.ceil(int(contrib.created_on.strftime("%m")) / 3.0)
                key = f"{year}/Q{quarter}"
                subkey = "One-Time"
                if (
                    contrib.subscription.contributor_profile.handle
                    in CLR_PAYOUT_HANDLES
                ):
                    subkey = "CLR"
        ...
        for key, val in sorted(
            month_to_contribution_numbers.items(), key=lambda kv: (kv[0])
        ):
    ...
    def url(self):
        ...
        return reverse(
            "grants:details", kwargs={"grant_id": self.pk, "grant_slug": slug}
        )
    ...
    def contract(self):
        ...
        grant_contract = web3.eth.contract(
            Web3.toChecksumAddress(self.contract_address), abi=self.abi
        )
    ...
    def cart_payload(self):
        return {
            ...
            "grant_logo": self.logo.url
            if self.logo and self.logo.url
            else f"v2/images/grants/logos/{self.id % 3}.png",
    ...
    def repr(self, user, build_absolute_uri):
        return {
            ...
            if self.logo and self.logo.url
                ...
                static(f"v2/images/grants/logos/{self.id % 3}.png")
            ),
            ...
            "is_on_team": is_grant_team_member(self, user.profile)
            if user.is_authenticated
            else False,
            "clr_prediction_curve": self.clr_prediction_curve,
            "last_clr_calc_date": naturaltime(self.last_clr_calc_date)
            if self.last_clr_calc_date
            else None,
            "safe_next_clr_calc_date": naturaltime(self.safe_next_clr_calc_date)
            if self.safe_next_clr_calc_date
            else None,
...
class Subscription(SuperModel):
    ...
    TENANT = [("ETH", "ETH"), ("ZCASH", "ZCASH")]

    active = models.BooleanField(
        default=True,
        db_index=True,
        help_text=_("Whether or not the Subscription is active."),
    )
    error = models.BooleanField(
        default=False, help_text=_("Whether or not the Subscription is erroring out.")
    )
    subminer_comments = models.TextField(
        default="", blank=True, help_text=_("Comments left by the subminer.")
    )
    comments = models.TextField(
        default="", blank=True, help_text=_("Comments left by subscriber.")
    )
    ...
    is_postive_vote = models.BooleanField(
        default=True,
        db_index=True,
        help_text=_("Whether this is positive or negative vote"),
    )
    split_tx_confirmed = models.BooleanField(
        default=False, help_text=_("Whether or not the split tx succeeded.")
    )
    ...
    num_tx_processed = models.DecimalField(
        ...
        max_digits=50,
        help_text=_(
            "The number of transactoins processed by the subminer for the Subscription."
        ),
    )
    network = models.CharField(
        max_length=8,
    ...
    tenant = models.CharField(
        ...
        default="ETH",
        choices=TENANT,
        help_text="specific tenant in which contribution is made",
    )
    ...
    def amount_per_period_minus_gas_price(self):
        amount = float(self.amount_per_period) - float(
            self.amount_per_period_to_gitcoin
        )
        return amount
    ...
    def amount_per_period_to_gitcoin(self):
        ...
        if self.created_on > timezone.datetime(2020, 6, 16, 15, 0).replace(
            tzinfo=pytz.utc
        ):
    ...
    def get_debug_info(self):
        ...
        try:
            ...
            token_contract = web3.eth.contract(
                Web3.toChecksumAddress(self.token_address), abi=erc20_abi
            )
            ...
            allowance = token_contract.functions.allowance(
                Web3.toChecksumAddress(self.contributor_address),
                Web3.toChecksumAddress(self.grant.contract_address),
            ).call()
...
"""
    ...
    def get_is_ready_to_be_processed_from_db(self):
        ...
        if not self.subscription_contribution.exists():
            return True
        return (
            self.next_contribution_date < timezone.now()
            and self.num_tx_processed < self.num_tx_approved
        )
    ...
    def helper_tx_dict(self, minutes_to_confirm_within=1):
        ...
        return {
            ...
            "value": 0,
            "gasPrice": int(
                recommend_min_gas_price_to_confirm_in_time(minutes_to_confirm_within)
                * 10 ** 9
            ),
            "gas": 204066,
        }
    ...
    def get_subscription_signer_from_web3(self):
        ...
        _hash = self.get_hash_from_web3()
        return self.grant.contract.functions.getSubscriptionSigner(
            _hash, self.contributor_signature
        ).call()
    ...
    def get_converted_amount(self, ignore_gitcoin_fee=False, only_gitcoin_fee=False):
        ...
        try:
            ...
            try:
                return Decimal(convert_amount(amount, self.token_symbol, "USDT"))
    ...
    def get_converted_monthly_amount(self, ignore_gitcoin_fee=False):
        converted_amount = (
            self.get_converted_amount(ignore_gitcoin_fee=ignore_gitcoin_fee) or 0
        )

        total_sub_seconds = Decimal(self.real_period_seconds) * Decimal(
            self.num_tx_approved
        )
        ...
        if total_sub_seconds < 2592000:
            ...
            result = Decimal(
                converted_amount
                * (Decimal(2592000) / Decimal(self.real_period_seconds))
            )
...
class Flag(SuperModel):
    ...
    comments_admin = models.TextField(
        default="", blank=True, help_text=_("The comments of an admin.")
    )
    ...
    def post_flag(self):
        ...
        activity = Activity.objects.create(
            profile=profile, activity_type="flagged_grant", grant=self.grant
        )
...
class Donation(SuperModel):
    ...
    token_amount_usdt = models.DecimalField(
        ...
        max_digits=50,
        help_text=_(
            "The donation amount converted to USDT/DAI at the moment of donation."
        ),
    )
...
class Contribution(SuperModel):
    ...
    tx_cleared = models.BooleanField(
        default=False, help_text=_("Whether or not tx cleared.")
    )
    tx_override = models.BooleanField(
        default=False,
        help_text=_(
            "Whether or not the tx success and tx_cleared have been manually overridden. If this setting is True, update_tx_status will not change this object."
        ),
    )
    ...
    match = models.BooleanField(
        default=True, help_text=_("Whether or not this contribution should be matched.")
    )
    ...
    validator_passed = models.BooleanField(
        default=False, help_text=_("Whether or not the backend validator passed.")
    )
    ...
    def is_first_in_sequence(self):
        ...
        other_contributions_after_this_one = Contribution.objects.filter(
            subscription=self.subscription, created_on__lt=self.created_on
        )
    ...
    def update_tx_status(self):
        ...
        try:
            ...
            if self.tx_id:
                tx_status, _ = get_tx_status(
                    self.tx_id, self.subscription.network, self.created_on
                )
            ...
            PROVIDER = (
                "wss://" + network + ".infura.io/ws/v3/" + settings.INFURA_V3_PROJECT_ID
            )
            w3 = Web3(Web3.WebsocketProvider(PROVIDER))

            # Get case number
            is_split_tx_id_address = (
                len(self.split_tx_id) == 42 and self.split_tx_id[0:2] == "0x"
            )
            if is_split_tx_id_address:
                ...
                split_tx_status, _ = get_tx_status(
                    self.split_tx_id, self.subscription.network, self.created_on
                )
                ...
                zksync_contract_addr = (
                    zksync_rinkeby_addr if network == "rinkeby" else zksync_mainnet_addr
                )
                batch_zksync_deposit_contract_addr = (
                    "0x9D37F793E5eD4EbD66d62D505684CD9f756504F6".lower()
                )
                zkSync_recipients = [
                    zksync_contract_addr.lower(),
                    batch_zksync_deposit_contract_addr.lower(),
                ]
            ...
            if case_number == 1:
                ...
                try:
                    ...
                    if "Expecting value" in str(e):
                        ...
                        self.validator_comment = (
                            "temporary stopgap success, alethio API failed/re-running"
                        )
                ...
                zksync_contract = w3.eth.contract(
                    address=zksync_contract_addr, abi=zksync_abi
                )
                parsed_logs = zksync_contract.events.OnchainDeposit().processReceipt(
                    receipt
                )
            ...
            if case_number == 2 or case_number == 3:
                ...
                #     all be indentical
                base_url = (
                    "https://rinkeby-api.zksync.io/api/v0.1"
                    if network == "rinkeby"
                    else "https://api.zksync.io/api/v0.1"
                )
                r = requests.get(
                    f"{base_url}/account/{gitcoin_zksync_addr}/history/older_than"
                )
                r.raise_for_status()
                ...
                token = (
                    Token.objects.filter(
                        network=network, symbol=expected_token, approved=True
                    )
                    .first()
                    .to_dict
                )
                ...
                expected_amount_min = expected_transfer_amount * (
                    Decimal(1 - transfer_tolerance)
                )
                expected_amount_max = expected_transfer_amount * (
                    Decimal(1 + transfer_tolerance)
                )
                ...
                for transaction in transactions:
                    ...
                    is_correct_recipient = (
                        transaction["tx"]["to"].lower() == expected_recipient.lower()
                    )
                    is_correct_token = transaction["tx"]["token"] == expected_token

                    transfer_amount = Decimal(transaction["tx"]["amount"])
                    is_correct_amount = (
                        transfer_amount > expected_amount_min
                        and transfer_amount < expected_amount_max
                    )
                ...
                if (
                    not is_correct_recipient
                    or not is_correct_token
                    or not is_correct_amount
                ):
        ...
        try:
            Earning.objects.update_or_create(
                source_type=ContentType.objects.get(
                    app_label="grants", model="contribution"
                ),
...
def presave_contrib(sender, instance, **kwargs):
    ...
    instance.normalized_data = {
        ...
        "url": grant.url,
        "title": grant.title,
        "amount_per_period_minus_gas_price": float(
            instance.subscription.amount_per_period_minus_gas_price
        ),
        "amount_per_period_to_gitcoin": float(
            instance.subscription.amount_per_period_to_gitcoin
        ),
...
class CLRMatch(SuperModel):
    ...
    has_passed_kyc = models.BooleanField(
        default=False, help_text=_("Has this grant gone through KYC?")
    )
    ready_for_test_payout = models.BooleanField(
        default=False, help_text=_("Ready for test payout or not")
    )
    ...
    ready_for_payout = models.BooleanField(
        default=False, help_text=_("Ready for regular payout or not")
    )
...
class MatchPledge(SuperModel):
    ...
    active = models.BooleanField(
        default=False, help_text=_("Whether or not the MatchingPledge is active.")
    )
    profile = models.ForeignKey(
        "dashboard.Profile",
    ...
    pledge_type = models.CharField(
        max_length=15,
        choices=PLEDGE_TYPES,
        default="tech",
        help_text=_("CLR pledge type"),
    )
    comments = models.TextField(default="", blank=True, help_text=_("The comments."))
...
class PhantomFunding(SuperModel):
    ...
    def competing_phantum_funds(self):
        return PhantomFunding.objects.filter(
            profile=self.profile, round_number=self.round_number
        )
...
class CartActivity(SuperModel):
    ...
    action = models.CharField(
        max_length=20, choices=ACTIONS, help_text=_("Type of activity")
    )
    metadata = JSONField(
        default=dict, blank=True, help_text=_("Related data to the action")
    )
...
class GrantCollection(SuperModel):
    grants = models.ManyToManyField(
        blank=True,
        to=Grant,
        help_text=_("References to grants related to this collection"),
    )
    profile = models.ForeignKey(
        "dashboard.Profile",
        help_text=_("Owner of the collection"),
        related_name="curator",
        on_delete=models.CASCADE,
    )
    title = models.CharField(max_length=255, help_text=_("Name of the collection"))
    description = models.TextField(
        default="", blank=True, help_text=_("The description of the collection")
    )
    cover = models.ImageField(
        ...
        blank=True,
        max_length=500,
        help_text=_("Collection image"),
    )
    hidden = models.BooleanField(
        default=False, help_text=_("Hide the collection"), db_index=True
    )
    cache = JSONField(
        default=dict,
        blank=True,
        help_text=_("Easy access to grant info"),
    )
    ...
    curators = models.ManyToManyField(
        blank=True, to="dashboard.Profile", help_text=_("List of allowed curators")
    )
            ...
            "grants": [
                {
                    "id": grant.id,
                    "logo": grant.logo.url
                    if grant.logo and grant.logo.url
                    else f"v2/images/grants/logos/{self.id % 3}.png",
                }
                for grant in grants
            ],
...
class GrantStat(SuperModel):
    SNAPSHOT_TYPES = [("total", "total"), ("increment", "increment")]
