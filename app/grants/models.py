...
class Contribution(SuperModel):
    ...
    def update_tx_status(self):
        ...
        try:
            ...
            # as noted in the comments below, split_tx_id may be an address, so we check that first
            is_split_tx_id_address = len(self.split_tx_id) == 42 and self.split_tx_id[0:2] == '0x'
            if not is_split_tx_id_address and self.split_tx_id:
            ...
            # First determine if we are in case 1, 2, or 3
            case_number = None
            if is_split_tx_id_address:
                # Case 3 -- this is an address
                case_number = 3
                isZkSyncDeposit = True
            else:
                # Determine if this is Case 2 (deposit into zkSync) or Case 1
            ...
            # Now we handle based on the scenarion we're in
