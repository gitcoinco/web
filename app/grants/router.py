...
class GrantViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if "title" in param_keys:
            queryset = queryset.filter(
                title_iexact=self.request.query_params.get("title")
            )

        # Filter by description.
        if "description" in param_keys:
            queryset = queryset.filter(
                description_iexact=self.request.query_params.get("description")
            )
        ...
        if "grant_type" in param_keys:
            queryset = queryset.filter(
                grant_type=self.request.query_params.get("grant_type")
            )
    ...
    def report(self, request):
        ...
        contributions_queryset = contributions_queryset.filter(
            subscription__contributor_address=eth_address
        )
        ...
        if "from_timestamp" in param_keys:
            ...
            try:
                ...
                raise NotFound(
                    detail="Please provide from_timestamp in the format: " + format
                )
        else:
            from_timestamp = datetime(1, 1, 1, 0, 0)
        grants_queryset = grants_queryset.filter(
            subscriptions__subscription_contribution__created_on__gte=from_timestamp
        )
        ...
        if "to_timestamp" in param_keys:
            ...
            try:
                ...
                raise NotFound(
                    detail="Please provide to_timestamp in the format: " + format
                )
        else:
            to_timestamp = datetime.now()
        grants_queryset = grants_queryset.filter(
            subscriptions__subscription_contribution__created_on__lte=to_timestamp
        )
        ...
        response = Response({"grantee": grantee_data, "donor": donor_data})
