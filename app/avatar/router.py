...
class UserAvatarsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return (
            BaseAvatar.objects.all()
            .order_by("-id")
            .filter(profile__pk=self.request.user.profile.pk)
        )
...
class RecommendedByStaffAvatarsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return (
            CustomAvatar.objects.all().order_by("-id").filter(recommended_by_staff=True)
        )
...
router.register(
    r"recommended-by-staff",
    RecommendedByStaffAvatarsViewSet,
    basename="recommended_by_staff_avatars",
)
