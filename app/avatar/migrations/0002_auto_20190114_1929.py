    ...
    for avatar in social_avatar_model.objects.all():
        try:
            avatar_img = ImageOps.fit(
                Image.open(BytesIO(avatar.png.read())), (215, 215), Image.ANTIALIAS
            )
            avatar.png.save(
                f"{avatar.profile.handle}.png",
                ContentFile(get_temp_image_file(avatar_img).getvalue()),
                save=True,
            )
            avatar.save()
        ...
        try:
            if not base_avatar.png:
                ...
                tmpfile_io = convert_img(
                    base_avatar.svg, input_fmt="svg", output_fmt="png"
                )
            ...
            logger.error(
                "Could not generate hash for avatar pk (%s), error (%s)",
                base_avatar.pk,
                e,
            )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunSQL(
            "create table avatar_avatar_tmp as select * from avatar_avatar;"
        ),
            ...
            fields=[
                ...
                (
                    ...
                    django.contrib.postgres.fields.jsonb.JSONField(
                        default=dict, help_text="The JSON configuration."
                    ),
                ),
            ],
            options={
        ...
        migrations.RunSQL(
            "insert into avatar_customavatar (baseavatar_ptr_id, recommended_by_staff, config) select ba.id as baseavatar_ptr_id, false asrecommended_by_staff, aa.config as config  from avatar_avatar_tmp aa join avatar_baseavatar ba on ba.source_avatar_id = aa.id where ba.is_custom = true"
        ),
        migrations.RunSQL(
            "insert into avatar_socialavatar (baseavatar_ptr_id) select id from avatar_baseavatar where is_custom = false"
        ),
