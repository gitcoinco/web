...
def get_avatar_text_if_any():
    obj = AvatarTextOverlayInput.objects.filter(
        num_uses_remaining__gt=0, active=True
    ).first()
    if obj:
...
def get_avatar_tone_map(tone="skin", skinTone="", theme="unisex"):
    ...
    for key in tones.keys():
        delta = sub_rgb_array(
            hex_to_rgb_array(key), hex_to_rgb_array(base_3d_tone), False
        )
    ...
    if avatar_3d_base_path:
        with open(avatar_3d_base_path) as file:
            ...
            for item in tree.getroot():
                include_item = (
                    item.attrib.get("id") in accept_ids
                    or item.tag in tags
                    or "gradient" in item.attrib.get("id", "").lower()
            ...
            for _type in tone_maps:
                ...
                if base_tone:
                    for _from, to in get_avatar_tone_map(
                        _type, base_tone, theme
                    ).items():
