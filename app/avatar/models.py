...
class BaseAvatar(SuperModel):
    ...
    svg = models.FileField(
        ...
        null=True,
        blank=True,
        help_text=_("The avatar SVG."),
    )
    png = models.ImageField(
        upload_to=get_upload_filename,
        null=True,
        blank=True,
        help_text=_("The avatar PNG."),
    )
    ...
    def find_similar(self):
        if self.hash:
            return BaseAvatar.objects.filter(
                profile=self.profile, hash=self.hash
            ).last()
        ...
        try:
            ...
            if output_fmt == "svg":
                tmpfile_io = convert_wand(
                    source, input_fmt=input_fmt, output_fmt=output_fmt
                )
            else:
...
class CustomAvatar(BaseAvatar):
    ...
    def create(cls, profile, config_json):
        ...
        try:
            ...
            avatar.hash = BaseAvatar.calculate_hash(
                Image.open(BytesIO(avatar.png.read()))
            )
    ...
    def create_3d(cls, profile, params, svg):
        ...
        with NamedTemporaryFile(mode="w+", suffix=".svg") as tmp:
            ...
            with open(tmp.name) as file:
                svg_name = (
                    profile.handle if profile and profile.handle else token_hex(8)
                )
...
def psave_avatar(sender, instance, **kwargs):
    if kwargs[
        "raw"
    ]:  # Skip adding updated_avatar activity if loading avatars from fixture
        return
    from dashboard.models import Activity

    metadata = {
        "url": instance.png.url if getattr(instance, "png", False) else None,
    }
    Activity.objects.create(
        profile=instance.profile, activity_type="updated_avatar", metadata=metadata
    )
...
class AvatarTheme(SuperModel):
    ...
    def __str__(self):
        """Return the str representing this avatar."""
        return (
            f"{self.humanized_name}, Active {self.active} -- {self.popularity} created"
        )
