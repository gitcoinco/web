...
def save_github_avatar(request):
    ...
    if (
        ...
        return JsonResponse(
            {"status": 405, "message": "Authentication required"}, status=405
        )
...
def handle_avatar(request, _org_name="", add_gitcoincologo=False):
    ...
    if _org_name:
        try:
            profile = (
                Profile.objects.prefetch_related("avatar_baseavatar_related")
                .filter(handle=_org_name.lower())
                .first()
            )
            if profile and profile.active_avatar_nocache:
                (
                    avatar_file,
                    content_type,
                ) = profile.active_avatar_nocache.determine_response(
            ...
            logger.error(
                "Handle Avatar - Exception: (%s) - Handle: (%s)", str(e), _org_name
            )
            logger.exception(e)
    ...
    try:
        ...
        logger.error(
            "Handle Avatar - Response error: (%s) - Handle: (%s)", str(e), _org_name
        )
        logger.exception(e)
        return get_err_response(request, blank_img=(_org_name == "Self"))
