...
def get_avatar_context_for_user(user):
    ...
    context["has_purchased_everything_package"] = purchases.filter(
        purchase__option="all"
    ).exists()
    for i in range(0, len(context["sections"])):
        purchase_objs = purchases.filter(
            purchase__option=context["sections"][i]["name"]
        )
        if context["has_purchased_everything_package"]:
            context["sections"][i]["purchases"] = [
                obj for obj in context["sections"][i]["options"]
            ]
        else:
            context["sections"][i]["purchases"] = [
                obj.purchase["value"] for obj in purchase_objs
            ]
        ...
        "sections": [
            ...
            },
            {
                ...
                "options": (
                    ...
                    ["None", "womanhair5"],
                    ["None", "man-hair"],
                    ["None", "girl_hairstyle1"],
                    ["None", "girl_hairstyle2"],
                    ["None", "girl_hairstyle3"],
                    ["None", "men_hairstyle1"],
                    ["None", "men_hairstyle2"],
                ),
                "paid_options": {},
...
def build_avatar_svg(
    ...
    for component in customizable_components:
        if component in payload:
            primary_color = payload.get(component, {}).get(
                "primary_color"
            ) or payload.get("skin_tone")
            components.append(
                build_temporary_avatar_component(
                    component_category=component,
                    component_type=payload.get(component, {}).get(
                        "item_type", "cardigan"
                    ),
                    primary_color=primary_color,
                )
        ...
        if component in payload:
            components.append(
                build_avatar_component(
                    f"{component.title()}/{payload.get(component, '0')}.svg", icon_size
                )
            )
    ...
    for component in multi_components:
        if component in payload:
            components.append(
                build_avatar_component(f"{component.title()}/{payload.get(component)}")
            )
...
def build_random_avatar(
    override_skin_tone=None, override_hair_color=None, add_facial_hair=True
):
    ...
    for section in context["sections"]:
        ...
        set_optional = (
            random.choice([False, True]) if section_name in optional else True
        )
        ...
        if set_optional == True:
            ...
            if section_name == "Wallpaper":
                ...
                payload["HairLong"] = (
                    f'{default_path}{section_name}/{random_choice[0]}-{payload["HairColor"]}.svg'
                    if random_choice[0] != "None"
                    else None
                )
                payload["HairShort"] = (
                    f'{default_path}{section_name}/{random_choice[1]}-{payload["HairColor"]}.svg'
                    if random_choice[1] != "None"
                    else None
                )
                ...
                if add_facial_hair:
                    ...
                    payload[
                        key
                    ] = f'{default_path}{section_name}/{random_choice}-{payload["HairColor"]}.svg'
                ...
                color = (
                    (f'-{payload["SkinTone"]}')
                    if section_name in ["Head", "Ears"]
                    else (
                        (f'-{payload["ClothingColor"]}')
                        if section_name == "Clothing"
                        else ""
                    )
                )
...
def handle_avatar_payload(body):
    ...
    for k, v in body.items():
        if v and k in valid_component_keys:
            component_type, svg_asset = v.lstrip(
                f"{settings.STATIC_URL}v2/images/avatar/"
            ).split("/")
            avatar_dict[k] = {
...
def add_gitcoin_logo_blend(avatar, icon_size):
    ...
    gitcoin_avatar = ImageOps.fit(
        gitcoin_avatar, (sub_avatar_size, sub_avatar_size), Image.ANTIALIAS
    )
...
def svg_to_png(
    svg_content, width=100, height=100, scale=1, index=None, prefer=None, extra_flags=""
):
