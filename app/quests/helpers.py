...
def record_award_helper(qa, profile, layer=1, action="Beat", value_multiplier=1):
    ...
    if layer > 1 or settings.DEBUG:
        ...
        send_notification_to_user(
            gitcoinbot.user, profile.user, cta_url, cta_text, msg_html
        )

    # recursively record points for your referals quest
    if profile.referrer:
        return record_award_helper(
            qa, profile.referrer, layer + 1, action, value_multiplier
        )
...
def get_base_quest_view_params(user, quest):
    ...
    attempts = (
        quest.attempts.filter(profile=profile)
        if profile
        else QuestAttempt.objects.none()
    )
    ...
    params = {
        ...
        + quest.title
        + (
            f"* Gitcoin Quest and win a *{quest.kudos_reward.humanized_name}* Kudos"
...
def get_leaderboard(max_entries=25, round_number=1):
    try:
        return (
            JSONStore.objects.filter(view="quests", key=f"leaderboard_{round_number}")
            .order_by("-pk")
            .first()
            .data
        )
...
def generate_leaderboard(max_entries=25, round_number=1):
    ...
    for ele in leaderboard:
        counter += 1
        btr = BulkTransferRedemption.objects.filter(
            coupon__tag="quest", redeemed_by__handle=ele[0]
        ).order_by("-created_on")
        kudii = list(
            set(
                [
                    (_ele.coupon.token.img_url, _ele.coupon.token.humanized_name)
                    for _ele in btr
                ]
            )
        )[:kudos_to_show_per_leaderboard_entry]
        display_pts = int(ele[1]) if not ele[1] % 1 else round(ele[1], 1)
        reward_kudos_pk = reward_kudos.get(counter)
        reward_kudoses = (
            Token.objects.get(pk=reward_kudos_pk) if reward_kudos_pk else None
        )
        reward_kudos_url = [None, None]
        if reward_kudoses:
            reward_kudos_url = [
                reward_kudoses.preview_img_url,
                reward_kudoses.humanized_name,
            ]
