...
def details(request, quest):
    ...
    params['hint'] = quest.questions.get('hint')
    params['cateogry'] = quest.questions.get('category')
    params['word'] = quest.questions.get('word')
    params['started'] = request.POST.get('start', '')
    response = TemplateResponse(request, 'quests/types/hangman_style.html', params)
    return response
