...
def editquest(request, pk=None):
    ...
    if package:
        ...
        if validation_pass:
            ...
            game_metadata = {
                "video": package.get("background").replace("back", "bg") + ".mp4"
                if package.get("video_enabled", 0)
                else None,
                "enemy": {
                    "id": enemy.pk,
                    "art": enemy.img_url,
            ...
            try:
                ...
                if quest.is_dead_end:
                    messages.warning(
                        request,
                        "Warning: this quest has at least one dead end question (no possible answers). It won't be approved until that is corrected.",
                    )
            except Exception as e:
                if settings.DEBUG:
                    raise e
    ...
    if pk and not package.get("title"):
        ...
        package["reading_material_url"] = quest.game_schema.get("prep_materials", [{}])[
            0
        ].get("url")
        package["reading_material_name"] = quest.game_schema.get(
            "prep_materials", [{}]
        )[0].get("title")
    ...
    params = {
        ...
        "avatar_url": request.build_absolute_uri(
            static("v2/images/twitter_cards/tw_cards-05.png")
        ),
...
def get_package_helper(quest_qs, request):
    return [
        (
            ele.is_unlocked_for(request.user),
            ele.is_beaten(request.user),
            ele.is_within_cooldown_period(request.user),
            ele,
        )
        for ele in quest_qs
    ]
...
def index(request):
    ...
    if show_quests:
        ...
        if focus_hackathon:
            quest_qs = Quest.objects.filter(visible=True).filter(
                unlocked_by_hackathon__slug=focus_hackathon
            )
        ...
        if query:
            quest_qs = (
                ...
                .filter(
                    ...
                    | Q(game_schema__icontains=query)
                    | Q(game_metadata__icontains=query)
                )
                .order_by("-ui_data__success_pct")
            )
            quest_package = get_package_helper(quest_qs, request)
        ...
        if request.user.is_authenticated:
            ...
            if attempts.exists():
                beaten = Quest.objects.filter(
                    pk__in=attempts.filter(success=True).values_list("quest", flat=True)
                )
                unbeaten = Quest.objects.filter(
                    pk__in=attempts.filter(success=False)
                    .exclude(quest__in=beaten)
                ...
                created_quests = request.user.profile.quests_created.filter(
                    visible=True
                )
                if created_quests:
                    quests.append(
                        ("Created", get_package_helper(created_quests, request))
                    )

        print(f" phase1.2 at {round(time.time(),2)} ")
        # difficulty tab
        for diff in Quest.DIFFICULTIES:
            quest_qs = Quest.objects.filter(difficulty=diff[0], visible=True).order_by(
                "-ui_data__success_pct"
            )
            quest_package = get_package_helper(quest_qs, request)
        ...
        new_quests = Quest.objects.filter(
            visible=True,
            created_on__gt=(timezone.now() - timezone.timedelta(hours=hours_new)),
        ).order_by("-ui_data__success_pct")
        ...
        # popular quests
        popular = Quest.objects.filter(visible=True).order_by(
    ...
    point_history = (
        request.user.profile.questpointawards.all()
        if request.user.is_authenticated
        else QuestPointAward.objects.none()
    )
    ...
    quests_attempts_per_day = (
        abs(
            round(
                ...
                / (QuestAttempt.objects.first().created_on - timezone.now()).days,
                1,
            )
        )
        if QuestAttempt.objects.count()
        else 0
    )
    ...
    params = {
        ...
        "quests": quests,
        "avg_play_count": round(
            QuestAttempt.objects.count() / (Quest.objects.count() or 1), 1
        ),
        ...
        "gitcoin_created": Quest.objects.filter(visible=True)
        .filter(creator=Profile.objects.filter(handle="gitcoinbot").first())
        .count(),
        "community_created": Quest.objects.filter(visible=True)
        .exclude(creator=Profile.objects.filter(handle="gitcoinbot").first())
        .count(),
        "country_count": 87,
        ...
        "REFER_LINK": f"https://gitcoin.co/quests/?cb=ref:{request.user.profile.ref_code}"
        if request.user.is_authenticated
        else None,
...
def details(request, obj_id, name, allow_feedback=False):
    ...
    try:
        ...
        if not quest.is_unlocked_for(request.user):
            unlock_how = (
                f"Unlock this quest by registering for {quest.unlocked_by_hackathon.name} Hackathon"
                if quest.unlocked_by_hackathon
                else f"Unlock by beating the '{quest.self.unlocked_by_quest}' Quest"
            )
            messages.info(
                request,
