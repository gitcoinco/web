...
class Quest(SuperModel):
    ...
    kudos_reward = models.ForeignKey(
        "kudos.Token",
        blank=True,
        null=True,
        related_name="quests_reward",
        on_delete=models.SET_NULL,
    )
    unlocked_by_quest = models.ForeignKey(
        "quests.Quest",
        blank=True,
        null=True,
        related_name="unblocks_quest",
        on_delete=models.SET_NULL,
    )
    unlocked_by_hackathon = models.ForeignKey(
    ...
    difficulty = models.CharField(
        max_length=100, default="Beginner", choices=DIFFICULTIES, db_index=True
    )
    style = models.CharField(max_length=100, default="quiz", choices=STYLES)
    value = models.FloatField(default=1)
    background = models.CharField(
        default="", max_length=100, blank=True, choices=BACKGROUNDS
    )
    ...
    def feedbacks(self):
        ...
        return_me = {"ratio": ratio_upvotes, "stats": stats, "feedback": []}
        for fb in self.feedback.all():
    ...
    def is_dead_end(self):
        ...
        for question in self.questions:
            is_this_question_dead_end = not any(
                ele["correct"] for ele in question["responses"]
            )
    ...
    def is_unlocked_for(self, user):
        ...
        if self.unlocked_by_quest:
            is_unlocked = user.profile.quest_attempts.filter(
                success=True, quest=self.unlocked_by_quest
            ).exists()

        if self.unlocked_by_hackathon:
            from dashboard.models import HackathonRegistration

            is_unlocked = HackathonRegistration.objects.filter(
                hackathon=self.unlocked_by_hackathon, registrant=user.profile
            ).exists()
        return is_unlocked
    ...
    def is_beaten(self, user):
        ...
        is_completed = user.profile.quest_attempts.filter(
            success=True, quest=self
        ).exists()
        return is_completed
    ...
    def is_within_cooldown_period(self, user):
        ...
        is_completed = user.profile.quest_attempts.filter(
            ...
            quest=self,
            created_on__gt=(
                timezone.now() - timezone.timedelta(minutes=cooldown_period_mins)
            ),
        ).exists()
        return is_completed
...
class QuestFeedback(SuperModel):
    ...
    def __str__(self):
        """Return the string representation of this obj."""
        return (
            f"{self.pk}, {self.profile.handle} => {self.quest.title} ({self.comment})"
        )


class QuestPointAward(SuperModel):

    questattempt = models.ForeignKey(
        "quests.QuestAttempt", related_name="pointawards", on_delete=models.CASCADE
    )
    profile = models.ForeignKey(
        "dashboard.Profile",
        on_delete=models.CASCADE,
