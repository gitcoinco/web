window.addEventListener('load', function() {
  ...
  $(document).on('click', '.accordion-set-own-limit', function() {
    $(this).parent().find('.recommended-prices').removeClass('active');
    if ($(this).find('i').hasClass('fa-angle-down')) {
      $(this).parent().find('.metamask_context').show();
      $(this).find('i').removeClass('fa-angle-down').addClass('fa-angle-up');
    } else {
      $(this).parent().find('.metamask_context').hide();
      $(this).find('i').removeClass('fa-angle-up').addClass('fa-angle-down');
    }
  });

  $(document).on('click', '.recommended-prices', function() {
    ...
    $('.recommended-prices').removeClass('active');
    $(this).addClass('active');
    $(this).parent().parent().find('#gas-usd').html('$' + usdAmount);
    $(this).parent().parent().find('#gas-eth').html(ethAmount + 'ETH');
    gasPrice = Math.round(gasPrice * 10) / 10.0;
    if (gasPrice < 0.1) {
      gasPrice = 0.1;
    }
    $(this).parent().parent().parent().parent().find('#gasPrice').val(gasPrice);
