...
var cloneKudos = function(name, numClones) {
  ...
  const kudosContractInstance = new window.web3.eth.Contract(kudos_abi, kudos_address());

  kudosContractInstance.methods.clone(name, numClones).send(
    {
      from: account,
      value: window.web3.utils.toBN(1000000000000000)
    },
    function(error, txid) {
