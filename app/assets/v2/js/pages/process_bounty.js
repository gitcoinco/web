...
window.onload = function () {
  ...
  setTimeout(function () {
    ...
    $('#tipPercent').change(function () {
      let is_valid = $(this).val() > 0 && !isNaN($(this).val());
      if (!is_valid) {
        $(this).val(0);
      }
    ...
    var fulfillmentCallback = function (results, status) {
      if (status !== 'success') {
        _alert({
          message: gettext('Could not get fulfillment details')
        }, 'warning');
        console.error(error);
      ...
      if (result === null) {
        _alert({
          message: gettext('No bounty fulfillments found for this Github URL.  Please use the advanced payout tool instead.')
        }, 'warning');
        unloading_button($('.submitBounty'));
        return;
      }
    ...
    $('#acceptBounty').on('click', function (e) {
      ...
      if ($('#terms:checked').length === 0) {
        _alert({
          message: gettext('Please accept the terms of service.')
        }, 'warning');
        isError = true;
      } else {
      ...
      if (issueURL === '') {
        _alert({
          message: gettext('Please enter a issue URL.')
        }, 'warning');
        isError = true;
      }
      if (fulfillmentId === null) {
        _alert({
          message: gettext('Please enter a fulfillment Id.')
        }, 'warning');
        isError = true;
      }
      if (isError) {
      ...
      var apiCallback = function (results, status) {
        if (status !== 'success') {
          _alert({
            message: gettext('Could not get bounty details')
          }, 'warning');
          console.error(error);
          unloading_button($('.submitBounty'));
          return;
        ...
        var final_callback = function (error, result) {
          ...
          var next = function () {
            ...
            _alert({
              message: gettext('Submitted transaction to web3, saving comment(s)...')
            }, 'info');

            var finishedComment = function () {
              _alert({
                message: gettext('Submitted transaction to web3.')
              }, 'info');
              setTimeout(() => {
          ...
          if (error) {
            _alert({
              message: gettext('There was an error')
            }, 'error');
            console.error(error);
            unloading_button($('.submitBounty'));
          } else {
        ...
        var send_payout = function () {
          bounty.acceptFulfillment(bountyId, fulfillmentId, {
