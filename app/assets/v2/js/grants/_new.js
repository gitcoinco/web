...
Vue.mixin({
  methods: {
    ...
    checkForm: async function(e) {
      ...
      } else if (vm.chainId == 'eth' && !vm.form.eth_recipent_address) {
        vm.$set(vm.errors, 'eth_recipent_address', 'Please enter ETH address');
      } else if (
        vm.chainId == 'zcash' &&
            (!vm.form.zcash_recipent_address || !vm.form.zcash_recipent_address.toLowerCase().startsWith('t'))
      ) {
        vm.$set(vm.errors, 'zcash_recipent_address', 'Please enter transparent ZCash address');
      }
    ...
    submitForm: async function(event) {
      ...
      const params = {
        ...
        'description_rich': form.description_rich,
        'team_members[]': form.team_members,
        'handle1': form.twitter_handle_1,
        'handle2': form.twitter_handle_2,
        'github_project_url': form.github_project_url,
        'eth_recipent_address': form.eth_recipent_address,
        'zcash_recipent_address': form.zcash_recipent_address,
    ...
    submitGrant(data) {
      ...
      const postGrantData = fetchData(apiUrlGrant, 'POST', data);

      $.when(postGrantData).then((response) => {
        ...
        console.log(err);
        _alert('Unable to create a grant. Please try again later', 'error');
      });
    },
    ...
    data() {
      return {
        ...
        form: {
          ...
          twitter_handle_2: '',
          github_project_url: '',
          eth_recipent_address: '',
          zcash_recipent_address: '',
          grant_type: '',
          grant_categories: [],
          network: 'mainnet'
        },
