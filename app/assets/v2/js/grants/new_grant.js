...
$(document).ready(function() {
  var jsDropEl = $('#js-drop');

  jsDropEl.on('dragover', function(event) {
    event.preventDefault();
    event.stopPropagation();
    $(this).addClass('is-dragging');
  });

  jsDropEl.on('dragleave', function(event) {
    event.preventDefault();
    event.stopPropagation();
    $(this).removeClass('is-dragging');
  });

  jsDropEl.on('drop', function(event) {
  ...
  $('#js-newGrant').validate({
    submitHandler: function(form) {
      if (!web3 || (web3.version.network !== '1' && web3.version.network === '4')) {
