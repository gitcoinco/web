d3 = function() {
  ...
  function d3_selection_attr(name, value) {
    ...
    function attrFunctionNS() {
      ...
      if (x == null)
        this.removeAttributeNS(name.space, name.local); else
  ...
  function d3_mousePoint(container, e) {
    ...
    if (svg.createSVGPoint) {
      ...
      if (d3_mouse_bug44083)
        point.x = e.pageX, point.y = e.pageY; else
        point.x = e.clientX,
        point.y = e.clientY;
  ...
  d3.behavior.zoom = function() {
    ...
    function mousewheeled() {
      ...
      if (mousewheelTimer)
        clearTimeout(mousewheelTimer); else
  ...
  d3.timer = function(callback, delay, then) {
    ...
    if (d3_timer_queueTail)
      d3_timer_queueTail.next = timer; else
  ...
  d3.format = function(specifier) {
    ...
    if (precision != null) {
      if (type == 'g')
        precision = Math.max(1, Math.min(21, precision)); else if (type == 'e' || type == 'f')
    ...
    var bound = {
      ...
      polygonEnd: function() {
        ...
        if (d3_geo_areaRingSum < 0)
          λ0 = -(λ1 = 180), φ0 = -(φ1 = 90); else if (dλSum > ε)
          φ1 = 90; else if (dλSum < -ε)
          φ0 = -90;
        ...
        if (antimeridian) {
          if (λ < λ_) {
            if (angle(λ0, λ) > angle(λ0, λ1))
              λ1 = λ;
          } else if (angle(λ, λ1) > angle(λ0, λ1))
            λ0 = λ;
        } else if (λ1 >= λ0) {
          if (λ < λ0)
            λ0 = λ;
  ...
  function d3_svg_line(projection) {
    ...
    line.interpolate = function(_) {
      ...
      if (typeof _ === 'function')
        interpolateKey = interpolate = _; else
  ...
  function d3_interpolateHcl(a, b) {
    ...
    if (isNaN(bh))
      bh = 0, ah = isNaN(ah) ? b.h : ah; else if (bh > 180)
      bh -= 360; else if (bh < -180)
      bh += 360;
    return function(t) {
  ...
  d3.layout.force = function() {
    ...
    force.start = function() {
      ...
      if (typeof linkDistance === 'function')
        for (i = 0; i < m; ++i)
          distances[i] = +linkDistance.call(this, links[i], i); else
        for (i = 0; i < m; ++i)
          distances[i] = linkDistance;
      strengths = [];
      if (typeof linkStrength === 'function')
        for (i = 0; i < m; ++i)
          strengths[i] = +linkStrength.call(this, links[i], i); else
        for (i = 0; i < m; ++i)
          strengths[i] = linkStrength;
      charges = [];
      if (typeof charge === 'function')
        for (i = 0; i < n; ++i)
          charges[i] = +charge.call(this, nodes[i], i); else
        for (i = 0; i < n; ++i)
          charges[i] = charge;
        ...
        for (i = 3; i < n; i++) {
          ...
          if (isect) {
            if (s1 < s2 || s1 == s2 && b.r < a.r)
              d3_layout_packSplice(a, b = j); else
  ...
  function d3_scale_linearTickRange(domain, m) {
    ...
    if (err <= .15)
      step *= 10; else if (err <= .35)
      step *= 5; else if (err <= .75)
      step *= 2;
  ...
  function d3_scale_log(linear, base, positive, domain) {
    ...
    scale.tickFormat = function(n, format) {
      ...
      if (arguments.length < 2)
        format = d3_scale_logFormat; else if (typeof format !== 'function')
  ...
  d3.svg.brush = function() {
    ...
    brush.clamp = function(z) {
      ...
      if (x && y)
        xClamp = !!z[0], yClamp = !!z[1]; else if (x)
        xClamp = !!z; else if (y)
        yClamp = !!z;
      return brush;
    };
