...
async function isClaimed(user_id) {
    // Get GTC Token contract instance
    let TokenDistributor = new web3.eth.Contract(token_distributor_abi, token_distributor_address());
    
    // Call getCurrentVotes function
    try {
        is_claimed = TokenDistributor.methods.isClaimed(user_id).call({from: selectedAccount})
    } catch(e) {
