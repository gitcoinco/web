      ...
      _alert(
        {
          message:
            gettext(
              "You are not on the right web3 network.  Please switch to "
            ) + document.network
        },
        "error"
      );
  ...
  $(document).on("click", "#receive", function(e) {
    ...
    if (typeof web3 == "undefined") {
      _alert(
        {
          message: gettext(
            "You are not on a web3 browser.  Please switch to a web3 browser."
          )
        },
        "error"
      );
    ...
    if (typeof document.tip == "undefined") {
      _alert(
        { message: gettext("You do not have permission to do that.") },
        "error"
      );
      return;
    }
    ...
    var success_callback = function(err, txid) {
      ...
      if (err) {
        ...
        document.location.href =
          ...
          txid +
          "&forwarding_address=" +
          $("#forwarding_address").val() +
          "&save_addr=" +
          ($("#save_addr").is(":checked") ? "1" : "0");
      }
    };
    ...
    var is_eth =
      document.tip["token_address"] == "0x0" ||
      document.tip["token_address"] ==
    ...
    web3.eth.getTransactionCount(holding_address, function(error, result) {
      ...
      web3.eth.getBalance(holding_address, function(error, result) {
        ...
        if (balance == 0) {
          _alert(
            "You must wait until the senders transaction confirm before claiming this tip."
          );
          return;
        }
        var rawTx;
        ...
        if (is_eth) {
          ...
          web3.eth.estimateGas(rawTx, function(err, gasLimit) {
            ...
            var send_amount = balance
              .minus(gasLimit.times(gas_price_wei))
              .minus(buffer);
          ...
          // send ERC20
          var data = token_contract.transfer.getData(
            forwarding_address,
            amount_in_wei.toString()
          );
          ...
          web3.eth.estimateGas(rawTx, function(err, gasLimit) {
            ...
            var will_fail_at_this_gas_price =
              gas_price_wei * gasLimit > balance;

            if (will_fail_at_this_gas_price) {
              // adjust if gas prices have increased since this tx was created
