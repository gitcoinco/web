...
def get_search(request):
    ...
    all_result_sets = [
        SearchResult.objects.filter(title__icontains=keyword),
        SearchResult.objects.filter(title=keyword),
    ]
    return_results = []
    exclude_pks = []
    for results in all_result_sets:
        if request.user.is_authenticated:
            results = results.filter(
                Q(visible_to__isnull=True) | Q(visible_to=request.user.profile)
            )
        else:
        ...
        inner_results = [
            {
                ...
                "img_url": ele.img_url
                if ele.img_url
                else "/static/v2/images/helmet.svg",
                "source_type": str(
                    str(ele.source_type).replace("token", "kudos")
                ).title(),
            }
            for ele in results
        ]
